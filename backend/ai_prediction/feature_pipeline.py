...
def _safe_float(value: Any, default: float = 0.0, name: str = "") -> float:
    ...
    try:
        ...
        if math.isnan(v) or math.isinf(v):
            if name:
                logger.warning("[FeaturePipeline] NaN/inf in '%s', using %.3f", name, default)
            return default
        return v
    except (TypeError, ValueError):
        if name:
            logger.warning("[FeaturePipeline] invalid '%s'=%r, using %.3f", name, value, default)
        return default


def _broker_hour(signal: Dict[str, Any]) -> int:
    """Q-6: broker_hour from signal takes priority over UTC."""
    if "broker_hour" in signal:
        return int(signal["broker_hour"]) % 24
...
def _london_ny_overlap(hour: int) -> float:
    return 1.0 if 12 <= hour < 16 else 0.0


def _minutes_to_session_open(hour: int, minute: int = 0) -> float:
    opens = [7, 12, 22, 0]
    return (
        min((o - hour) * 60 - minute for o in opens if (o - hour) * 60 - minute >= 0)
        if any((o - hour) * 60 - minute >= 0 for o in opens)
        else 0.0
    )
