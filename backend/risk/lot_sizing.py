...
HF-3: Broker-based pip/tick calculation
  Lookup order:
    ...
    2. Static curated table (39 symbols)
  ...
  Features:
    ...
    - XAGUSD pip_value=50.0 (FIX #4: was 5.0, Silver 5000oz contract)
    - Alias resolution: GOLD->XAUUSD, SILVER->XAGUSD, BTC->BTCUSD ...
    - Broker suffix strip: XAUUSDm->XAUUSD, EURUSDpro->EURUSD ...
"""
...
#   proof: 5000 oz * $0.001/oz tick * 10 pips/tick = $50.00/pip
# FIX #4: AUS200 added (was missing from lot_sizing, present in portfolio_risk)
# ---------------------------------------------------------------------------
...
# FIX #4: Alias table (DRY with portfolio_risk._SYMBOL_ALIASES)
...
class LotSizer:
    ...
    FIX #4: Alias + suffix resolution added so SILVER, XAUUSDm etc. work.
    ...
    async def calculate(
        self, balance: float, stop_loss_pips: float, symbol: str,
        atr_pips: float = 0.0, win_rate: float = 0.55,
        avg_rr: float = 1.5, volatility_ratio: float = 1.0,
    ) -> LotSizeResult:
        sym = symbol.upper().strip()
        if stop_loss_pips <= 0:
        ...
        kelly_pct = max(0.0, min(0.25,
            (win_rate - (1 - win_rate) / avg_rr) if avg_rr > 0 else 0.0))
        kelly_lot = (
            (balance * kelly_pct * self.config.kelly_fraction)
            / (stop_loss_pips * pip_val)
        )
        ...
        method = (
            "min_fallback" if final_lot <= self.config.min_lot
            else "kelly_blend" if kelly_pct > 0
            else "fixed_risk"
        )
        logger.debug(
            "LotSizer %s: pip=%.4f(%s) sl=%.1f final=%.2f",
            sym, pip_val, source, stop_loss_pips, final_lot,
        )
        return LotSizeResult(
            lot_size=final_lot, pip_value_used=pip_val,
    ...
    async def _resolve_pip_value(self, sym: str) -> Tuple[float, str]:
        """FIX #4: cache -> MT5 -> static table (alias + suffix) -> UnknownSymbolError."""
        now    = time.monotonic()
        cached = self._cache.get(sym)
        if cached and (now - cached[1]) < _PIP_CACHE_TTL:
            return cached[0], cached[2]

        # 1. MT5 terminal (most accurate, broker-specific)
        ...
        # 2. Static table with alias + suffix resolution
        canonical = self._resolve_canonical(sym)
        static    = _PIP_VALUE_TABLE.get(canonical)
        if static is not None:
            src = "static_table" if canonical == sym else f"static_table({canonical})"
            self._cache[sym] = (static, now, src)
            return static, src
    ...
    @staticmethod
    def _resolve_canonical(sym: str) -> str:
        """FIX #4: Alias lookup + broker suffix strip (1-4 chars)."""
...
def get_lot_sizer(
    config: Optional[LotSizingConfig] = None, mt5_connector=None
) -> LotSizer:
    global _lot_sizer
