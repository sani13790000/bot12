...
class EquityProtectionEngine:
    ...
    def initialize(self, balance: float) -> None:
        ...
        if balance <= 0:
            logger.debug(
                "balance invalid, using fallback", balance=balance, fallback=_DEFAULT_BALANCE
            )
    ...
    def _evaluate(self) -> None:
        ...
        if (
            drawdown_pct >= config.total_drawdown_limit_pct
            or daily_pct >= config.daily_loss_limit_pct
        ):
            reason = (
                f"drawdown={drawdown_pct:.1f}%"
                if drawdown_pct >= config.total_drawdown_limit_pct
...
async def persist_halt_state(ep_engine: EquityProtectionEngine, db: Any) -> None:
    try:
        await db.upsert(
            "equity_protection_state",
            {
                "status": ep_engine.state.status.value,
                "halt_reason": ep_engine.state.halt_reason,
                "halted_at": ep_engine.state.halted_at.isoformat()
                if ep_engine.state.halted_at
                else None,
            },
        )
