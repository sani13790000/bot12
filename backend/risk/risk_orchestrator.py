...
FIX #10 - Execution trace audit fixes:
  ISSUE-1: RiskInput.extra_ctx field added; win_rate/avg_rr moved from direct fields
           to extra_ctx dict - was TypeError: __init__() got unexpected kwarg 'win_rate'
  ISSUE-2: LotSizer.calculate() now receives win_rate/avg_rr from ctx
           via **inp.extra_ctx forwarding in assess() -> check(**ctx)
"""
...
class RiskInput:
    ...
    # ISSUE-1 FIX: extra_ctx carries Kelly parameters (win_rate, avg_rr) from signal
    # without polluting the fixed RiskInput interface with LotSizer internals.
...
class RiskOrchestrator:
        ...
        if self._lot_sizer is not None:
            try:
                lot_res = await self._lot_sizer.calculate(
                    ...
                    # ISSUE-2 FIX: Kelly blend uses real signal win_rate/avg_rr.
                    # Previously always used LotSizer defaults (0.55 / 1.5).
                    # Now forwarded from RiskInput.extra_ctx via **inp.extra_ctx in assess().
                    win_rate=float(ctx.get("win_rate", 0.55)),
                    avg_rr  =float(ctx.get("avg_rr",   1.5)),
                )
    ...
    async def assess(self, inp: "RiskInput") -> "RiskCheckResult":
        ...
        return await self.check(
            ...
            # ISSUE-2 FIX: forward Kelly params from signal to LotSizer via ctx.
            **inp.extra_ctx,
        )
...
async def get_risk_orchestrator() -> RiskOrchestrator:
    ...
    try:
        ...
        from backend.risk.lot_sizing import get_lot_sizer
        return RiskOrchestrator(
            equity_guard       = get_equity_protection(),
            daily_limits       = DailyLimitsEngine(),
            volatility_filter  = get_volatility_filter(),
            correlation_filter = CorrelationFilter(),
            exposure_control   = get_exposure_control(),
            lot_sizer          = get_lot_sizer(),
        )
    except Exception as e:
        logger.warning("get_risk_orchestrator fallback (no gates): %s", e)
        return RiskOrchestrator()
