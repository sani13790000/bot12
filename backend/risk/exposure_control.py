...
try:
    from backend.risk.fail_mode import FailMode
    from backend.risk.fail_mode import coerce as _coerce_fail_mode
except ImportError:
...
def _open_snapshot() -> ExposureSnapshot:
    return ExposureSnapshot(
        ...
        sell_trades=0,
        can_open_new=True,
        block_reason="FAIL_OPEN_EXCEPTION_IGNORED",
    )
...
class ExposureControlEngine:
    ...
    def __init__(
        ...
        self._fail_mode = (
            _coerce_fail_mode(fail_mode) if fail_mode is not None else FailMode.FAIL_CLOSED
        )
        ...
        try:
            ...
            if self._fail_mode is FailMode.FAIL_CLOSED:
                ...
                return ExposureCheckResult(
                    can_trade=False,
                    reason=reason,
                    snapshot=_blocked_snapshot(reason),
                    projected_total_risk=0.0,
                )
            logger.critical(
                "FAIL_OPEN: ExposureControl exception swallowed, trade ALLOWED. symbol=%s",
                new_symbol,
            )
            return ExposureCheckResult(
                can_trade=True,
    ...
    def _check_inner(
        ...
        def _block(msg: str) -> ExposureCheckResult:
            ...
            snap.block_reason = msg
            return ExposureCheckResult(False, msg, snap, projected_total)

        if projected_total > cfg.max_total_exposure_percent:
            return _block(
                f"Total exposure {projected_total:.2f}% > limit {cfg.max_total_exposure_percent}%"
            )
        if projected_sym > cfg.max_per_symbol_percent:
            return _block(
                f"Symbol {new_symbol} exposure {projected_sym:.2f}% > limit {cfg.max_per_symbol_percent}%"
            )
        for ccy, val in projected_ccy.items():
            if val > cfg.max_per_currency_percent:
                return _block(
                    f"Currency {ccy} exposure {val:.2f}% > limit {cfg.max_per_currency_percent}%"
                )
