...
==================================
Surgical refactor — 8 fixes applied.

FIX #1  Real News Filter gate (was dead config fields)
FIX #2  Robust ATR baseline  (median replaces arithmetic mean)
FIX #3  Symbol-specific volatility thresholds
FIX #6  Fail-closed mode for every exception path
FIX #7  Dead-code removal (unused asyncio lock, stray singleton init)

Public API is 100 % backward-compatible:
  - VolatilityFilter.check() signature unchanged
  - VolatilityFilterConfig dataclass unchanged (new optional fields added)
  - get_volatility_filter() unchanged
"""
...
# ---------------------------------------------------------------------------
# FIX #6 — Fail mode enum
# ---------------------------------------------------------------------------
class FailMode(str, Enum):
    FAIL_CLOSED = "FAIL_CLOSED"   # exception → block trade  (DEFAULT)
    FAIL_OPEN   = "FAIL_OPEN"     # exception → allow trade


# ---------------------------------------------------------------------------
# FIX #1 — News event dataclass
# ---------------------------------------------------------------------------
@dataclass
class NewsEvent:
    title:      str
    ...
    event_time: datetime             # must be tz-aware (UTC)


# ---------------------------------------------------------------------------
# FIX #3 — Per-symbol threshold bundle
# ---------------------------------------------------------------------------
@dataclass
...
# ---------------------------------------------------------------------------
# Config — backward-compatible (all new fields have defaults)
# ---------------------------------------------------------------------------
@dataclass
class VolatilityFilterConfig:
    # ATR ratio thresholds — GLOBAL defaults (FIX #3 may override per symbol)
    ...
    # FIX #1 — News filter (now actually used)
    news_block_minutes_before: int  = 30
    news_block_minutes_after:  int  = 15
    enable_news_filter:        bool = True

    # FIX #2 — ATR estimator: "median" (default) | "ema" | "mean"
    atr_estimator: str = "median"

    # FIX #3 — Per-symbol threshold overrides  {symbol_upper: SymbolThresholds}
...
class VolatilityCheckResult:
    ...
    avg_spread:      float
    # FIX #1 extras (never break callers that don't read them)
...
class VolatilityFilter:
    ...
    Changes vs original:
      FIX #1  Real news-event gate (was dead config fields).
      FIX #2  avg_atr uses median (or EMA) — spike-robust.
      FIX #3  Per-symbol ATR thresholds with global fallback.
      FIX #6  Fail-closed: any internal exception → EXTREME block.
    """

    def __init__(self, config: Optional[VolatilityFilterConfig] = None) -> None:
        self._cfg = config or VolatilityFilterConfig()
        # FIX #1: live news registry — callers push events via load_news_events()
        self._news_events: List[NewsEvent] = []

    # ------------------------------------------------------------------ #
    # FIX #1 — public API to register upcoming news events                #
    # ------------------------------------------------------------------ #
