...
class SecurityScoreEngine:
    ...
    async def _refresh(self) -> None:
        ...
        snap = SecuritySnapshot(
            ...
            trend=_trend(composite, prev_1h),
            dimensions=results,
    ...
    async def _auth_dim(self) -> DimensionScore:
        ...
        try:
            ...
            if f > 100:
                score -= 0.5
                notes.append(f"{f} failed logins/h")
            elif f > 20:
                score -= 0.2
                notes.append(f"{f} failed logins/h")
            elif f > 5:
            ...
            if b > 50:
                score -= 0.3
                notes.append(f"{b} IPs blocked/24h")
            elif b > 10:
                score -= 0.1
    ...
    async def _trading_dim(self) -> DimensionScore:
        ...
        try:
            ...
            if t > 20:
                score -= 0.4
                notes.append(f"{t} suspicious trades/24h")
            elif t > 5:
                score -= 0.2
            try:
                ...
                open_cbs = sum(
                    1
                    for cb in circuit_breaker_manager._breakers.values()
                    if cb.state.value == "open"
                )
                if open_cbs > 0:
                    score -= 0.2 * open_cbs
                    notes.append(f"{open_cbs} CBs OPEN")
            except Exception:
                pass
        except Exception as ex:
            notes.append(str(ex))
        ...
        return DimensionScore(
            "trading_security", max(0.0, score), _WEIGHTS["trading_security"], notes
        )
    ...
    async def _session_dim(self) -> DimensionScore:
        ...
        try:
            ...
            if s > 10:
                score -= 0.3
                notes.append(f"{s} session anomalies/24h")
            elif s > 3:
                score -= 0.15
        except Exception as ex:
            notes.append(str(ex))
    ...
    async def _infra_dim(self) -> DimensionScore:
        ...
        try:
            ...
            rc = aioredis.from_url(
                os.getenv("REDIS_URL", "redis://redis:6379/0"), socket_timeout=1.0
            )
            await asyncio.wait_for(rc.ping(), timeout=1.0)
            await rc.aclose()
        except Exception:
            score -= 0.2
            notes.append("Redis unreachable")
        try:
            db = self._db()
            await asyncio.wait_for(
                db.table("security_audit_logs").select("id").limit(1).execute(), timeout=2.0
            )
        except Exception:
            score -= 0.4
            notes.append("DB unreachable")
    ...
    async def _integrity_dim(self) -> DimensionScore:
        ...
        try:
            ...
            r = await asyncio.wait_for(
                db.table("security_audit_logs")
                .select("id", count="exact")
                .eq("event_type", "data_integrity_error")
                .gte("created_at", h24)
                .execute(),
                timeout=_DB_TIMEOUT,
            )
            ...
            if e > 5:
                score -= 0.4
                notes.append(f"{e} integrity errors/24h")
            elif e > 0:
                score -= 0.1
        except Exception as ex:
            notes.append(str(ex))
    ...
    async def _persist(self, snap: SecuritySnapshot) -> None:
        try:
                ...
                .insert(
                    {
                        ...
                        "trend": snap.trend,
                        "dimensions": {d.name: round(d.score * 100, 1) for d in snap.dimensions},
            ...
            log.debug("persist score: %s", e)

    async def _check_thresholds(self, snap: SecuritySnapshot) -> None:
        try:
            from backend.circuit_breaker import circuit_breaker_manager
            from backend.telegram.alerts import alert_circuit_breaker, alert_score_drop
            ...
            if snap.score < _BREAKER_THRESHOLD and not self._breaker_open:
                ...
                asyncio.create_task(
                    alert_circuit_breaker("security_global", "OPEN", f"score {snap.score:.1f}")
                )
                ...
                try:
                    ...
                    asyncio.create_task(
                        alert_circuit_breaker("security_global", "CLOSED", "recovered")
                    )
                except Exception:
                    pass
        except ImportError:
            ...
            log.warning("threshold check: %s", e)
