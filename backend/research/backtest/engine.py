    ...
    def sortino_ratio(returns, risk_free_rate=0.0, periods_per_year=252):
        ...
        try:
            ...
            if downside_std == 0:
                return 0.0
            return round(
                (statistics.mean(returns) - target) / downside_std * math.sqrt(periods_per_year), 4
            )
        except Exception:
            return 0.0
...
class BacktestResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "equity_curve": [
                {"t": p.timestamp, "eq": p.equity, "dd": p.drawdown} for p in self.equity_curve
            ],
        }
...
class BacktestEngine:
    ...
    def _is_valid_signal(sig, cfg):
        ...
        if risk <= 0 or reward <= 0:
            return False
        if reward / risk < cfg.min_rr_ratio:
            return False
        return True
...
def apply_slippage(
    price: float, direction: str, slippage_pips: float, symbol: str = "XAUUSD"
) -> float:
