...
class DeploymentValidator:
    ...
    def lint_dockerfile(self, content: str) -> ValidationResult:
        ...
        non_root = [
            u for u in re.findall(r"^USER\s+(\S+)", content, re.M) if u not in ("root", "0")
        ]
        if not non_root:
    ...
    def full_report(
        self, dockerfile_content: str, compose_content: str, env: Dict[str, str]
    ) -> Dict[str, Any]:
        checks = []
        for name, result in [
            ("dockerfile", self.lint_dockerfile(dockerfile_content)),
            ("compose", self.lint_compose(compose_content)),
            ("env", self.validate_env(env)),
        ]:
