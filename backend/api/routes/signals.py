...
async def list_signals(
    ...
    rows = await db.select(
        "signals", filters, order_by="created_at", order_desc=True, limit=limit, offset=offset
    )
...
async def get_signal(
    signal_id: str, user: dict = Depends(get_current_user), db=Depends(get_db)
) -> SignalResponse:
...
async def create_signal(
    body: SignalCreate, user: dict = Depends(get_current_user), db=Depends(get_db)
) -> SignalResponse:
...
async def execute_signal(
    ...
    if sig.get("status") != "pending":
        raise HTTPException(
            status_code=http_status.HTTP_409_CONFLICT, detail="Signal is not pending"
        )
    confidence = sig.get("confidence", 0.0)
    if confidence < _MIN_CONFIDENCE:
        raise HTTPException(
            status_code=http_status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Signal confidence {confidence:.1f}% below minimum {_MIN_CONFIDENCE:.0f}%",
        )
    now = datetime.now(timezone.utc).isoformat()
    try:
