...
async def list_signals(
    ...
    """List signals for the authenticated user, newest first."""
...
async def create_signal(
    ...
    """Create a new trading signal (status=pending, not executed yet)."""
...
async def execute_signal(
    ...
    """
    Execute a pending signal - opens a real MT5 trade.

    1. Fetch signal - must be pending and belong to user
    2. Check confidence >= 60%
    3. Call ExecutionService -> MT5 open_position()
    4a. Success: update status='executed' + mt5_ticket
    4b. Failure: update status='failed', raise 502
    """
    sig = await db.select_one("signals", {"id": signal_id, "user_id": user["sub"]})
    if not sig:
        raise HTTPException(status_code=404, detail="Signal not found")
    if sig.get("status") != "pending":
        raise HTTPException(
            ...
            detail=f"Signal is not pending (status: {sig.get('status')})",
        )
    ...
    if confidence < _MIN_CONFIDENCE:
        raise HTTPException(
            status_code=http_status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=(
                f"Signal confidence {confidence:.1f}% is below minimum "
                f"{_MIN_CONFIDENCE:.0f}% required for execution"
            ),
        )
    ...
    try:
        ...
        updated = await db.update(
            "signals",
            {"id": signal_id},
            {"status": "executed", "mt5_ticket": ticket, "executed_at": now, "updated_at": now},
        )
        logger.info(
            "Signal executed: id=%s ticket=%s symbol=%s direction=%s",
            signal_id, ticket, sig["symbol"], sig["direction"],
        )
        return {"success": True, "signal_id": signal_id, "mt5_ticket": ticket,
                "signal": updated[0] if updated else sig}
...
async def cancel_signal(
    ...
    """Cancel a pending or active signal."""
    sig = await db.select_one("signals", {"id": signal_id, "user_id": user["sub"]})
    if not sig:
        raise HTTPException(status_code=404, detail="Signal not found")
    if sig.get("status") not in ("pending", "active"):
        raise HTTPException(
            status_code=http_status.HTTP_409_CONFLICT,
            detail=f"Cannot cancel signal with status '{sig.get('status')}'",
        )
    now = datetime.now(timezone.utc).isoformat()
    updated = await db.update(
        "signals",
    ...
    logger.info("Signal cancelled: id=%s", signal_id)
