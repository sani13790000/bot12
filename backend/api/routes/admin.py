...
async def get_config(_user=Depends(_require_admin)) -> Dict[str, Any]:
    ...
    return {field: getattr(settings, field, None) for field in safe_fields if hasattr(settings, field)}
...
async def list_users(
    ...
    try:
        db = get_db_client()
        result = (db.table("users").select("id, email, created_at, is_active, role").range(offset, offset + limit - 1).execute())
...
@router.post("/kill")
async def activate_kill_switch(body: KillSwitchAction, _user=Depends(_require_admin)) -> Dict[str, Any]:
    ...
    return {"status": "activated", "reason": body.reason, "timestamp": datetime.now(timezone.utc).isoformat()}


@router.post("/resume")
async def deactivate_kill_switch(body: KillSwitchAction, _user=Depends(_require_admin)) -> Dict[str, Any]:
    ...
    return {"status": "deactivated", "reason": body.reason, "timestamp": datetime.now(timezone.utc).isoformat()}
...
async def kill_switch_status(_user=Depends(_require_admin)) -> Dict[str, Any]:
    ...
    ks = get_kill_switch()
    return {"active": ks.is_active, "stats": ks.stats(), "timestamp": datetime.now(timezone.utc).isoformat()}
