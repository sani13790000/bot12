...
async def list_trades(
    ...
    rows = await db.select(
        "trades", filters, order_by="opened_at", order_desc=True, limit=limit, offset=offset
    )
...
async def get_trade(
    trade_id: str, user: dict = Depends(get_current_user), db=Depends(get_db)
) -> TradeResponse:
...
async def open_trade(
    body: TradeCreate, user: dict = Depends(get_current_user), db=Depends(get_db)
) -> TradeResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=http_status.HTTP_502_BAD_GATEWAY, detail=f"MT5 execution failed: {exc}"
        )
...
async def close_trade(
    ...
    if trade.get("status") != "open":
        raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail="Trade is not open")
    ticket = trade.get("mt5_ticket")
    if not ticket:
        raise HTTPException(
            status_code=http_status.HTTP_409_CONFLICT, detail="Trade has no MT5 ticket"
        )
    try:
        svc = _get_execution_service()
        ...
        raise HTTPException(
            status_code=http_status.HTTP_502_BAD_GATEWAY, detail=f"MT5 close failed: {exc}"
        )
    now = datetime.now(timezone.utc).isoformat()
    updated = await db.update(
        "trades", {"id": trade_id}, {"status": "closed", "closed_at": now, "pnl": pnl}
    )
