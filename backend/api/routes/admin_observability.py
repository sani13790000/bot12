"""
backend/api/routes/admin_observability.py — Phase 15 (NEW FILE)
...
P15-OBS-ROUTE-8: all require MANAGE_OBSERVABILITY perm
"""
...
async def get_issue_trace(
    ...
    events = tracer.issue_trace(user_id=user_id, trace_id=trace_id, category=category,
                                level=level, since_ts=since_ts, until_ts=until_ts, limit=limit)
    return {"events": events, "total": len(events), "summary": tracer.summary(),
            "query": {"user_id": user_id, "trace_id": trace_id, "category": category}}
...
async def deep_health(
    ...
    status = "healthy"
    if issues: status = "degraded" if len(issues) < 3 else "unhealthy"
    return {"status": status, "issues": issues, "metrics": snap,
            "alert_stats": manager.stats(), "trace_summary": tracer.summary(), "ts": time.time()}
