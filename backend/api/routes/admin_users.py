...
async def change_role(
    ...
    if not rbac_engine.can_escalate_to(ctx, target_role):
        raise HTTPException(
            status_code=403,
            detail=f"Cannot assign role '{target_role}' -- must be strictly below your own",
        )
    ...
    audit_logger.role_changed(
        target_id=user_id, old=old_role, new=target_role, actor_id=ctx.user_id
    )
