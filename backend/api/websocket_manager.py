...
class WebSocketManager:
    ...
    async def connect(self, ws: WebSocket, user_id: str, topics: Optional[List[str]] = None) -> str:
        ...
        async with self._lock:
            ...
            if len(self._conns) >= _MAX_TOTAL:
                ...
                return ""
            conn_id = str(uuid4())
            self._conns[conn_id] = _ConnInfo(
                ws=ws, user_id=user_id, conn_id=conn_id, subscriptions=set(topics or [])
            )
    ...
    async def disconnect(self, ws: WebSocket) -> None:
        async with self._lock:  # T-11
            ...
            if cid:
                await self._evict(cid, "disconnect")
    ...
    async def broadcast(
        ...
        if not isinstance(msg_type, WSMessageType):
            return 0
        ...
        for info in candidates:
            if user_id and info.user_id != user_id:
                continue
            if topic and info.subscriptions and topic not in info.subscriptions:
                continue  # T-12
            try:
    ...
    async def subscribe(self, conn_id: str, topics: List[str]) -> None:
        async with self._lock:
            if conn_id in self._conns:
                self._conns[conn_id].subscriptions.update(topics)

    async def unsubscribe(self, conn_id: str, topics: List[str]) -> None:
        async with self._lock:
            if conn_id in self._conns:
                self._conns[conn_id].subscriptions -= set(topics)
    ...
    async def _ping_loop(self) -> None:  # T-10
        while True:
            ...
            async with self._lock:
                stale = [
                    cid
                    for cid, info in self._conns.items()
                    if now - info.last_ping_ok > _PING_INTERVAL_S + _PING_TIMEOUT_S
                ]
            for cid in stale:
                async with self._lock:
                    await self._evict(cid, "ping_timeout")
            async with self._lock:
                ...
                try:
                    ...
                    async with self._lock:
                        if cid in self._conns:
                            self._conns[cid].last_ping_ok = time.monotonic()
                except Exception:
    ...
    async def _handle_broken(self, conn_id: str) -> None:
        async with self._lock:
            await self._evict(conn_id, "broken_pipe")
