...
def _create_app() -> FastAPI:
    ...
    # CORS middleware.
    # SECURITY FIX: read the real config field (ALLOWED_ORIGINS); the previous
    # code read a non-existent "CORS_ORIGINS" attribute so the configured origins
    # were always ignored. A "*" wildcard is NEVER used together with
    # allow_credentials=True (that combination leaks credentialed responses to any
    # origin), and methods/headers are restricted to an explicit allow-list.
    cors_origins = getattr(settings, "ALLOWED_ORIGINS", None) or getattr(settings, "CORS_ORIGINS", None)
    env = getattr(settings, "ENVIRONMENT", "development")
    if cors_origins and "*" in cors_origins:
        # Wildcard is incompatible with credentials — drop it and fall back to a
        # safe explicit list so we never reflect credentialed responses to "*".
        logger.warning("[CORS] wildcard origin ignored (incompatible with credentials)")
        cors_origins = [o for o in cors_origins if o != "*"]
    if not cors_origins:
        if env == "production":
            logger.warning("[CORS] ALLOWED_ORIGINS not set in production -- defaulting to localhost only")
    ...
    app.add_middleware(
        ...
        allow_headers=["Authorization", "Content-Type", "X-Request-ID", "X-License-Key", "Accept"],
    )
    ...
    from backend.api.routes import (
        ...
        portfolio, rate_limit_routes, reports, research, risk,
        security_ai, security_ai_extended, self_learning, signals,
    ...
    async def root() -> Dict[str, Any]:
        ...
        return {"service": "Galaxy Vast AI Trading Platform", "version": "1.0.0", "status": "running", "uptime_seconds": uptime}
