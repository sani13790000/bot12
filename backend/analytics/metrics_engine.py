...
class MetricsEngine:
    ...
    def calculate(self, trades: List[TradeRecord]) -> PerformanceMetrics:
        ...
        # Expectancy
        m.expectancy = round((m.win_rate * m.avg_win) + ((1 - m.win_rate) * m.avg_loss), 4)
            ...
            r = await asyncio.wait_for(
                asyncio.to_thread(
                    lambda: (
                        db.table("trades")
                        .select(
                            "pnl,entry_price,exit_price,stop_loss,take_profit,direction,opened_at,closed_at,symbol,commission"
                        )
                        .eq("status", "closed")
                        .gte("closed_at", since)
                        .execute()
