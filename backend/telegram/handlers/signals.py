...
from ..keyboards import get_signal_action_keyboard, get_signals_keyboard
...
def register_signal_handlers(dp: Dispatcher) -> None:
    ...
    async def show_active_signals(callback: types.CallbackQuery):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{_API_BASE_URL}/api/signals/active", timeout=10.0)
            ...
            if response.status_code == 200:
                ...
                if not signals:
                    ...
                    for signal in signals[:3]:
                        ...
                        keyboard = get_signal_action_keyboard(signal["id"], can_execute=can_execute)
                        await callback.message.answer(
                            text, reply_markup=keyboard, parse_mode="HTML"
                        )
                    await callback.message.delete()
            else:
                await callback.message.edit_text("❌ خطا در دریافت سیگنال‌ها", parse_mode="HTML")

        except Exception as e:
            logger.error(f"خطا در دریافت سیگنال‌های فعال: {e}", exc_info=True)
            await callback.message.edit_text("❌ خطا در ارتباط با سرور", parse_mode="HTML")
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{_API_BASE_URL}/api/signals/", params={"limit": 10}, timeout=10.0
                )
            ...
            if response.status_code == 200:
                ...
                if not signals:
                    await callback.message.edit_text(
                        "🔭 <b>تاریخچه سیگنال‌ها</b>\n\nهیچ سیگنالی ثبت نشده.", parse_mode="HTML"
                    )
                else:
                    ...
                    for signal in signals[:10]:
                        status_emoji = {"executed": "✅", "expired": "⏰", "skipped": "⏭"}.get(
                            signal.get("status"), "❓"
                        )
                ...
                await callback.message.edit_text("❌ خطا در دریافت تاریخچه", parse_mode="HTML")
    ...
    async def execute_signal(callback: types.CallbackQuery):
        ...
        if not await _rbac_service.check_permission(user_id, Permission.EXECUTE_SIGNAL):
            await callback.answer(
                "⛔️ فقط TRADER و بالاتر می‌توانند سیگنال اجرا کنند", show_alert=True
            )
            logger.warning(f"تلاش غیرمجاز برای اجرای سیگنال — کاربر: {user_id}")
            return
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{_API_BASE_URL}/api/signals/{signal_id}/execute", timeout=30.0
                )
    ...
    async def skip_signal(callback: types.CallbackQuery):
        ...
        try:
            async with httpx.AsyncClient() as client:
                await client.post(f"{_API_BASE_URL}/api/signals/{signal_id}/skip", timeout=10.0)
        except Exception as e:
            logger.warning(f"خطا در رد سیگنال {signal_id}: {e}")

        await callback.message.edit_text("⏭ <b>سیگنال رد شد</b>", parse_mode="HTML")
        await callback.answer()
    ...
    async def remind_signal(callback: types.CallbackQuery):
        ...
        await callback.message.edit_text(
            "🔔 <b>یادآوری تنظیم شد</b>\n\nبه زودی یادآوری دریافت خواهید کرد.", parse_mode="HTML"
        )
        await callback.answer()
