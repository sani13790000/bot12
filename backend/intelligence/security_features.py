    ...
    def to_array(self) -> np.ndarray:
        return np.array(
            [
                ...
                self.trade_volume_change,
                self.session_duration,
                self.endpoint_diversity,
                self.error_rate,
                self.unusual_hour,
                self.geo_change_flag,
                self.payload_size_avg,
                self.repeat_pattern_score,
            ],
            dtype=np.float32,
        )
...
class _IPBuffer:
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        for p in paths:
            path_counts[p] += 1
        entropy = -sum((c / total) * math.log2(c / total) for c in path_counts.values() if c > 0)
...
def _classify_risk(fv: FeatureVector) -> str:
    score = (
        fv.failed_login_ratio * 3.0
        + fv.request_rate * 2.0
        + fv.error_rate * 2.0
        + fv.geo_change_flag * 2.0
        + fv.unusual_hour
        + fv.repeat_pattern_score
...
class SecurityFeatureExtractor:
    ...
    def _maybe_evict(self) -> None:
        if len(self._buffers) <= _MAX_TRACKED:
            return
