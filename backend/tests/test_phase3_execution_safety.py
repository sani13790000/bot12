...
_ALLOWED_TRANSITIONS = {
    ...
    OrderState.SUBMITTED: {OrderState.FILLED, OrderState.PARTIAL, OrderState.FAILED, OrderState.TIMEOUT},
...
class OrderTransition:
    def __init__(self, from_state, to_state, order_id): pass
...
class OrderStateMachine:
    ...
    async def create_order(self, signal_id: str, symbol: str) -> str:
        ...
        async with self._lock:
            self._orders[order_id] = ManagedOrder(order_id=order_id, signal_id=signal_id, symbol=symbol)
        return order_id
    ...
    async def transition(self, order_id: str, new_state: str, **kwargs):
        ...
        for cb in self._callbacks:
            try: await cb(order_id, new_state)
            except: pass
...
class FailureRecoveryEngine:
    ...
    @property
    def dead_letter_queue(self): return list(self._dead_letter)
    ...
    async def handle_failure(self, order_id, signal_id, error, retcode=0, metadata=None):
        ...
        failed = FailedOrder(order_id=order_id, signal_id=signal_id, error=error,
                             retcode=retcode, attempts=1, strategy=strategy,
...
class OrderJournal:
    ...
    async def record(self, event_type, signal_id, order_id=None, **data):
        async with self._lock:
            self._entries.append(JournalEntry(event_type=event_type, order_id=order_id,
                                              signal_id=signal_id, data=data))
    ...
    def stats(self):
        return {'total': len(self._entries),
                'by_type': {t: sum(1 for e in self._entries if e.event_type == t)
...
class BrokerConnectionError(Exception): pass
class RiskBlockedError(Exception): pass
...
class ExecutionService:
    ...
    async def submit(self, signal_id: str, symbol: str, direction: str,
                     lots: float = 1.0, **kw) -> Dict[str, Any]:
        # idempotency
        ...
        if self._reconciliation:
            ...
            if dup:
                await self._journal.record(JournalEventType.RISK_BLOCKED, signal_id,
        ...
        try:
            result = await asyncio.wait_for(self._execute(order_id, signal_id, symbol, lots), self._timeout)
            return result
        ...
        if self._broker:
            ...
            if retcode == MT5_RETCODE_DONE:
                ...
                await self._osm.transition(order_id, OrderState.FILLED,
                                           fill_price=result.price, slippage_pips=slippage)
                await self._journal.record(JournalEventType.FILLED, signal_id, order_id,
                                           price=result.price, slippage=slippage)
                ...
                raise OrderSubmissionError(symbol, order_id, retcode=retcode, detail='trade_disabled')
...
class TestFailureRecovery:
    ...
    async def test_retry_then_dead_letter(self):
        ...
        await fr.start()
        await fr.handle_failure(order_id='ord-1', signal_id='sig-1',
                                error='broker down', retcode=10011,
    ...
    async def test_retry_success_clears_queue(self):
        ...
        await fr.start()
        await fr.handle_failure('ord-2', 'sig-2', 'transient', retcode=10016,
