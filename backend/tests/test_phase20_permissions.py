...
from backend.core.permissions import (
    P, Role, ROLE_RANK, ROLE_PERMISSIONS, PLAN_PERMISSIONS,
    PERM_DESCRIPTIONS, ENDPOINT_REGISTRY, EndpointSpec,
    AuthContext, RBACEngineV2, rbac_v2,
    PermissionDeniedError, EscalationError,
    expand_permissions, normalize_role, assert_no_escalation,
)
from backend.core.deps_v3 import (
    require_perm, require_any_perm, require_all_perms,
    require_rank, build_auth_context,
)
from backend.middleware.permission_middleware import (
    PermissionEnforcer, enforcer, _match_endpoint,
)
...
class TestPermissionRegistry:
    ...
    def test_T007_license_perms_complete(self):
        for p in [P.LICENSE_READ_OWN, P.LICENSE_READ_ANY, P.LICENSE_ISSUE, P.LICENSE_REVOKE, P.LICENSE_SUSPEND]:
...
class TestPrivilegeEscalation:
    def test_T033_customer_cannot_escalate_to_support(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.CUSTOMER, Role.SUPPORT)
    def test_T034_customer_cannot_escalate_to_admin(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.CUSTOMER, Role.ADMIN)
    def test_T035_support_cannot_escalate_to_admin(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.SUPPORT, Role.ADMIN)
    def test_T036_admin_cannot_escalate_to_super(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.ADMIN, Role.SUPER)
    ...
    def test_T039_admin_cannot_assign_same_rank(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.ADMIN, Role.ADMIN)
    def test_T040_write_admin_cannot_assign_admin(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.WRITE_ADMIN, Role.ADMIN)
    ...
    def test_T042_readonly_cannot_escalate_to_any(self):
        for role in [Role.CUSTOMER, Role.SUPPORT, Role.WRITE_ADMIN, Role.ADMIN, Role.SUPER]:
            with pytest.raises(EscalationError): assert_no_escalation(Role.READONLY, role)
    ...
    def test_T046_unknown_target_role_raises(self):
        with pytest.raises(EscalationError): assert_no_escalation(Role.ADMIN, "hacker_role")
    def test_T047_rbac_require_no_escalation_raises_on_attempt(self):
        engine = RBACEngineV2()
        with pytest.raises(PermissionDeniedError): engine.require_no_escalation(CTX_CUSTOMER, Role.ADMIN)
...
class TestAuthContextEffectivePerms:
    ...
    def test_T063_tenant_id_custom(self):
        assert AuthContext(user_id="u1", role=Role.CUSTOMER, tenant_id="t_acme").tenant_id == "t_acme"
...
class TestRBACEngineV2:
    def setup_method(self): self.engine = RBACEngineV2()
    ...
    def test_T067_require_raises_on_missing(self):
        with pytest.raises(PermissionDeniedError): self.engine.require(CTX_CUSTOMER, P.LICENSE_ISSUE)
    ...
    def test_T077_permission_matrix_has_all_roles(self):
        ...
        for role in [Role.READONLY, Role.CUSTOMER, Role.SUPPORT, Role.WRITE_ADMIN, Role.ADMIN, Role.SUPER]:
            assert role in m["roles"]
    def test_T078_permission_matrix_correctness(self):
        m = self.engine.permission_matrix()["permissions"]
        assert m[P.TRADE_EXECUTE][Role.CUSTOMER] == True
        assert m[P.TRADE_EXECUTE][Role.READONLY] == False
...
class TestOwnerScopedAccess:
    ...
    def test_T085_require_resource_raises_on_other_owner(self):
        ...
        with pytest.raises(PermissionDeniedError): self.engine.require_resource(c, P.TRADE_READ_OWN, "B")
    ...
    def test_T094_blocked_user_cannot_access_own_resource(self):
        assert not ctx(Role.CUSTOMER, user_id="A", is_blocked=True).can_access_resource(P.TRADE_READ_OWN, "A")
    def test_T095_inactive_user_cannot_access_own_resource(self):
        assert not ctx(Role.CUSTOMER, user_id="A", is_active=False).can_access_resource(P.TRADE_READ_OWN, "A")
...
class TestDepsV3Factories:
    ...
    def test_T098_require_perm_denied(self):
        with pytest.raises(PermissionDeniedError): require_perm(P.LICENSE_ISSUE)(CTX_CUSTOMER)
    ...
    def test_T100_require_any_perm_all_missing(self):
        with pytest.raises(PermissionDeniedError): require_any_perm(P.LICENSE_ISSUE, P.USER_DELETE)(CTX_CUSTOMER)
    ...
    def test_T102_require_all_perms_one_missing(self):
        with pytest.raises(PermissionDeniedError): require_all_perms(P.TRADE_READ_OWN, P.LICENSE_ISSUE)(CTX_CUSTOMER)
    ...
    def test_T104_require_rank_denied(self):
        with pytest.raises(PermissionDeniedError): require_rank(Role.ADMIN)(CTX_SUPPORT)
    ...
    def test_T106_build_auth_context_invalid_token(self):
        with pytest.raises(PermissionDeniedError): build_auth_context("not.a.token")
    def test_T107_require_perm_admin_has_all_critical(self):
        for perm in [P.LICENSE_ISSUE, P.RISK_KILL_SWITCH, P.USER_DELETE, P.AUDIT_EXPORT, P.TENANT_MANAGE]:
            assert require_perm(perm)(CTX_ADMIN) is CTX_ADMIN
    def test_T108_require_perm_blocked_user_denied(self):
        with pytest.raises(PermissionDeniedError): require_perm(P.PROFILE_READ_OWN)(ctx(Role.ADMIN, is_blocked=True))


class TestPermissionMiddleware:
    def setup_method(self): self.e = PermissionEnforcer()
...
class TestEndpointRegistry:
    ...
    def test_T128_license_issue_requires_license_issue(self):
        ep = next(e for e in ENDPOINT_REGISTRY if e.path == "/api/v1/license" and e.method == "POST")
        assert ep.permission == P.LICENSE_ISSUE
    def test_T129_user_delete_requires_user_delete(self):
        ep = next(e for e in ENDPOINT_REGISTRY if "delete" in e.method.lower() and "admin/users" in e.path)
...
class TestPlanPermissions:
    def test_T133_trial_has_basic_read(self): assert P.TRADE_READ_OWN in PLAN_PERMISSIONS["trial"]
    def test_T134_trial_no_trade_execute(self): assert P.TRADE_EXECUTE not in PLAN_PERMISSIONS["trial"]
    def test_T135_basic_has_trade_execute(self): assert P.TRADE_EXECUTE in PLAN_PERMISSIONS["basic"]
    def test_T136_pro_same_as_customer_role(self): assert PLAN_PERMISSIONS["pro"] == PLAN_PERMISSIONS["pro"]
    def test_T137_vip_extends_customer(self): assert P.RISK_READ_ANY in PLAN_PERMISSIONS["vip"]
    def test_T138_vip_has_risk_read_any(self): assert P.RISK_READ_ANY in PLAN_PERMISSIONS["vip"]
    def test_T139_trial_customer_cannot_execute(self):
        assert not AuthContext(user_id="u1", role=Role.CUSTOMER, plan="trial").has_perm(P.TRADE_EXECUTE)
    ...
    def test_T143_trial_has_release_download(self): assert P.RELEASE_DOWNLOAD in PLAN_PERMISSIONS["trial"]
    def test_T144_trial_no_billing_checkout(self): assert P.BILLING_CHECKOUT not in PLAN_PERMISSIONS["trial"]


class TestRoleConveniences:
    def test_T145_normalize_role_user_to_customer(self): assert normalize_role("user") == Role.CUSTOMER
    def test_T146_normalize_role_trader_to_customer(self): assert normalize_role("trader") == Role.CUSTOMER
    def test_T147_normalize_role_superadmin_to_super(self): assert normalize_role("superadmin") == Role.SUPER
    def test_T148_normalize_role_read_only(self): assert normalize_role("read_only") == Role.READONLY
    def test_T149_normalize_role_unknown_passthrough(self): assert normalize_role("custom_role") == "custom_role"
    ...
    def test_T155_support_has_user_list(self): assert P.USER_LIST in expand_permissions(Role.SUPPORT)
    def test_T156_support_has_no_user_block(self): assert P.USER_BLOCK not in expand_permissions(Role.SUPPORT)
...
class TestIntegrationFlows:
    ...
    def test_T161_admin_role_assign_with_escalation_guard(self):
        ...
        with pytest.raises(PermissionDeniedError): e.require_no_escalation(CTX_ADMIN, Role.SUPER)
    ...
    def test_T163_write_admin_can_halt_not_kill_switch(self):
        ...
        with pytest.raises(PermissionDeniedError): e.require(CTX_WRITE_ADMIN, P.RISK_KILL_SWITCH)
    ...
    def test_T165_trial_customer_blocked_from_execute(self):
        ...
        with pytest.raises(PermissionDeniedError): RBACEngineV2().require(c, P.TRADE_EXECUTE)
    def test_T166_middleware_full_flow_admin(self):
        e = PermissionEnforcer()
        for method, path in [("POST","/api/v1/risk/halt"),("POST","/api/v1/risk/kill_switch"),("GET","/api/v1/audit/log")]:
    ...
    def test_T168_permission_matrix_complete(self):
        ...
        assert m[P.LICENSE_ISSUE][Role.ADMIN] == True
        assert m[P.LICENSE_ISSUE][Role.CUSTOMER] == False
        for perm_key in list(m.keys())[:5]:
            assert m[perm_key][Role.SUPER] == True
