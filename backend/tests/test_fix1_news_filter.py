...
from news_filter import NewsEvent, NewsFilterGate, NewsImpact
...
def test_naive_datetime_normalised():
    ev = NewsEvent(
        title="T", currency="USD", impact="HIGH", event_time=datetime(2026, 6, 23, 8, 30, 0)
    )
...
def test_multiple_events_currency_isolation():
    ...
    gate.load_events(
        [
            _event(delta_s=0, currency="JPY", title="JPY"),
            _event(delta_s=0, currency="USD", title="USD"),
        ]
    )
...
def test_volatility_filter_news_delegation():
    ...
    cfg = vf_mod.VolatilityFilterConfig(
        enable_news_filter=True, news_block_minutes_before=30, news_block_minutes_after=15
    )
    vf = vf_mod.VolatilityFilter(cfg)
    ev = vf_mod.NewsEvent(title="CPI", currency="USD", impact="HIGH", event_time=datetime.now(UTC))
...
if __name__ == "__main__":
    tests = [
        test_no_events_not_blocked,
        test_blocks_inside_window,
        test_not_blocked_outside_window,
        test_multiple_events_first_match,
        test_currency_filter_no_match,
        test_currency_all_blocks_any,
        test_failsafe_provider_exception,
        test_load_events_rejects_invalid,
        test_clear_events,
        test_add_event,
        test_upcoming_events,
        test_naive_datetime_normalised,
        test_block_at_exact_start_of_window,
        test_block_at_exact_end_of_window,
        test_not_blocked_before_window,
        test_not_blocked_after_window,
        test_impact_filter_low_no_block,
        test_fomc_always_blocks,
        test_refresh_no_provider,
        test_refresh_provider_timeout,
        test_block_result_fields,
        test_minutes_to_event_positive_future,
        test_minutes_to_event_negative_past,
        test_multiple_events_currency_isolation,
        test_volatility_filter_news_delegation,
    ]
    ok = fail = 0
    for t in tests:
        try:
