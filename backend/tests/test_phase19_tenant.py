...
from backend.middleware.tenant_middleware import (
    TenantMiddleware,
    _extract_tenant_id_from_jwt_sub,
    _is_public,
)
...
class TestAssertTenantAccess:
    ...
    def test_T057_callback_called(self):
        called = []
        assert_tenant_access(
            "t_a", "t_b", "admin", "a1", audit_callbacks=[lambda e: called.append(e)]
        )
        assert len(called) == 1
    ...
    def test_T058_callback_not_called_same_tenant(self):
        called = []
        assert_tenant_access(
            "t_a", "t_a", "customer", "u1", audit_callbacks=[lambda e: called.append(e)]
        )
        assert len(called) == 0
...
class TestTenantScopedStore:
    ...
    def test_T075_concurrent_puts_same_tenant(self):
        ...
        def _put(i):
            try:
                self.store.put(f"k_{i}", f"v_{i}", "t_conc")
            except Exception as e:
                errors.append(e)
...
class TestTenantAuditStore:
    ...
    def test_T132_concurrent_records(self):
        ...
        def _record(i):
            try:
                self.store.record(f"t_{i % 3}", f"u{i}", "customer", "act", "res")
            except Exception as e:
                errors.append(e)
