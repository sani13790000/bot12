...
from backend.core.final_acceptance import (
    CriteriaID, CriteriaStatus, Severity, AcceptanceDecision,
    REQUIRED_CONFIG_KEYS, BLOCKING_CRITERIA, REQUIRES_REASON,
    ADMIN_CAPABILITIES, CUSTOMER_ALLOWED_ROUTES, ADMIN_ONLY_ROUTES,
    ALLOWED_CUSTOMER_EXTENSIONS, AC_DESCRIPTIONS,
    AcceptanceAuditChain, CriteriaResult, AcceptanceReport,
    ProductionConfigGate, MT5Credentials, MT5CredentialsGate,
    TradeAuthContext, TradeAuthGate,
    EAStartupContext, EAFailClosedGate,
    HeartbeatRecord, HeartbeatGate,
    LicenseRevocationGate, DeviceLimitGate,
    SourceAccessGate, CustomerDeliveryGate,
    DashboardSeparationGate, TenantIsolationGate,
    AdminControlGate, DuplicateOrderGate,
    MT5TradeRecord, MT5ReconciliationGate,
    RiskContext, RiskFailClosedGate,
    KillSwitchGate, HardcodedSecretScanner,
    LicenseStorageGate, PaymentWebhookGate,
    TestSuiteResult, TestGate,
    DocAlignmentResult, DocsGate,
    DockerReadinessResult, DockerGate,
    DockerComposeGenerator, AcceptanceAdmin,
    FinalAcceptanceEngine, build_acceptance_system,
)
...
def good_mt5():
    return MT5Credentials(account_id=12345, password="SecretPwd1",
                          server="MetaQuotes-Demo", is_live=True)

def good_trade_ctx():
    return TradeAuthContext(
        license_id="lic-001", license_status="ACTIVE",
        subscription_ok=True, device_id="dev-001",
        device_allowed=True, tenant_id="t1")

def good_ea_ctx():
    return EAStartupContext(
        config_ok=True, license_ok=True,
        credentials_ok=True, heartbeat_ok=True, risk_ok=True)

def good_risk():
    return RiskContext(
        drawdown_pct=5.0, open_positions=3,
        margin_level=500.0, kill_switch_on=False, daily_loss_pct=1.0)

def make_trade(ticket, symbol="EURUSD", direction="BUY", volume=0.1, tenant="t1"):
    return MT5TradeRecord(ticket=ticket, symbol=symbol, direction=direction,
                          volume=volume, open_price=1.1000, tenant_id=tenant)

def make_system():
    return build_acceptance_system(secret="test-secret-36")

def full_ctx(sys_, device_id="dev-001"):
    hb = HeartbeatRecord(device_id=device_id, tenant_id="t1",
                         received_at=time.time(), ea_version="2.3.0",
                         symbol="EURUSD", is_live=True)
    sys_["hb_gate"].record_heartbeat(hb)
    sys_["device_gate"].set_limit("lic-001", 3)
    tr = TestSuiteResult(total=4435, passed=4435, failed=0,
                         phases={f"P{i}": 200 for i in range(6, 36)})
    doc = DocAlignmentResult(total_docs=10, aligned=10, mismatched=[], missing_docs=[])
    docker = DockerReadinessResult(
        staging_ready=True, prod_ready=True, has_dockerfile=True,
        has_compose=True, has_health_check=True, has_env_template=True, has_migrations=True)
    raw_key = "RAW-LICENSE-KEY-123"
    stored  = hashlib.sha256(raw_key.encode()).hexdigest()
    return {
        "tenant_id": "t1", "env": good_env(), "trading_enabled": True,
        "mt5_creds": good_mt5(), "trade_ctx": good_trade_ctx(),
        "ea_ctx": good_ea_ctx(), "device_id": device_id,
        ...
        "risk_ctx": good_risk(),
        "reconciliation": {"our": [make_trade(1), make_trade(2)],
        ...
        "test_result": tr, "doc_alignment": doc, "docker": docker,
...
class TestAcceptanceAuditChain:
    ...
    def test_T018_single_entry_valid(self):
        a = make_audit(); a.record("TEST", "actor")
    ...
    def test_T027_query_by_action(self):
        a = make_audit()
        a.record("A", "x"); a.record("B", "x"); a.record("A", "x")
    ...
    def test_T029_seq_increments(self):
        a = make_audit()
        e1 = a.record("A", "x"); e2 = a.record("B", "x")
    ...
    def test_T033_verify_after_concurrent(self):
        import threading
        a = make_audit()
        for _ in range(10): a.record("T", "a")
        results = []
        def chk(): results.append(a.verify_chain())
...
class TestAC01ProductionConfig:
    ...
    def test_T038_missing_jwt_fails(self):
        env = good_env(); del env["JWT_SECRET"]
    ...
    def test_T039_empty_value_fails(self):
        env = good_env(); env["SUPABASE_URL"] = "   "
        r = ProductionConfigGate(make_audit()).check(env, allow_test_stripe=True)
        assert r.status == CriteriaStatus.FAIL
    def test_T040_change_me_fails(self):
        env = good_env(); env["ENCRYPTION_KEY"] = "CHANGE_ME"
        r = ProductionConfigGate(make_audit()).check(env, allow_test_stripe=True)
        assert r.status == CriteriaStatus.FAIL
    def test_T041_todo_fails(self):
        env = good_env(); env["AUDIT_CHAIN_SECRET"] = "TODO"
        r = ProductionConfigGate(make_audit()).check(env, allow_test_stripe=True)
        assert r.status == CriteriaStatus.FAIL
    def test_T042_sk_test_blocked_in_prod(self):
        env = good_env(); env["STRIPE_SECRET_KEY"] = "sk_test_abc123xyz"
    ...
    def test_T046_dummy_fails(self):
        env = good_env(); env["WEBHOOK_HMAC_SECRET"] = "dummy"
        r = ProductionConfigGate(make_audit()).check(env, allow_test_stripe=True)
        assert r.status == CriteriaStatus.FAIL
...
class TestAC02MT5Credentials:
    def test_T049_valid_passes(self):
        assert MT5CredentialsGate(make_audit()).check(good_mt5(), True).status == CriteriaStatus.PASS
    ...
    def test_T057_ac02(self):
        assert MT5CredentialsGate(make_audit()).check(good_mt5(), True).criteria_id == CriteriaID.AC02
...
class TestAC03TradeAuth:
    ...
    def test_T059_revoked(self):
        ctx = good_trade_ctx(); ctx.license_status = "REVOKED"
        assert TradeAuthGate(make_audit()).check(ctx).status == CriteriaStatus.FAIL
    def test_T060_suspended(self):
        ctx = good_trade_ctx(); ctx.license_status = "SUSPENDED"
        assert TradeAuthGate(make_audit()).check(ctx).status == CriteriaStatus.FAIL
    def test_T061_expired(self):
        ctx = good_trade_ctx(); ctx.license_status = "EXPIRED"
        assert TradeAuthGate(make_audit()).check(ctx).status == CriteriaStatus.FAIL
    def test_T062_bad_subscription(self):
        ctx = good_trade_ctx(); ctx.subscription_ok = False
    ...
    def test_T063_device_not_allowed(self):
        ctx = good_trade_ctx(); ctx.device_allowed = False
        assert TradeAuthGate(make_audit()).check(ctx).status == CriteriaStatus.FAIL
    def test_T064_all_three_fail(self):
        ctx = good_trade_ctx()
        ctx.license_status = "REVOKED"; ctx.subscription_ok = False; ctx.device_allowed = False
        assert TradeAuthGate(make_audit()).check(ctx).status == CriteriaStatus.FAIL
...
class TestAC04EAFailClosed:
    ...
    def test_T067_config_fail(self):
        ctx = good_ea_ctx(); ctx.config_ok = False
    ...
    def test_T071_any_single_fail(self):
        ...
        for attr in ("config_ok","license_ok","credentials_ok","heartbeat_ok","risk_ok"):
            ctx = good_ea_ctx(); setattr(ctx, attr, False)
...
class TestAC05Heartbeat:
    ...
    def test_T075_multi_device(self):
        ...
        for i in range(5): assert gate.check(f"d{i}").status == CriteriaStatus.PASS
...
class TestAC06LicenseRevoke:
    def test_T077_fast_propagation(self):
        assert LicenseRevocationGate(make_audit()).check_propagation("l", time.time()).status == CriteriaStatus.PASS
    def test_T078_stale_fails(self):
        assert LicenseRevocationGate(make_audit()).check_propagation("l", time.time()-100).status == CriteriaStatus.FAIL
    def test_T079_revoke_needs_reason(self):
        with pytest.raises(ValueError): LicenseRevocationGate(make_audit()).revoke("l", "")
    ...
    def test_T082_revoke_audited(self):
        audit = make_audit(); gate = LicenseRevocationGate(audit)
        gate.revoke("l", "test")
...
class TestAC08SourceAccess:
    def test_T088_ex5_ok(self):
        assert SourceAccessGate(make_audit()).check_delivery(["b.ex5","g.pdf"],"CUSTOMER").status == CriteriaStatus.PASS
    ...
    def test_T090_mq5_fails(self):
        assert SourceAccessGate(make_audit()).check_delivery(["s.mq5"],"CUSTOMER").status == CriteriaStatus.FAIL
    def test_T091_backend_path_fails(self):
        assert SourceAccessGate(make_audit()).check_delivery(["backend/core/x.py"],"CUSTOMER").status == CriteriaStatus.FAIL
    def test_T092_admin_ok(self):
        assert SourceAccessGate(make_audit()).check_delivery(["main.py"],"ADMIN").status == CriteriaStatus.PASS


class TestAC09CustomerDelivery:
    def test_T093_ex5_pdf_ok(self):
        assert CustomerDeliveryGate(make_audit()).check(["b.ex5","g.pdf"]).status == CriteriaStatus.PASS
...
class TestAC11TenantIsolation:
    def test_T103_same_ok(self):
        assert TenantIsolationGate(make_audit()).check_query("t1","t1","o").status == CriteriaStatus.PASS
    ...
    def test_T105_cross_critical(self):
        assert TenantIsolationGate(make_audit()).check_query("t1","t2","l").severity == Severity.CRITICAL
    def test_T106_audited(self):
        audit = make_audit(); TenantIsolationGate(audit).check_query("t1","t2","o")
...
class TestAC12AdminControl:
    ...
    def test_T108_missing_cap_fails(self):
        gate = AdminControlGate(make_audit()); gate._registered.discard("kill_switch")
        r = gate.check()
        assert r.status == CriteriaStatus.FAIL and "kill_switch" in r.evidence
    def test_T109_register_extra(self):
        gate = AdminControlGate(make_audit()); gate.register_capability("x")
...
class TestAC16KillSwitch:
    ...
    def test_T127_deactivate(self):
        gate = KillSwitchGate(make_audit())
        gate.activate("t1","e","a1"); gate.deactivate("t1","a1",reason="r")
        assert not gate.is_active("t1")
    def test_T128_activate_needs_reason(self):
        with pytest.raises(ValueError): KillSwitchGate(make_audit()).activate("t","","a")
    def test_T129_deactivate_needs_reason(self):
        gate = KillSwitchGate(make_audit()); gate.activate("t","r","a")
        with pytest.raises(ValueError): gate.deactivate("t","a",reason="")
    ...
    def test_T131_isolation(self):
        gate = KillSwitchGate(make_audit()); gate.activate("t1","r","a")
        assert gate.is_active("t1") and not gate.is_active("t2")
    def test_T132_audited(self):
        audit = make_audit(); gate = KillSwitchGate(audit)
...
class TestAC17HardcodedSecrets:
    ...
    def test_T134_password_fails(self):
        assert HardcodedSecretScanner(make_audit()).check({"c.py":'password = "mysecret123"'}).status == CriteriaStatus.FAIL
    def test_T135_stripe_live_fails(self):
        assert HardcodedSecretScanner(make_audit()).check({"p.py":"STRIPE='sk_live_abc123xyz'"}).status == CriteriaStatus.FAIL
    def test_T136_rsa_key_fails(self):
        assert HardcodedSecretScanner(make_audit()).check({"k.py":"-----BEGIN RSA PRIVATE KEY-----"}).status == CriteriaStatus.FAIL
...
class TestAC18LicenseStorage:
    def test_T139_hashed_ok(self):
        ...
        raw = "RAW-KEY-XYZ"; stored = gate.hash_license(raw)
...
class TestAC19PaymentWebhook:
    def _g(self): return PaymentWebhookGate(make_audit(), secret="wh-secret")
    def test_T144_valid_processes(self):
        g = self._g(); p = b'{"type":"payment"}'
    ...
    def test_T149_many_idempotent(self):
        g = self._g(); p = b"p"; sig = g.generate_signature(p)
        for _ in range(5): g.process("e100", p, sig)
...
class TestAC20_22Gates:
    ...
    def test_T151_failed_tests_fail(self):
        assert TestGate(make_audit()).check(TestSuiteResult(100,95,5,{})).status == CriteriaStatus.FAIL
    ...
    def test_T154_docker_ok(self):
        r = DockerGate(make_audit()).check(DockerReadinessResult(True,True,True,True,True,True,True))
        assert r.status == CriteriaStatus.PASS
    def test_T155_no_dockerfile_fails(self):
        r = DockerGate(make_audit()).check(DockerReadinessResult(True,True,False,True,True,True,True))
        assert r.status == CriteriaStatus.FAIL
    def test_T156_prod_not_ready_fails(self):
        r = DockerGate(make_audit()).check(DockerReadinessResult(True,False,True,True,True,True,False))
        assert r.status == CriteriaStatus.FAIL
...
class TestFinalAcceptanceEngine:
    def test_T165_go(self):
        sys_ = make_system(); ctx = full_ctx(sys_)
    ...
    def test_T167_no_go_no_config(self):
        sys_ = make_system(); ctx = full_ctx(sys_); ctx["env"] = {}
...
class TestAcceptanceAdmin:
    def test_T179_store_retrieve(self):
        sys_ = make_system(); ctx = full_ctx(sys_)
        r = sys_["engine"].run(ctx); sys_["admin"].store_report(r)
        assert sys_["admin"].latest_report() is r
    def test_T180_summary(self):
        sys_ = make_system(); ctx = full_ctx(sys_)
        for _ in range(3): sys_["admin"].store_report(sys_["engine"].run(ctx))
...
class TestBuildAcceptanceSystem:
    ...
    def test_T185_isolated_secrets(self):
        s1 = build_acceptance_system("A"); s2 = build_acceptance_system("B")
        assert s1["audit"]._genesis != s2["audit"]._genesis
    def test_T186_independent(self):
        s1 = make_system(); s2 = make_system()
...
class TestSQLMigration:
    ...
    def sql(self):
        ...
        paths = [
            os.path.join(base,"supabase","migrations","20260629_045_phase36_final_acceptance.sql"),
    ...
    def test_T189_acceptance_runs(self, sql): assert "acceptance_runs" in sql
    def test_T190_criteria_results(self, sql): assert "criteria_results" in sql
    def test_T191_audit_log(self, sql): assert "acceptance_audit_log" in sql
    def test_T192_rls(self, sql): assert "ROW LEVEL SECURITY" in sql.upper() or "ENABLE ROW LEVEL" in sql
    def test_T193_trigger(self, sql): assert "TRIGGER" in sql.upper()
    def test_T194_chain_hash(self, sql): assert "CHAR(64)" in sql or "chain_hash" in sql
    def test_T195_indexes(self, sql): assert "INDEX" in sql.upper()
    def test_T196_views(self, sql): assert "VIEW" in sql.upper()


class TestIntegrationFlows:
    def test_T197_trading_disabled(self):
        sys_ = make_system(); ctx = full_ctx(sys_)
        ctx["trading_enabled"] = False; ctx["mt5_creds"] = None
    ...
    def test_T199_webhook_replay(self):
        g = make_system()["webhook_gate"]; p = b"p"; sig = g.generate_signature(p)
        g.process("e1",p,sig)
...
class TestEdgeCasesAndFinalAcceptance:
    def test_T206_to_dict(self):
        r = CriteriaResult(CriteriaID.AC01,"t",CriteriaStatus.PASS,"ok",Severity.CRITICAL,True,"P")
        assert r.to_dict()["criteria_id"] == "AC01"
    def test_T207_blocking_fail_logic(self):
        r1 = CriteriaResult(CriteriaID.AC01,"",CriteriaStatus.FAIL,"",Severity.CRITICAL,True,"")
    ...
    def test_T208_no_audit_no_crash(self):
        gate = KillSwitchGate(None); gate.activate("t","r","a")
        assert gate.is_active("t")
    def test_T209_ghost_trade(self):
        rec = MT5ReconciliationGate(make_audit()).reconcile([make_trade(1)],[make_trade(1),make_trade(999)])
    ...
    def test_T211_env_template_keys(self):
        ...
        for k in ["JWT_SECRET","DATABASE_URL","STRIPE_SECRET_KEY"]: assert k in t
    def test_T212_dedup_per_tenant(self):
        gate = DuplicateOrderGate(make_audit())
        gate.check_order("E","B",0.1,"A"); gate.check_order("E","B",0.1,"B")
    ...
    def test_T216_ac23_pass_iff_go(self):
        ...
        if report.decision == AcceptanceDecision.GO: assert ac23.status == CriteriaStatus.PASS
        else: assert ac23.status == CriteriaStatus.FAIL
    def test_T217_audit_grows(self):
        sys_ = make_system(); ctx = full_ctx(sys_)
        n0 = len(sys_["audit"]); sys_["engine"].run(ctx)
        assert len(sys_["audit"]) > n0
    def test_T218_concurrent_safe(self):
        import threading
        sys_ = make_system(); ctx = full_ctx(sys_)
        results = []
        def run(): c = deepcopy(ctx); results.append(sys_["engine"].run(c).decision)
    ...
    def test_T222_no_trade_revoked_or_overlimit(self):
        tg = TradeAuthGate(make_audit()); ctx = good_trade_ctx(); ctx.license_status = "REVOKED"
        assert tg.check(ctx).status == CriteriaStatus.FAIL
        dg = DeviceLimitGate(make_audit()); dg.set_limit("l",1)
        dg.check("l","d1")
