...
#     FIX: changed to `times[-1] if times else datetime.utcnow()`
# ─────────────────────────────────────────────────────────────────────────────────
import asyncio, math, sys, os, time, pytest
...
class TestFlashCrash:
    ...
    def test_win_rate_zero_no_crash(self):
        from backend.risk.lot_sizing import LotSizer
        result = run(LotSizer().calculate(balance=10000, stop_loss_pips=20, symbol='EURUSD', win_rate=0.0))
...
class TestHighVolatility:
    def test_blocks_extreme_atr(self):
        from backend.risk.volatility_filter import VolatilityFilter, VolatilityConfig
...
class TestAPIFailure:
    def test_mt5_not_ready_returns_failure(self):
        ...
        conn._ready = False; conn._mt5 = None; conn._lock = asyncio.Lock()
        conn._timeout = 10000; conn._sim_mode = False
        req = MT5OrderRequest(symbol='EURUSD', direction='BUY', lot_size=0.01,
                               sl=1.08, tp=1.09, order_type='MARKET')
...
class TestNetworkFailure:
    def test_circuit_breaker_recovery(self):
        from backend.circuit_breaker import CircuitBreaker, BreakerConfig
        cb = CircuitBreaker('net', BreakerConfig(failure_threshold=2,
                            recovery_timeout_s=0.05, success_threshold=1))
        for _ in range(2):
            try: run(_fail_cb(cb))
            except Exception: pass
        assert cb._stats.state.value == 'open'
...
class TestMissingCandle:
    ...
    def test_single_candle_no_crash(self):
        from backend.analysis.smc_engine import SMCEngine
        result = SMCEngine().analyze('EURUSD', {'opens':[1.0],'highs':[1.1],'lows':[0.9],'closes':[1.05],'times':[]})
        assert result is not None

    def test_49_candles_returns_empty_result(self):
        from backend.analysis.smc_engine import SMCEngine
        n = 49
        data = {'opens':[1.0]*n,'highs':[1.01]*n,'lows':[0.99]*n,'closes':[1.005]*n,'times':[]}
    ...
    def test_nan_in_candles_no_crash(self):
        ...
        closes[30] = float('nan')
        data = {'opens':[1.0]*n,'highs':[1.01]*n,'lows':[0.99]*n,'closes':closes,'times':[]}
        try:
            assert SMCEngine().analyze('EURUSD', data) is not None
        except Exception as e:
    ...
    def test_all_same_price_no_crash(self):
        ...
        from backend.analysis.smc_engine import SMCEngine
        n = 60
        data = {'opens':[1.0]*n,'highs':[1.0]*n,'lows':[1.0]*n,'closes':[1.0]*n,'times':[]}
        try:
            assert SMCEngine().analyze('EURUSD', data) is not None
        except Exception as e:
...
class TestDuplicateTick:
    async def test_dedup_second_is_duplicate(self):
        ...
        es_mod._IDEMPLOTENCY_STORE.clear(); es_mod._IDEMPLOTENCY_TIMESTAMPS.clear(); es_mod._INFLIGHT_SIGNALS.clear()
        mock_risk = MagicMock(); mock_risk.assess = AsyncMock(return_value=MagicMock(allowed=True, lot_size=0.01))
        mock_broker = AsyncMock(); mock_broker.send_order = AsyncMock(return_value=True)
        svc = ExecutionService.__new__(ExecutionService)
        svc._risk = mock_risk; svc._broker = mock_broker
        svc._lock = asyncio.Lock(); svc._idempotency_lock = asyncio.Lock()
        signal = {'signal_id':'dedup-001','symbol':'EURUSD','direction':'BUY','stop_loss_pips':20,'account_balance':10000}
    ...
    async def test_inflight_cleaned_on_success(self):
        ...
        svc._risk = mock_risk; svc._broker = mock_broker
        svc._lock = asyncio.Lock(); svc._idempotency_lock = asyncio.Lock()
        signal = {'signal_id':'inf-001','symbol':'EURUSD','direction':'BUY','stop_loss_pips':20,'account_balance':10000}
    ...
    async def test_inflight_cleaned_on_exception(self):
        ...
        from backend.execution.execution_service import ExecutionService
        es_mod._IDEMPLOTENCY_STORE.clear(); es_mod._IDEMPLOTENCY_TIMESTAMPS.clear(); es_mod._INFLIGHT_SIGNALS.clear()
        mock_risk = MagicMock(); mock_risk.assess = AsyncMock(side_effect=RuntimeError('DB down'))
        svc = ExecutionService.__new__(ExecutionService)
        svc._risk = mock_risk; svc._broker = AsyncMock()
        svc._lock = asyncio.Lock(); svc._idempotency_lock = asyncio.Lock()
        signal = {'signal_id':'exc-001','symbol':'EURUSD','direction':'BUY','stop_loss_pips':20,'account_balance':10000}
    ...
    async def test_concurrent_only_one_submitted(self):
        ...
        from backend.execution.execution_service import ExecutionService
        es_mod._IDEMPLOTENCY_STORE.clear(); es_mod._IDEMPLOTENCY_TIMESTAMPS.clear(); es_mod._INFLIGHT_SIGNALS.clear()
        async def slow_send(*a, **kw): await asyncio.sleep(0.02); return True
        mock_risk = MagicMock(); mock_risk.assess = AsyncMock(return_value=MagicMock(allowed=True, lot_size=0.01))
        mock_broker = AsyncMock(); mock_broker.send_order = slow_send
        svc = ExecutionService.__new__(ExecutionService)
        svc._risk = mock_risk; svc._broker = mock_broker
        svc._lock = asyncio.Lock(); svc._idempotency_lock = asyncio.Lock()
        signal = {'signal_id':'conc-001','symbol':'EURUSD','direction':'BUY','stop_loss_pips':20,'account_balance':10000}
        results = await asyncio.gather(*[svc.execute_signal(dict(signal)) for _ in range(10)], return_exceptions=True)
        submitted = [r for r in results if isinstance(r,dict) and r.get('status')=='submitted']
        blocked = [r for r in results if isinstance(r,dict) and r.get('status') in ('duplicate','in_flight')]
