...
class TestPlanAndRingGates:
    ...
    def test_T028_ring_internal_sees_flag(self, evaluator):
        cfg = FlagConfig(
            key=FlagKey.EA_CLOUD_CONFIG,
            enabled=True,
            strategy=RolloutStrategy.RING,
            min_ring=ReleaseRing.INTERNAL,
        )
    ...
    def test_T030_ring_beta_sees_beta_flag(self, evaluator):
        cfg = FlagConfig(
            key=FlagKey.DASHBOARD_AI_INSIGHTS,
            enabled=True,
            strategy=RolloutStrategy.RING,
            min_ring=ReleaseRing.BETA,
        )
...
class TestKillOverride:
    ...
    def test_T039_tenant_scoped_kill(self, store):
        ...
        assert store.evaluate(
            FlagKey.RISK_KILL_SWITCH_V2, EvalContext(user_id="u1", tenant_id="t_good")
        ).enabled
        assert not store.evaluate(
            FlagKey.RISK_KILL_SWITCH_V2, EvalContext(user_id="u2", tenant_id="t_bad")
        ).enabled
    ...
    def test_T040_global_kill_blocks_all(self, store):
        ...
        for tid in ["t1", "t2", "t3"]:
            assert not store.evaluate(
                FlagKey.RISK_KILL_SWITCH_V2, EvalContext(user_id="u1", tenant_id=tid)
            ).enabled
    ...
    def test_T042_kill_audited(self, store):
        ...
        assert any(
            r.flag_key == FlagKey.BILLING_STRIPE_V2.value for r in store.audit.query(action="kill")
        )
    ...
    def test_T043_kill_reset_audited(self, store):
        ...
        assert any(
            r.flag_key == FlagKey.BILLING_STRIPE_V2.value
            for r in store.audit.query(action="kill_reset")
        )
    ...
    def test_T047_ko_not_expired_no_ttl(self):
        ko = KillOverride(
            flag_key=FlagKey.RISK_KILL_SWITCH_V2, reason="r", actor_id="a", tenant_id=None
        )
        assert not ko.is_expired()

    def test_T048_ko_expired_past_ttl(self):
        ko = KillOverride(
            flag_key=FlagKey.RISK_KILL_SWITCH_V2,
            reason="r",
            actor_id="a",
            tenant_id=None,
...
class TestFlagAuditChain:
    ...
    def test_T055_tamper_detected(self, chain):
        ...
        chain._records.clear()
        chain._records.extend(recs)
    ...
    def test_T059_total_count(self, chain):
        for i in range(5):
            chain.record("k", "create", f"u{i}", f"r{i}", {})
    ...
    def test_T063_query_most_recent_first(self, chain):
        for i in range(5):
            chain.record("k", "create", "admin", f"r{i}", {"i": i})
...
class TestAuditedFlagStore:
    ...
    def test_T068_update_action(self, store, basic_flag):
        ...
        assert (
            len(store.audit.query(flag_key=FlagKey.RISK_KILL_SWITCH_V2.value, action="update")) >= 1
        )
    ...
    def test_T080_concurrent_eval(self, store):
        ...
        def worker():
            try:
                store.evaluate(
                    FlagKey.RISK_KILL_SWITCH_V2, EvalContext(user_id="u1", tenant_id="t1")
                )
            except Exception as e:
                errors.append(e)
...
class TestRolloutStrategies:
    def test_T081_percentage_0_blocks_all(self, store):
        ...
        hits = sum(
            ...
            for i in range(100)
            if store.is_enabled(
                FlagKey.DASHBOARD_REALTIME_PNL, EvalContext(user_id=f"u{i}", tenant_id="t1")
            )
        )
        assert hits == 0
    ...
    def test_T087_allowlist_user(self, store):
        cfg = FlagConfig(
            key=FlagKey.EA_TELEMETRY_V2,
            enabled=True,
            strategy=RolloutStrategy.ALLOWLIST,
            allowlist_users={"u_vip"},
        )
        store.set_flag(cfg, "admin", "allowlist")
        assert store.is_enabled(
            FlagKey.EA_TELEMETRY_V2, EvalContext(user_id="u_vip", tenant_id="t1")
        )
        assert not store.is_enabled(
            FlagKey.EA_TELEMETRY_V2, EvalContext(user_id="u_other", tenant_id="t1")
        )
    ...
    def test_T088_allowlist_tenant(self, store):
        cfg = FlagConfig(
            ...
            enabled=True,
            strategy=RolloutStrategy.ALLOWLIST,
            allowlist_tenants={"t_beta"},
        )
        store.set_flag(cfg, "admin", "tenant allowlist")
        assert store.is_enabled(
            FlagKey.EA_TELEMETRY_V2, EvalContext(user_id="any", tenant_id="t_beta")
        )
        assert not store.is_enabled(
            FlagKey.EA_TELEMETRY_V2, EvalContext(user_id="any", tenant_id="t_other")
        )
    ...
    def test_T089_blocklist_user(self, store):
        ...
        assert not store.is_enabled(
            FlagKey.RISK_DYNAMIC_DRAWDOWN, EvalContext(user_id="u_bad", tenant_id="t1")
        )
        assert store.is_enabled(
            FlagKey.RISK_DYNAMIC_DRAWDOWN, EvalContext(user_id="u_good", tenant_id="t1")
        )

    def test_T090_blocklist_tenant(self, store):
        cfg = FlagConfig(
            key=FlagKey.RISK_DYNAMIC_DRAWDOWN, enabled=True, blocklist_tenants={"t_suspended"}
        )
        store.set_flag(cfg, "admin", "tenant block")
        assert not store.is_enabled(
            FlagKey.RISK_DYNAMIC_DRAWDOWN, EvalContext(user_id="u1", tenant_id="t_suspended")
        )
        assert store.is_enabled(
            FlagKey.RISK_DYNAMIC_DRAWDOWN, EvalContext(user_id="u1", tenant_id="t_good")
        )

    def test_T091_blocklist_priority_over_allowlist(self, store):
        cfg = FlagConfig(
            key=FlagKey.LICENSE_GRACE_PERIOD,
            enabled=True,
            blocklist_users={"u1"},
            allowlist_users={"u1"},
        )
        ...
        assert not store.evaluate(
            FlagKey.LICENSE_GRACE_PERIOD, EvalContext(user_id="u1", tenant_id="t1")
        ).enabled
    ...
    def test_T093_eval_result_has_flag_key(self, store, ctx):
        store.set_flag(FlagConfig(key=FlagKey.RISK_KILL_SWITCH_V2, enabled=True), "admin", "init")
        assert (
            store.evaluate(FlagKey.RISK_KILL_SWITCH_V2, ctx).flag_key
            == FlagKey.RISK_KILL_SWITCH_V2.value
        )
    ...
    def test_T096_tenant_allowlist_scope(self, store):
        ...
        assert (
            store.evaluate(
                FlagKey.RISK_KILL_SWITCH_V2, EvalContext(user_id="u_other", tenant_id="t1")
            ).scope
            == FlagScope.TENANT
        )
...
class TestGradualRolloutManager:
    ...
    def test_T098_start_creates_flag(self, rollout, store):
        ...
        assert (
            cfg
            and cfg.enabled
            and cfg.strategy == RolloutStrategy.PERCENTAGE
            and cfg.rollout_pct == 1.0
        )
    ...
    def test_T108_step_up_requires_reason(self, rollout):
        rollout.start_rollout(FlagKey.RISK_KILL_SWITCH_V2, "admin", "start")
        with pytest.raises(ValueError):
            rollout.step_up(FlagKey.RISK_KILL_SWITCH_V2, "admin", "")

    def test_T109_step_down_requires_reason(self, rollout):
        rollout.start_rollout(FlagKey.RISK_KILL_SWITCH_V2, "admin", "start")
        with pytest.raises(ValueError):
            rollout.step_down(FlagKey.RISK_KILL_SWITCH_V2, "admin", "")

    def test_T110_pause_requires_reason(self, rollout):
        with pytest.raises(ValueError):
            rollout.pause(FlagKey.RISK_KILL_SWITCH_V2, "admin", "")

    def test_T111_resume_requires_reason(self, rollout):
        with pytest.raises(ValueError):
            rollout.resume(FlagKey.RISK_KILL_SWITCH_V2, "admin", "")
...
class TestAuditIntegrity:
    def test_T113_chain_valid_many_ops(self, store):
        for i in range(20):
            store.set_flag(
                FlagConfig(key=FlagKey.RISK_KILL_SWITCH_V2, enabled=(i % 2 == 0)), f"u{i}", f"r{i}"
            )
    ...
    def test_T115_audit_has_tenant_id(self, store):
        store.set_flag(
            FlagConfig(key=FlagKey.RISK_KILL_SWITCH_V2, enabled=True),
            "admin",
            "op",
            tenant_id="t_acme",
        )
        assert (
            store.audit.query(flag_key=FlagKey.RISK_KILL_SWITCH_V2.value)[0].tenant_id == "t_acme"
        )
    ...
    def test_T116_audit_has_payload(self, store):
        ...
        assert (
            store.audit.query(flag_key=FlagKey.RISK_KILL_SWITCH_V2.value)[0].payload.get(
                "rollout_pct"
            )
            == 42.0
        )
    ...
    def test_T121_remove_action_audited(self, store):
        ...
        store.remove_flag(FlagKey.RISK_KILL_SWITCH_V2, "admin", "cleanup")
        assert any(
            r.flag_key == FlagKey.RISK_KILL_SWITCH_V2.value
            for r in store.audit.query(action="remove")
        )
    ...
    def test_T123_concurrent_chain_valid(self, store):
        def worker(i):
            store.set_flag(
                FlagConfig(key=FlagKey.RISK_KILL_SWITCH_V2, enabled=True), f"u{i}", f"op {i}"
            )
    ...
    def test_T124_record_fields_complete(self, store):
        store.set_flag(
            FlagConfig(key=FlagKey.RISK_KILL_SWITCH_V2, enabled=True), "admin", "full check"
        )
        ...
        assert (
            r.reason == "full check"
            and r.ts > 0
            and len(r.chain_hash) == 64
            and len(r.prev_hash) == 64
        )
...
class TestSQLMigration:
    def test_T129_sql_exists(self):
        assert len(MIGRATION_SQL) > 100

    def test_T130_begin_commit(self):
        assert "BEGIN;" in MIGRATION_SQL and "COMMIT;" in MIGRATION_SQL

    def test_T131_feature_flags_table(self):
        assert "CREATE TABLE IF NOT EXISTS feature_flags" in MIGRATION_SQL

    def test_T132_kill_overrides_table(self):
        assert "CREATE TABLE IF NOT EXISTS flag_kill_overrides" in MIGRATION_SQL

    def test_T133_audit_log_table(self):
        assert "CREATE TABLE IF NOT EXISTS flag_audit_log" in MIGRATION_SQL

    def test_T134_rollout_history_table(self):
        assert "CREATE TABLE IF NOT EXISTS flag_rollout_history" in MIGRATION_SQL

    def test_T135_immutable_trigger(self):
        assert "prevent_flag_audit_mutation" in MIGRATION_SQL

    def test_T136_rls_enabled(self):
        assert "ENABLE ROW LEVEL SECURITY" in MIGRATION_SQL

    def test_T137_rls_policies(self):
        assert all(
            p in MIGRATION_SQL
            for p in ["flag_tenant_isolation", "kill_tenant_isolation", "audit_tenant_isolation"]
        )

    def test_T138_indexes(self):
        assert "idx_feature_flags_key" in MIGRATION_SQL and "idx_flag_audit_seq" in MIGRATION_SQL

    def test_T139_cleanup_fn(self):
        assert "cleanup_expired_kills" in MIGRATION_SQL

    def test_T140_active_kills_view(self):
        assert "vw_active_flag_kills" in MIGRATION_SQL

    def test_T141_tenant_id_in_tables(self):
        assert MIGRATION_SQL.count("tenant_id") >= 4

    def test_T142_rollout_pct_constraint(self):
        assert "rollout_pct BETWEEN 0 AND 100" in MIGRATION_SQL

    def test_T143_reason_not_empty(self):
        assert "reason_not_empty" in MIGRATION_SQL

    def test_T144_chain_hash_length(self):
        assert "char_length(chain_hash) = 64" in MIGRATION_SQL
...
class TestScopeAndPrecedence:
    ...
    def test_T147_plan_gate_before_pct(self, store):
        ...
        store.set_flag(cfg, "admin", "init")
        result = store.evaluate(
            FlagKey.BILLING_CRYPTO_PAY,
            EvalContext(user_id="u1", tenant_id="t1", plan=PlanTier.TRIAL),
        )
        assert not result.enabled and "plan_gate" in result.reason
    ...
    def test_T153_ring_scope(self, store):
        ...
        assert (
            store.evaluate(
                FlagKey.EA_CLOUD_CONFIG,
                EvalContext(user_id="u1", tenant_id="t1", ring=ReleaseRing.BETA),
            ).scope
            == FlagScope.RING
        )
    ...
    def test_T154_plan_scope(self, store):
        ...
        assert (
            store.evaluate(
                FlagKey.BILLING_CRYPTO_PAY,
                EvalContext(user_id="u1", tenant_id="t1", plan=PlanTier.BASIC),
            ).scope
            == FlagScope.PLAN
        )
    ...
    def test_T155_pct_scope_global(self, store):
        ...
        assert (
            store.evaluate(
                FlagKey.DASHBOARD_REALTIME_PNL, EvalContext(user_id="u1", tenant_id="t1")
            ).scope
            == FlagScope.GLOBAL
        )
    ...
    def test_T156_tenant_scope(self, store):
        ...
        assert (
            store.evaluate(
                FlagKey.RISK_KILL_SWITCH_V2, EvalContext(user_id="u_new", tenant_id="t1")
            ).scope
            == FlagScope.TENANT
        )
    ...
    def test_T158_kill_blocks_admin_plan(self, store):
        ...
        assert not store.evaluate(
            FlagKey.RISK_KILL_SWITCH_V2,
            EvalContext(
                user_id="root", tenant_id="t1", plan=PlanTier.ADMIN, ring=ReleaseRing.INTERNAL
            ),
        ).enabled
...
class TestIntegrationFlows:
    ...
    def test_T162_canary_to_ga_flow(self, store):
        ...
        cfg = FlagConfig(
            key=key, enabled=True, strategy=RolloutStrategy.RING, min_ring=ReleaseRing.INTERNAL
        )
        store.set_flag(cfg, "admin", "canary")
        assert store.is_enabled(
            key, EvalContext(user_id="u1", tenant_id="t1", ring=ReleaseRing.INTERNAL)
        )
        assert not store.is_enabled(
            key, EvalContext(user_id="u2", tenant_id="t2", ring=ReleaseRing.GA)
        )
    ...
    def test_T164_chain_survives_200_ops(self, store):
        for i in range(100):
            store.set_flag(
                FlagConfig(key=FlagKey.RISK_KILL_SWITCH_V2, enabled=(i % 2 == 0)),
                f"u{i}",
                f"op {i}",
            )
        for i in range(50):
    ...
    def test_T165_concurrent_rollout_safe(self, store, rollout):
        ...
        def worker():
            try:
                rollout.step_up(FlagKey.RISK_KILL_SWITCH_V2, "admin", "concurrent step")
            except Exception as e:
                errors.append(str(e))
    ...
    def test_T166_ea_kill_flow(self, store):
        ...
        assert not store.is_enabled(key, ctx)
        assert store.is_enabled(
            key, EvalContext(user_id="mt4_002", tenant_id="broker_beta", plan=PlanTier.PRO)
        )
    ...
    def test_T167_billing_plan_gate_flow(self, store):
        ...
        assert not store.is_enabled(
            key, EvalContext(user_id="trial", tenant_id="t1", plan=PlanTier.TRIAL)
        )
    ...
    def test_T169_is_enabled_helper(self):
        get_store().set_flag(
            FlagConfig(key=FlagKey.RISK_NEWS_FILTER, enabled=True), "admin", "test"
        )
        assert isinstance(
            is_enabled(FlagKey.RISK_NEWS_FILTER, EvalContext(user_id="u", tenant_id="t")), bool
        )
    ...
    def test_T173_flag_description(self):
        assert (
            "kill switch"
            in FlagConfig(
                key=FlagKey.RISK_KILL_SWITCH_V2, description="enables new kill switch logic"
            ).description
        )
...
class TestEdgeCasesAndCoverage:
    ...
    def test_T178_plan_tier_5(self):
    ...
    def test_T187_string_secret(self):
        assert (
            len(
                FlagAuditChain(secret="plain").record("k", "create", "admin", "test", {}).chain_hash
            )
            == 64
        )
    ...
    def test_T188_bytes_secret(self):
        assert (
            len(
                FlagAuditChain(secret=b"bytes")
                .record("k", "create", "admin", "test", {})
                .chain_hash
            )
            == 64
        )
