...
KSC = KillSwitchConfig(
    absolute_floor_usd=5_000,
...
async def test_T01_basic_lot():
    r = await _ls().calculate(
        balance=10_000, stop_loss_pips=20, symbol="EURUSD", equity=10_000, free_margin=5_000
    )
...
async def test_T03_margin_cap():
    r = await _ls().calculate(
        balance=100_000, stop_loss_pips=20, symbol="EURUSD", equity=100_000, free_margin=200
    )
...
async def test_T04_zero_sl_raises():
    with pytest.raises(ValueError):
        await _ls().calculate(balance=10_000, stop_loss_pips=0, symbol="EURUSD")
...
async def test_T06_unknown_symbol_min_lot():
    ls = LotSizer()
    ls.get_pip_value = AsyncMock(side_effect=Exception("no pip"))
...
async def test_T07_kelly_capped():
    r = await _ls().calculate(
        balance=10_000, stop_loss_pips=20, symbol="EURUSD", equity=10_000, win_rate=0.95, avg_rr=3.0
    )
...
async def test_T10_higher_risk_bigger_lot():
    ls = _ls()
    r1 = await ls.calculate(
        balance=10_000, stop_loss_pips=20, symbol="EURUSD", override_risk_pct=1.0
    )
    r2 = await ls.calculate(
        balance=10_000, stop_loss_pips=20, symbol="EURUSD", override_risk_pct=2.0
    )
...
async def test_T11_risk_pct_capped():
    ...
    ls = LotSizer(config=cfg)
    ls.get_pip_value = AsyncMock(return_value=(10.0, "mock"))
    r = await ls.calculate(
        balance=10_000, stop_loss_pips=20, symbol="EURUSD", equity=10_000, override_risk_pct=10.0
    )
...
async def test_T12_large_margin_no_cap():
    r = await _ls().calculate(
        balance=10_000, stop_loss_pips=20, symbol="EURUSD", equity=10_000, free_margin=1_000_000
    )
...
async def test_T14_low_level_blocked():
    r = await _mg().check(
        symbol="EURUSD", lot_size=0.1, balance=10_000, equity=110, free_margin=10, used_margin=100
    )
...
async def test_T17_nan_equity_blocked():
    r = await _mg().check(
        symbol="EURUSD", lot_size=0.1, balance=10_000, equity=float("nan"), free_margin=5_000
    )
    assert not r.can_trade
...
async def test_T18_level_at_threshold_blocked():
    r = await _mg().check(
        symbol="EURUSD", lot_size=0.01, balance=10_000, equity=150, free_margin=50, used_margin=100
    )
    assert not r.can_trade
...
async def test_T20_negative_free_margin_blocked():
    r = await _mg().check(
        symbol="EURUSD", lot_size=0.1, balance=10_000, equity=10_000, free_margin=-100
    )
    assert not r.can_trade
...
async def test_T23_large_lot_small_account():
    r = await _mg().check(
        symbol="EURUSD", lot_size=100.0, balance=100, equity=100, free_margin=10, used_margin=0
    )
    assert not r.can_trade
...
async def test_T24_fail_open_no_crash():
    with patch("asyncio.wait_for", side_effect=asyncio.TimeoutError()):
        r = await _mg(FailMode.FAIL_OPEN).check(
            symbol="EURUSD",
            lot_size=0.1,
            balance=10_000,
            equity=10_000,
...
async def test_T25_floor_triggers():
    ...
    try:
        await ks.check(equity=4_000, balance=10_000)
...
async def test_T26_drawdown_triggers():
    ...
    try:
        await ks.check(equity=8_900, balance=10_000)
    except KillSwitchActivatedError:
        pass
    assert ks.state.active
...
async def test_T28_manual_activate():
    ks = KillSwitch(config=KSC)
    await ks.activate(reason="admin")
    assert ks.state.active
...
async def test_T38_reason_stored():
    ks = KillSwitch(config=KSC)
    await ks.activate(reason="risk_breach")
    assert ks.state.reason != ""


# T39-T50 DAILY LIMITS + EQUITY PROTECTION
def test_T39_daily_loss():
    assert (
        not DailyLimitsEngine(max_daily_loss_pct=3.0)
        .check_limits(10_000, TodayTrades(pnl_usd=-350))
        .can_trade
    )


def test_T40_daily_trades():
    assert (
        not DailyLimitsEngine(max_daily_trades=5)
        .check_limits(10_000, TodayTrades(trade_count=5))
        .can_trade
    )


def test_T41_weekly_loss():
    assert (
        not DailyLimitsEngine(max_weekly_loss_pct=7.0)
        .check_limits(10_000, TodayTrades(), week_pnl_usd=-750)
        .can_trade
    )


def test_T42_monthly_dd():
    assert (
        not DailyLimitsEngine(max_monthly_dd_pct=15.0)
        .check_limits(10_000, TodayTrades(), month_pnl_usd=-1600)
        .can_trade
    )


def test_T43_warning():
    r = DailyLimitsEngine(max_daily_loss_pct=3.0).check_limits(10_000, TodayTrades(pnl_usd=-260))
    assert r.can_trade and r.status == LimitStatus.WARNING


def test_T44_all_clear():
    r = DailyLimitsEngine().check_limits(10_000, TodayTrades())
    assert r.can_trade and r.status == LimitStatus.OK


def test_T45_zero_balance():
    assert not DailyLimitsEngine().check_limits(0, TodayTrades()).can_trade


def test_T46_defaults():
    t = TodayTrades()
    assert t.trade_count == 0 and t.pnl_usd == 0.0


def test_T47_snapshot():
    dl = DailyLimitsEngine(max_daily_trades=8)
    assert DailyLimitsEngine.from_snapshot(dl.to_snapshot())._max_daily_trades == 8
...
def test_T48_drawdown_no_negative():
    ...
    ep.state.high_water_mark = 10_000
    ep.state.current_equity = 11_000
    ep.state.status = ProtectionStatus.SAFE
    assert ep.check().can_trade, "Equity above HWM must not trigger halt"


@pytest.mark.asyncio
async def test_T49_halt_on_drawdown():
    ep = EquityProtectionEngine(
        EquityProtectionConfig(total_drawdown_limit_pct=10.0, daily_loss_limit_pct=50.0)
    )
    ep.initialize(10_000)
    await ep.update_equity(8_900, 10_000)
    r = ep.check()
    assert not r.can_trade and r.level == ProtectionStatus.HALTED


@pytest.mark.asyncio
async def test_T50_cooldown_enforced():
    cfg = EquityProtectionConfig(
        total_drawdown_limit_pct=5.0, cooldown_minutes=60, daily_loss_limit_pct=50.0
    )
    ep = EquityProtectionEngine(cfg)
    ep.initialize(10_000)
    await ep.update_equity(9_400, 10_000)
    await ep.update_equity(9_800, 10_000)
...
async def test_T51_happy_path():
    ...
    ep = EquityProtectionEngine()
    ep.initialize(10_000)
    orch = m.RiskOrchestrator(
        equity_engine=ep, daily_limits=DailyLimitsEngine(), margin_gate=MarginGate()
    )
...
async def test_T56_no_margin_gate_approved():
    ...
    ep.initialize(10_000)
    assert (
        await m.RiskOrchestrator(equity_engine=ep, daily_limits=DailyLimitsEngine()).assess(_inp(m))
    ).approved
...
async def test_T57_margin_gate_blocks_low_equity():
    ...
    r = await m.RiskOrchestrator(
        equity_engine=ep, daily_limits=DailyLimitsEngine(), margin_gate=MarginGate()
    ).assess(_inp(m, equity=110, free_margin=10))
