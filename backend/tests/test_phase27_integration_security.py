...
class TestIntegrationKindAndPolicy:
    ...
    def test_T003_all_kinds_have_default_policy(self):
        for kind in IntegrationKind:
            assert kind in DEFAULT_POLICIES
    ...
    def test_T013_policy_deepcopy_isolation(self):
        r1 = IntegrationRegistry()
        r2 = IntegrationRegistry()
    ...
    def test_T014_custom_policy_override(self):
        ...
        r = IntegrationRegistry()
        r.register(IntegrationKind.PAYMENT, policy=p)
    ...
    def test_T016_idempotency_ttl_24h(self):
        for p in DEFAULT_POLICIES.values():
            assert p.idempotency_ttl == 86_400
...
class TestReplayProtector:
    ...
    def test_T019_different_events_allowed(self):
        rp = ReplayProtector(300)
        for i in range(10):
            rp.check_and_record(f"e{i}")
        assert rp.size == 10

    def test_T020_old_timestamp_rejected(self):
        rp = ReplayProtector(300)
        with pytest.raises(ReplayError):
            rp.check_and_record("old", int((time.time() - 400) * 1000))

    def test_T021_future_timestamp_rejected(self):
        rp = ReplayProtector(300)
        with pytest.raises(ReplayError):
            rp.check_and_record("fut", int((time.time() + 400) * 1000))
    ...
    def test_T023_is_seen_true(self):
        rp = ReplayProtector()
        rp.check_and_record("x")
        assert rp.is_seen("x") is True
    ...
    def test_T027_thread_safety(self):
        rp = ReplayProtector(600)
        seen = set()
        lock = threading.Lock()

        def w(i):
            try:
                rp.check_and_record(f"t{i}")
            except ReplayError:
                pass
            with lock:
                seen.add(i)
    ...
    def test_T031_boundary_outside(self):
        rp = ReplayProtector(300)
        with pytest.raises(ReplayError):
            rp.check_and_record("out", int((time.time() - 301) * 1000))

    def test_T032_zero_window_entries_expire_immediately(self):
        rp = ReplayProtector(0)
        rp.check_and_record("z1")
        rp.check_and_record("z2")
        assert rp.size <= 2
...
class TestSignatureVerifier:
    def test_T033_hmac256_valid(self):
        ...
        b = b"data"
        assert sv.verify(SignatureScheme.HMAC_SHA256, b, _sign(b)) is True

    def test_T034_hmac256_invalid(self):
        with pytest.raises(SignatureError):
            SignatureVerifier(SECRET).verify(SignatureScheme.HMAC_SHA256, b"d", "bad")

    def test_T035_hmac512_valid(self):
        sv = SignatureVerifier(SECRET)
        b = b"p"
        assert (
            sv.verify(SignatureScheme.HMAC_SHA512, b, _sign(b, scheme=SignatureScheme.HMAC_SHA512))
            is True
        )

    def test_T036_hmac512_invalid(self):
        with pytest.raises(SignatureError):
            SignatureVerifier(SECRET).verify(SignatureScheme.HMAC_SHA512, b"d", "bad")
    ...
    def test_T038_plain_token_invalid(self):
        with pytest.raises(SignatureError):
            SignatureVerifier(SECRET).verify(SignatureScheme.PLAIN_TOKEN, b"any", "wrong")
    ...
    def test_T040_rsa_stub_valid(self):
        ...
        b = b"rsa"
        assert (
            sv.verify(SignatureScheme.RSA_SHA256, b, sv.sign(SignatureScheme.RSA_SHA256, b)) is True
        )

    def test_T041_ed25519_stub_valid(self):
        sv = SignatureVerifier(SECRET)
        b = b"ed"
        assert sv.verify(SignatureScheme.ED25519, b, sv.sign(SignatureScheme.ED25519, b)) is True

    def test_T042_wrong_secret(self):
        with pytest.raises(SignatureError):
            SignatureVerifier("wrong").verify(SignatureScheme.HMAC_SHA256, b"d", _sign(b"d"))
    ...
    def test_T046_bytes_secret(self):
        sv = SignatureVerifier(SECRET_B)
        b = b"bs"
        assert (
            sv.verify(SignatureScheme.HMAC_SHA256, b, sv.sign(SignatureScheme.HMAC_SHA256, b))
            is True
        )
...
class TestIdempotencyStore:
    def test_T049_new_key_none(self):
        assert IdempotencyStore().check("k", {"a": 1}) is None

    def test_T050_cached(self):
        s = IdempotencyStore()
        s.record("k", {"a": 1}, "ok")
        assert s.check("k", {"a": 1}) == "ok"
    ...
    def test_T052_invalidate(self):
        ...
        assert s.check("k", {"x": 1}) is None

    def test_T053_invalidate_missing(self):
        assert IdempotencyStore().invalidate("no") is False

    def test_T054_size(self):
        s = IdempotencyStore()
        for i in range(5):
            s.record(f"k{i}", {"i": i}, i)
        assert s.size == 5
    ...
    def test_T056_order_insensitive(self):
        s = IdempotencyStore()
        s.record("k", {"b": 2, "a": 1}, "r")
        assert s.check("k", {"a": 1, "b": 2}) == "r"

    def test_T057_complex_payload(self):
        s = IdempotencyStore()
        p = {"n": {"x": [1, 2]}}
        s.record("c", p, "d")
        assert s.check("c", p) == "d"

    def test_T058_none_result(self):
        s = IdempotencyStore()
        s.record("n", {}, None)
        assert s.size == 1
...
class TestCircuitBreaker:
    def test_T065_initial_closed(self):
        assert CircuitBreaker(3).state == CircuitState.CLOSED

    def test_T066_allow_closed(self):
    ...
    def test_T074_record_failure_returns_state(self):
        cb = CircuitBreaker(2)
        assert cb.record_failure() == CircuitState.CLOSED
        assert cb.record_failure() == CircuitState.OPEN

    def test_T075_thread_safe(self):
        cb = CircuitBreaker(100)
        ts = [threading.Thread(target=cb.record_failure) for _ in range(50)]
        for t in ts:
            t.start()
        for t in ts:
            t.join()
...
class TestRetryPolicy:
    def test_T081_within_limit(self):
        r = RetryPolicy(3)
        assert r.should_retry(1) and r.should_retry(2) and r.should_retry(3)

    def test_T082_at_limit(self):
        assert RetryPolicy(3).should_retry(4) is False
    ...
    def test_T084_delay_capped(self):
        ...
        for a in range(20):
            assert r.delay_ms(a) <= 5000

    def test_T085_jitter_bounds(self):
        r = RetryPolicy(100, 1000, jitter=True)
        for a in range(10):
            assert 0 <= r.delay_ms(a) <= 1000

    def test_T086_no_jitter_deterministic(self):
        r = RetryPolicy(100, 100_000, jitter=False)
        assert r.delay_ms(3) == r.delay_ms(3)

    def test_T087_zero_retries(self):
        assert RetryPolicy(0).should_retry(1) is False

    def test_T088_base_ms(self):
        assert RetryPolicy(200, 200, jitter=False).delay_ms(0) == 200

    def test_T089_attempt_zero(self):
        assert RetryPolicy(100, 100_000, jitter=False).delay_ms(0) == 100

    def test_T090_attempt_one(self):
        assert RetryPolicy(100, 100_000, jitter=False).delay_ms(1) == 200

    def test_T091_attempt_two(self):
        assert RetryPolicy(100, 100_000, jitter=False).delay_ms(2) == 400

    def test_T092_max_retries_5(self):
        r = RetryPolicy(5)
        assert r.should_retry(5) is True
        assert r.should_retry(6) is False

    def test_T093_exc_ignored(self):
        assert RetryPolicy(3).should_retry(1, ValueError()) is True

    def test_T094_large_attempt_capped(self):
        assert RetryPolicy(100, 1000, jitter=False).delay_ms(100) == 1000

    def test_T095_from_policy(self):
        p = DEFAULT_POLICIES[IntegrationKind.EMAIL]
        assert RetryPolicy(p.max_retries).max_retries == 5

    def test_T096_webhook_no_retry(self):
        p = DEFAULT_POLICIES[IntegrationKind.WEBHOOK_IN]
        assert RetryPolicy(p.max_retries).should_retry(1) is False


class TestIntegrationAuditChain:
    def test_T097_genesis_64(self):
        assert len(IntegrationAuditChain(SECRET).genesis_hash) == 64

    def test_T098_genesis_hmac(self):
        import hmac as h

        assert (
            IntegrationAuditChain(SECRET).genesis_hash
            == h.new(SECRET_B, b"GENESIS:INTEGRATION:SECURITY:V27", hashlib.sha256).hexdigest()
        )
    ...
    def test_T100_verify_empty(self):
        assert IntegrationAuditChain(SECRET).verify_chain() is True

    def test_T101_verify_valid(self):
        c = IntegrationAuditChain(SECRET)
        for i in range(5):
            c.record(AuditAction.CALL_OK, IntegrationKind.WEBHOOK_IN, f"e{i}")
        assert c.verify_chain() is True

    def test_T102_tamper_detected(self):
        c = IntegrationAuditChain(SECRET)
        for i in range(3):
            c.record(AuditAction.CALL_OK, IntegrationKind.PAYMENT, f"e{i}")
        rs = list(c._records)
        rs[1].chain_hash = "a" * 64
        c._records.clear()
        c._records.extend(rs)
    ...
    def test_T106_wrong_secret_invalid(self):
        c1 = IntegrationAuditChain("A")
        c1.record(AuditAction.CALL_OK, IntegrationKind.PAYMENT, "e1")
        c2 = IntegrationAuditChain("B")
    ...
    def test_T112_size(self):
        c = IntegrationAuditChain(SECRET)
        for i in range(7):
            c.record(AuditAction.CALL_OK, IntegrationKind.EMAIL, f"e{i}")
        assert c.size == 7
    ...
    def test_T116_detail_stored(self):
        c = IntegrationAuditChain(SECRET)
        e = c.record(
            AuditAction.CALL_FAIL, IntegrationKind.PAYMENT, "p1", detail={"error": "timeout"}
        )
...
class TestSafeIntegrationCall:
    def _setup(self, kind=IntegrationKind.WEBHOOK_IN):
        r = _registry(kind)
        a = IntegrationAuditChain(SECRET)
        c = SafeIntegrationCall(r, audit=a)
        return r, a, c
    ...
    def test_T117_success(self):
        ...
        sig = _sign(raw)
        assert (
            c.call(_event(signature=sig), lambda e: "ok", raw_body=raw).result
    ...
    def test_T118_bad_sig_blocked(self):
        _, _, c = self._setup()
        assert (
            c.call(_event(signature="bad"), lambda e: "ok", raw_body=b"d").result
            == IntegrationResult.SIG_INVALID
        )
    ...
    def test_T119_replay_blocked(self):
        ...
        r = c.call(
            _event(event_id=eid, signature=sig, payload={"a": 1}), lambda e: "ok", raw_body=raw
        )
    ...
    def test_T120_idempotent_hit(self):
        ...
        c.call(
            IntegrationEvent(
                IntegrationKind.WEBHOOK_IN, str(uuid.uuid4()), p, _sign(raw), idempotency_key=ikey
            ),
            lambda e: "first",
            raw_body=raw,
        )
    ...
    def test_T121_circuit_opens(self):
        ...
        c = SafeIntegrationCall(r)
        for _ in range(2):
            c.call(
                _event(), lambda e: (_ for _ in ()).throw(RuntimeError("x")), verify_signature=False
            )
        assert (
            c.call(
                _event(), lambda e: (_ for _ in ()).throw(RuntimeError("x")), verify_signature=False
            ).result
            == IntegrationResult.CIRCUIT_OPEN
        )
    ...
    def test_T122_dead_letter(self):
        ...
        assert (
            c.call(
                _event(), lambda e: (_ for _ in ()).throw(RuntimeError("f")), verify_signature=False
            ).result
            == IntegrationResult.DEAD_LETTERED
        )
    ...
    def test_T125_reset_requires_reason(self):
        ...
        with pytest.raises(MissingReasonError):
            c.reset_circuit(IntegrationKind.WEBHOOK_IN, "")
    ...
    def test_T126_reset_closes_circuit(self):
        ...
        c = SafeIntegrationCall(r)
        c.call(_event(), lambda e: (_ for _ in ()).throw(RuntimeError("x")), verify_signature=False)
        c.reset_circuit(IntegrationKind.WEBHOOK_IN, "maint")
        assert c.circuit_state(IntegrationKind.WEBHOOK_IN) == CircuitState.CLOSED

    def test_T127_no_verify_skip(self):
        _, _, c = self._setup()
        assert (
            c.call(_event(signature=None), lambda e: "ok", verify_signature=False).result
            == IntegrationResult.SUCCESS
        )
    ...
    def test_T129_latency_positive(self):
        ...
        sig = _sign(raw)
        assert (
            c.call(_event(signature=sig, payload={}), lambda e: "ok", raw_body=raw).latency_ms >= 0
        )
    ...
    def test_T133_payment_flow(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.PAYMENT, secret=SECRET)
        c = SafeIntegrationCall(r)
        raw = b'{"charge":"ch","amount":5000}'
        sig = _sign(raw)
        assert (
            c.call(
                _event(
                    IntegrationKind.PAYMENT, signature=sig, payload={"charge": "ch", "amount": 5000}
                ),
                lambda e: {"status": "ok"},
                raw_body=raw,
            ).result
    ...
    def test_T134_telegram_flow(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.TELEGRAM, secret=SECRET)
        c = SafeIntegrationCall(r)
        assert (
            c.call(
                _event(IntegrationKind.TELEGRAM, signature=SECRET, payload={"id": 1}),
                lambda e: "ack",
                raw_body=b'{"id":1}',
            ).result
            == IntegrationResult.SUCCESS
    ...
    def test_T135_webhook_out_no_replay(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.WEBHOOK_OUT, secret=SECRET)
        c = SafeIntegrationCall(r)
        eid = str(uuid.uuid4())
        raw = b'{"e":"paid"}'
        sig = _sign(raw)
        for _ in range(2):
            res = c.call(
                _event(
                    IntegrationKind.WEBHOOK_OUT, event_id=eid, signature=sig, payload={"e": "paid"}
                ),
                lambda e: "ok",
    ...
    def test_T136_market_data_flow(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.MARKET_DATA, secret=SECRET)
        c = SafeIntegrationCall(r)
        raw = b'{"sym":"EURUSD"}'
        sig = _sign(raw)
        assert (
            c.call(
                _event(IntegrationKind.MARKET_DATA, signature=sig, payload={"sym": "EURUSD"}),
                lambda e: {"t": "ok"},
                raw_body=raw,
            ).result
            == IntegrationResult.SUCCESS
...
class TestIntegrationRegistry:
    def test_T137_register_policy(self):
        ...
        r.register(
            ...
            policy=IntegrationPolicy(kind=IntegrationKind.EMAIL, max_retries=7),
        )
        assert r.policy(IntegrationKind.EMAIL).max_retries == 7

    def test_T138_register_secret(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.EMAIL, secret="s")
        assert r.secret(IntegrationKind.EMAIL) == b"s"

    def test_T139_missing_secret_raises(self):
        with pytest.raises(IntegrationPolicyError):
            IntegrationRegistry().secret(IntegrationKind.EMAIL)

    def test_T140_missing_policy_raises(self):
        r = IntegrationRegistry()
        del r._policies[IntegrationKind.EMAIL]
        with pytest.raises(IntegrationPolicyError):
            r.policy(IntegrationKind.EMAIL)

    def test_T141_revoke_removes(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.EMAIL, secret="s")
        r.revoke_secret(IntegrationKind.EMAIL, "c")
    ...
    def test_T142_revoke_requires_reason(self):
        ...
        r.register(IntegrationKind.EMAIL, secret="s")
        with pytest.raises(MissingReasonError):
            r.revoke_secret(IntegrationKind.EMAIL, "")

    def test_T143_has_secret_false(self):
        assert IntegrationRegistry().has_secret(IntegrationKind.EMAIL) is False

    def test_T144_has_secret_true(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.EMAIL, secret="s")
        assert r.has_secret(IntegrationKind.EMAIL) is True

    def test_T145_list_kinds(self):
        assert len(IntegrationRegistry().list_kinds()) == 7

    def test_T146_bytes_secret(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.PAYMENT, secret=b"bs")
        assert r.secret(IntegrationKind.PAYMENT) == b"bs"
    ...
    def test_T148_register_twice_overwrites(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.EMAIL, secret="v1")
        r.register(IntegrationKind.EMAIL, secret="v2")
    ...
    def test_T149_thread_safe_register(self):
        ...
        def w(i):
            try:
                r.register(IntegrationKind.EMAIL, secret=f"s{i}")
            except Exception as e:
                errs.append(e)
    ...
    def test_T150_revoke_missing_silent(self):
        IntegrationRegistry().revoke_secret(IntegrationKind.EMAIL, reason="c")

    def test_T151_default_not_modified(self):
        orig = DEFAULT_POLICIES[IntegrationKind.PAYMENT].max_retries
        r = IntegrationRegistry()
        r._policies[IntegrationKind.PAYMENT].max_retries = 999
        assert DEFAULT_POLICIES[IntegrationKind.PAYMENT].max_retries == orig

    def test_T152_policy_kind_match(self):
        r = IntegrationRegistry()
        r.register(IntegrationKind.EMAIL, policy=IntegrationPolicy(kind=IntegrationKind.EMAIL))
...
class TestIntegrationAdmin:
    def _setup(self):
        ...
        for k in IntegrationKind:
            r.register(k, secret=SECRET)
        a = IntegrationAuditChain(SECRET)
        c = SafeIntegrationCall(r, audit=a)
        adm = IntegrationAdmin(r, c, a)
        return r, a, c, adm
    ...
    def test_T154_summary_verified(self):
        assert self._setup()[3].summary()["audit_verified"] is True

    def test_T155_circuits_all_closed(self):
        for v in self._setup()[3].inspect_circuits().values():
            assert v == CircuitState.CLOSED.value

    def test_T156_revoke_requires_reason(self):
        with pytest.raises(MissingReasonError):
            self._setup()[3].revoke_key(IntegrationKind.PAYMENT, "  ")

    def test_T157_revoke_removes_secret(self):
        r, _, _, adm = self._setup()
        adm.revoke_key(IntegrationKind.PAYMENT, "c")
        assert r.has_secret(IntegrationKind.PAYMENT) is False

    def test_T158_revoke_audited(self):
        _, a, _, adm = self._setup()
        adm.revoke_key(IntegrationKind.TELEGRAM, "r")
        assert len(a.query(action=AuditAction.KEY_REVOKED)) >= 1

    def test_T159_drain_requires_reason(self):
        with pytest.raises(MissingReasonError):
            self._setup()[3].drain_dlq(reason="")

    def test_T160_drain_returns_items(self):
        r, a, c, adm = self._setup()
        p = IntegrationPolicy(
            kind=IntegrationKind.WEBHOOK_IN,
            scheme=SignatureScheme.NONE,
            circuit_threshold=100,
    ...
    def test_T161_reset_via_admin(self):
        ...
        r.register(IntegrationKind.WEBHOOK_IN, policy=p, secret=SECRET)
        c.call(_event(), lambda e: (_ for _ in ()).throw(RuntimeError("x")), verify_signature=False)
        adm.reset_circuit(IntegrationKind.WEBHOOK_IN, "m")
        assert c.circuit_state(IntegrationKind.WEBHOOK_IN) == CircuitState.CLOSED
    ...
    def test_T163_revoke_chain_valid(self):
        _, a, _, adm = self._setup()
        adm.revoke_key(IntegrationKind.EMAIL, "t")
        assert a.verify_chain() is True

    def test_T164_registered_kinds_count(self):
        assert len(self._setup()[3].summary()["registered_kinds"]) == 7
    ...
    def test_T165_audit_grows(self):
        ...
        sig = _sign(raw)
        for _ in range(5):
            c.call(_event(signature=sig, payload={"x": 1}), lambda e: "ok", raw_body=raw)
        assert adm.summary()["audit_size"] >= 5

    def test_T166_reset_requires_reason(self):
        with pytest.raises(MissingReasonError):
            self._setup()[3].reset_circuit(IntegrationKind.EMAIL, "")
    ...
    def test_T168_circuit_all_kinds(self):
        ...
        for k in IntegrationKind:
            assert k.value in states


class TestBuildIntegration:
    def test_T169_returns_tuple(self):
        assert all(x is not None for x in build_integration(IntegrationKind.PAYMENT, SECRET))

    def test_T170_registers_secret(self):
        assert build_integration(IntegrationKind.EMAIL, SECRET)[1].has_secret(IntegrationKind.EMAIL)

    def test_T171_custom_policy(self):
        p = IntegrationPolicy(
            kind=IntegrationKind.TELEGRAM, scheme=SignatureScheme.PLAIN_TOKEN, max_retries=0
        )
        assert (
            build_integration(IntegrationKind.TELEGRAM, SECRET, policy=p)[1]
            .policy(IntegrationKind.TELEGRAM)
            .max_retries
            == 0
        )
    ...
    def test_T172_shared_audit(self):
        a = IntegrationAuditChain(SECRET)
        assert build_integration(IntegrationKind.PAYMENT, SECRET, audit=a)[2] is a

    def test_T173_payment_call(self):
        c, r, a = build_integration(IntegrationKind.PAYMENT, SECRET)
        raw = b'{"ch":"x"}'
        sig = _sign(raw)
        assert (
            c.call(
                _event(IntegrationKind.PAYMENT, signature=sig, payload={"ch": "x"}),
                lambda e: "ok",
                raw_body=raw,
            ).result
            == IntegrationResult.SUCCESS
    ...
    def test_T174_email_call(self):
        c, r, a = build_integration(IntegrationKind.EMAIL, SECRET)
        raw = b'{"to":"u@e.com"}'
        sig = _sign(raw)
        assert (
            c.call(
                _event(IntegrationKind.EMAIL, signature=sig, payload={"to": "u@e.com"}),
                lambda e: "q",
                raw_body=raw,
            ).result
            == IntegrationResult.SUCCESS
    ...
    def test_T176_market_data(self):
        c, r, a = build_integration(IntegrationKind.MARKET_DATA, SECRET)
        raw = b'{"sym":"GBP"}'
        sig = _sign(raw)
        assert (
            c.call(
                _event(IntegrationKind.MARKET_DATA, signature=sig, payload={"sym": "GBP"}),
                lambda e: {"t": True},
                raw_body=raw,
            ).result
            == IntegrationResult.SUCCESS
...
class TestIntegrationFlows:
    def test_T177_fake_webhook_rejected(self):
        c, r, a = build_integration(IntegrationKind.WEBHOOK_IN, "real")
        raw = b'{"o":"1"}'
        sig = _hmac.new(b"attacker", raw, hashlib.sha256).hexdigest()
        assert (
            c.call(
                _event(IntegrationKind.WEBHOOK_IN, signature=sig, payload={"o": "1"}),
                lambda e: "ok",
                raw_body=raw,
            ).result
            == IntegrationResult.SIG_INVALID
    ...
    def test_T178_replayed_blocked(self):
        c, r, a = build_integration(IntegrationKind.WEBHOOK_IN, SECRET)
        raw = b'{"o":"2"}'
        sig = _sign(raw)
        eid = str(uuid.uuid4())
        for _ in range(2):
            res = c.call(
                _event(IntegrationKind.WEBHOOK_IN, event_id=eid, signature=sig, payload={"o": "2"}),
                lambda e: "ok",
                raw_body=raw,
            )
    ...
    def test_T179_old_ts_rejected(self):
        ...
        res = c.call(
            _event(
                IntegrationKind.WEBHOOK_IN,
                signature=sig,
                payload={"o": "3"},
                timestamp_ms=int((time.time() - 400) * 1000),
            ),
            lambda e: "ok",
            raw_body=raw,
        )
    ...
    def test_T180_payment_idempotency(self):
        ...
        c.call(
            IntegrationEvent(
                IntegrationKind.PAYMENT, str(uuid.uuid4()), p, _sign(raw), idempotency_key=ikey
            ),
            lambda e: {"s": "ok"},
            raw_body=raw,
        )
    ...
    def test_T181_circuit_protects(self):
        r = IntegrationRegistry()
        p = IntegrationPolicy(
            kind=IntegrationKind.MARKET_DATA,
            scheme=SignatureScheme.NONE,
            circuit_threshold=3,
            circuit_timeout_s=60,
            max_retries=0,
        )
        r.register(IntegrationKind.MARKET_DATA, policy=p, secret=SECRET)
        c = SafeIntegrationCall(r)
        for _ in range(3):
            c.call(
                _event(IntegrationKind.MARKET_DATA),
                lambda e: (_ for _ in ()).throw(ConnectionError("down")),
                verify_signature=False,
            )
        assert (
            c.call(
                _event(IntegrationKind.MARKET_DATA), lambda e: "ok", verify_signature=False
            ).result
            == IntegrationResult.CIRCUIT_OPEN
        )

    def test_T182_auth_flow(self):
        c, r, a = build_integration(IntegrationKind.AUTH_PROVIDER, SECRET)
        raw = b'{"tok":"x"}'
        sig = _sign(raw)
        assert (
            c.call(
                _event(IntegrationKind.AUTH_PROVIDER, signature=sig, payload={"tok": "x"}),
                lambda e: {"v": True},
                raw_body=raw,
            ).result
            == IntegrationResult.SUCCESS
    ...
    def test_T183_audit_100_events(self):
        ...
        sig = _sign(raw)
        for _ in range(100):
            c.call(_event(signature=sig, payload={"n": 1}), lambda e: "ok", raw_body=raw)
        assert a.verify_chain() is True
    ...
    def test_T184_concurrent_50_success(self):
        ...
        def w():
            raw = b'{"x":1}'
            sig = _sign(raw)
            e = _event(signature=sig, payload={"x": 1})
    ...
    def test_T185_revoke_blocks(self):
        c, r, a = build_integration(IntegrationKind.PAYMENT, SECRET)
        r.revoke_secret(IntegrationKind.PAYMENT, "rot")
        with pytest.raises(IntegrationPolicyError):
            c.verify_inbound(_event(IntegrationKind.PAYMENT, signature="s"), b"d")
    ...
    def test_T186_dlq_max(self):
        ...
        c._dlq = collections.deque(maxlen=5)
        for _ in range(10):
            c.call(
                _event(), lambda e: (_ for _ in ()).throw(RuntimeError("x")), verify_signature=False
            )
    ...
    def test_T187_no_exception_leaks(self):
        ...
        r.register(IntegrationKind.EMAIL, policy=p, secret=SECRET)
        c = SafeIntegrationCall(r)
        res = c.call(
            _event(IntegrationKind.EMAIL),
    ...
    def test_T188_hmac512_scheme(self):
        r = IntegrationRegistry()
        p = IntegrationPolicy(
            kind=IntegrationKind.EMAIL, scheme=SignatureScheme.HMAC_SHA512, max_retries=0
        )
        r.register(IntegrationKind.EMAIL, policy=p, secret=SECRET)
        c = SafeIntegrationCall(r)
        raw = b'{"to":"x"}'
        sig = _sign(raw, scheme=SignatureScheme.HMAC_SHA512)
        assert (
            c.call(
                _event(IntegrationKind.EMAIL, signature=sig, payload={"to": "x"}),
                lambda e: "q",
                raw_body=raw,
            ).result
            == IntegrationResult.SUCCESS
    ...
    def test_T189_none_scheme_no_secret(self):
        r = IntegrationRegistry()
        p = IntegrationPolicy(kind=IntegrationKind.WEBHOOK_IN, scheme=SignatureScheme.NONE)
        r.register(IntegrationKind.WEBHOOK_IN, policy=p, secret="d")
        c = SafeIntegrationCall(r)
    ...
    def test_T190_idempotency_no_double_exec(self):
        ...
        c.call(
            IntegrationEvent(
                IntegrationKind.WEBHOOK_IN, str(uuid.uuid4()), pl, idempotency_key=ikey
            ),
            h,
            verify_signature=False,
        )
    ...
    def test_T192_tamper_detect(self):
        ...
        for _ in range(5):
            c.call(
                _event(IntegrationKind.PAYMENT, signature=sig, payload={"am": 500}),
                lambda e: "ok",
                raw_body=raw,
            )
        assert a.verify_chain() is True
        rs = list(a._records)
        rs[2].chain_hash = "dead" * 16
        a._records.clear()
        a._records.extend(rs)
    ...
    def test_T194_call_result_defaults(self):
        r = CallResult(IntegrationResult.SUCCESS, "e1")
        assert r.attempts == 0 and r.cached is False and r.error is None

    def test_T195_dead_letter_fields(self):
        d = DeadLetterItem(_event(), "t", 3, time.time())
        assert d.attempts == 3 and d.reason == "t"
