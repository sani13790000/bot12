...
def make_engine() -> LicenseEngine:
    return LicenseEngine(
        server_secret=SERVER_SECRET, license_salt=LICENSE_SALT, device_salt=DEVICE_SALT
    )
...
def make_heartbeat_req(key_hash, device_id, offset_sec=0.0, nonce=None):
    ...
    return HeartbeatRequest(
        key_hash=key_hash, device_id=device_id, nonce=nonce, timestamp=ts, client_sig=sig
    ), nonce


def register_device(engine, key_hash, client_id="client_abc", ip="1.2.3.4", ua="TestAgent/1.0"):
    ok, msg, did = engine.register_device(
        key_hash=key_hash, client_id=client_id, ip=ip, user_agent=ua
    )
...
class TestRawKeyNeverStored:
    def test_T11_record_has_no_raw_key_field(self):
        e = make_engine()
        raw, h = e.create_license("u1", PlanTier.PRO)
        rec = e.get_record(h)
...
class TestHeartbeat:
    def _setup(self):
        e = make_engine()
        _, h = setup_active_license(e, PlanTier.PRO)
    ...
    def test_T23_signature_verifiable_by_client(self):
        ...
        payload = {
            "valid": True,
            "plan": resp.plan,
            "days": resp.days_remaining,
            "feats": resp.features,
            "ts": resp.server_ts,
            "nonce": resp.nonce_echo,
        }
...
class TestDeviceFingerprint:
    ...
    def test_T32_fingerprint_changes_with_client(self):
        assert _device_fingerprint("c1", "1.1.1.1", "UA", "s") != _device_fingerprint(
            "c2", "1.1.1.1", "UA", "s"
        )
    ...
    def test_T36_salt_changes_fingerprint(self):
        assert _device_fingerprint("c", "1.1.1.1", "UA", "salt1") != _device_fingerprint(
            "c", "1.1.1.1", "UA", "salt2"
        )
...
class TestSubscriptionFailClosed:
    ...
    def test_T49_trial_limits_respected(self):
        e = make_engine()
        _, h = setup_active_license(e, PlanTier.TRIAL)
        r = e.check(h)
        assert (
            r.allowed
            and PLAN_LIMITS[PlanTier.TRIAL].max_devices < PLAN_LIMITS[PlanTier.PRO].max_devices
        )

    def test_T50_lifetime_always_allowed(self):
        e = make_engine()
        _, h = setup_active_license(e, PlanTier.LIFETIME)
...
class TestSignedResponse:
    ...
    def test_T54_different_secret_different_sig(self):
        p = {"a": 1}
        assert _sign_response(p, "secret_A_32_chars_padding_ok!!") != _sign_response(
            p, "secret_B_32_chars_padding_ok!!"
        )
...
class TestDeviceLimitAtomic:
    def test_T61_pro_allows_3_devices(self):
        ...
        _, h = setup_active_license(e, PlanTier.PRO)
        for i in range(3):
            ok, msg, _ = e.register_device(h, f"c{i}", f"1.1.1.{i}", "UA")
            assert ok
    ...
    def test_T62_pro_blocks_4th_device(self):
        ...
        _, h = setup_active_license(e, PlanTier.PRO)
        for i in range(3):
            e.register_device(h, f"c{i}", f"1.1.1.{i}", "UA")
    ...
    def test_T64_device_count_accurate(self):
        ...
        _, h = setup_active_license(e, PlanTier.PRO)
        for i in range(3):
            e.register_device(h, f"c{i}", f"1.{i}.{i}.{i}", "UA")
...
class TestLicenseStateMachine:
    ...
    def test_T72_active_to_suspended(self):
        ...
        _, h = setup_active_license(e)
        e.suspend_license(h, "r", "admin")
        assert e.get_record(h).status == LicenseStatus.SUSPENDED

    def test_T73_suspended_to_active(self):
        e = make_engine()
        _, h = setup_active_license(e)
        e.suspend_license(h, "r", "admin")
        e.resume_license(h, "admin")
        assert e.get_record(h).status == LicenseStatus.ACTIVE

    def test_T74_active_to_revoked(self):
        e = make_engine()
        _, h = setup_active_license(e)
        e.revoke_license(h, "fraud", "admin")
        assert e.get_record(h).status == LicenseStatus.REVOKED

    def test_T75_revoked_cannot_be_revoked_again(self):
        e = make_engine()
        _, h = setup_active_license(e)
        e.revoke_license(h, "r1", "admin")
        assert not e.revoke_license(h, "r2", "admin")
    ...
    def test_T79_cannot_suspend_revoked(self):
        ...
        _, h = setup_active_license(e)
        e.revoke_license(h, "r", "admin")
        assert not e.suspend_license(h, "r2", "admin")

    def test_T80_state_changes_logged(self):
        e = make_engine()
        _, h = setup_active_license(e)
        e.suspend_license(h, "fraud", "admin")
        e.resume_license(h, "admin")
...
class TestAuditLog:
    def test_T81_create_logs_event(self):
        e = make_engine()
        _, h = e.create_license("u1", PlanTier.PRO, actor="admin")
    ...
    def test_T88_audit_capped_at_500(self):
        ...
        did = register_device(e, h)
        for i in range(600):
            nonce = f"nonce_{i:06d}_{secrets.token_hex(4)}"
            ts = time.time()
            req = HeartbeatRequest(
                ...
                nonce=nonce,
                timestamp=ts,
                client_sig=_hmac_sha256(DEVICE_SECRET, f"{nonce}:{ts}:{did}"),
            )
            e.heartbeat(req, DEVICE_SECRET)
...
class TestNonceStore:
    ...
    def test_T95_store_bounded(self):
        ...
        ns._MAX_NONCES = 10
        for i in range(15):
            ns.consume(f"nonce_{i:04d}", time.time())
        assert len(ns) <= 10

    def test_T96_engine_secret_min_length_enforced(self):
        with pytest.raises(ValueError, match="32"):
            LicenseEngine("short", LICENSE_SALT, DEVICE_SALT)
