...
class InMemoryTable:
    ...
    def add_check(self, fn): self._checks.append(fn); return self
    def add_not_null(self, col): self._not_null.append(col); return self
    ...
    def all(self) -> List[Dict]: return list(self._rows)
...
SUBSCRIPTION_STATUSES = {"trial","active","past_due","suspended","cancelled","revoked","expired"}
...
def make_refresh_tokens_table() -> InMemoryTable:
    ...
    t.add_check(lambda r: r.get("revoke_reason") in (None,"rotation","logout","admin_revoke","security_event"))
    return t
...
class TestLicenseHardening:
    ...
    def test_T11_insert_ok(self):
        key = gen_license_key()
        row = self.t.insert({"license_key":key,"key_hash":hash_license_key(key),"user_id":"u1","status":"active"})
    ...
    def test_T12_duplicate_key_raises(self):
        ...
        with pytest.raises(UniqueViolation):
            self.t.insert({"license_key":key,"key_hash":h+"x","user_id":"u2","status":"active"})
    ...
    def test_T13_duplicate_hash_raises(self):
        ...
        self.t.insert({"license_key":key,"key_hash":h,"user_id":"u1","status":"active"})
        with pytest.raises(UniqueViolation):
            self.t.insert({"license_key":"OTHER-KEY","key_hash":h,"user_id":"u2","status":"active"})
    ...
    def test_T15_invalid_status_raises(self):
        with pytest.raises(CheckViolation):
            self.t.insert({"license_key":gen_license_key(),"key_hash":"h","user_id":"u1","status":"INVALID"})

    def test_T16_all_6_statuses_ok(self):
        for s in LICENSE_STATUSES:
            key = gen_license_key()
            row = self.t.insert({"license_key":key,"key_hash":hash_license_key(key),"user_id":"u1","status":s})
            assert row["status"] == s
    ...
    def test_T21_two_users_different_keys(self):
        ...
        self.t.insert({"license_key":k1,"key_hash":hash_license_key(k1),"user_id":"u1","status":"active"})
        self.t.insert({"license_key":k2,"key_hash":hash_license_key(k2),"user_id":"u2","status":"active"})
...
class TestLicenseDevices:
    ...
    def test_T30_heartbeat_fields_exist(self):
        row = self.t.insert({"license_id":"lic1","device_fingerprint":"fp1","last_heartbeat_ts":time.time(),"heartbeat_interval_s":60})
...
class TestAuditLog:
    ...
    def test_T33_insert_audit(self):
        row = self.t.insert({"actor_id":"admin1","action":"license.revoke","ts":time.time(),"target_id":"lic1"})
    ...
    def test_T37_tamper_chain_concept(self):
        ...
        prev_hash = "0" * 64
        for i in range(5):
            entry = {"actor_id":"a1","action":f"act{i}","ts":time.time(),"prev_hash":prev_hash}
            entry["row_hash"] = hashlib.sha256(json.dumps({k:v for k,v in entry.items()}).encode()).hexdigest()
    ...
    def test_T41_immutable_concept(self):
        self.t.insert({"actor_id":"a1","action":"sensitive","ts":1.0,"sensitive_data":"secret"})
        rows = self.t.all()
        assert len(rows) == 1
...
class TestRefreshTokens:
    ...
    def _make_token(self, user_id="u1") -> Dict:
        raw = uuid.uuid4().hex
        return {"token_hash":hashlib.sha256(raw.encode()).hexdigest(),"user_id":user_id,"family_id":uuid.uuid4().hex,"expires_at":time.time()+3600}
    ...
    def test_T51_revoke_token(self):
        tok = self._make_token()
        self.t.insert(tok)
        self.t.update({"token_hash":tok["token_hash"]},{"revoke_reason":"logout","revoked_at":time.time()})
    ...
    def test_T52_family_reuse_detection(self):
        ...
        t1 = self._make_token(); t1["family_id"] = family
        t2 = self._make_token(); t2["family_id"] = family
        self.t.insert(t1)
        self.t.insert(t2)
...
class TestNonceStore:
    ...
    def test_T55_insert_nonce(self):
        row = self.t.insert({"nonce":uuid.uuid4().hex,"context":"heartbeat","expires_at":time.time()+300})
        assert row is not None
...
class TestBillingHardening:
    ...
    def test_T63_invoice_insert(self):
        row = self.inv.insert({"user_id":"u1","provider":"stripe","provider_ref":"pi_123","amount_usd":29.0,"status":"paid"})
        assert row is not None

    def test_T64_duplicate_provider_ref_raises(self):
        self.inv.insert({"user_id":"u1","provider":"stripe","provider_ref":"pi_123","amount_usd":29.0})
        with pytest.raises(UniqueViolation):
            self.inv.insert({"user_id":"u2","provider":"stripe","provider_ref":"pi_123","amount_usd":29.0})
    ...
    def test_T69_partial_unique_active_sub(self):
        ...
        self.sub.insert({"user_id":"u1","status":"active","plan_id":"basic","_active_key":"u1"})
        active_for_u1 = [r for r in self.sub.all() if r["user_id"]=="u1" and r["status"] in ACTIVE_SUB_STATUSES]
    ...
    def test_T71_on_conflict_nothing(self):
        ...
        result = self.inv.insert({"user_id":"u1","provider":"stripe","provider_ref":"pi_dup"}, on_conflict="nothing")
    ...
    def test_T73_invoice_audit_trail(self):
        invoices = []
        for i in range(3):
            row = self.inv.insert({"user_id":"u1","provider":"stripe","provider_ref":f"pi_{i}","amount_usd":29.0})
            invoices.append(row)
...
class TestTradeDedup:
    ...
    def test_T75_insert_order(self):
        row = self.t.insert({"user_id":"u1","mt5_ticket":"TKT001","idempotency_key":"ik1","symbol":"EURUSD","lots":0.1})
        assert row is not None
    ...
    def test_T79_idempotent_insert_fn(self):
        def insert_trade_idempotent(table, row):
            try:
                ...
                existing = [r for r in table.all() if r.get("idempotency_key")==row["idempotency_key"]]
                return existing[0] if existing else None
        self.t.insert({"user_id":"u1","mt5_ticket":"TKT001","idempotency_key":"ik1"})
        result = insert_trade_idempotent(self.t, {"user_id":"u1","mt5_ticket":"TKT001","idempotency_key":"ik1"})
    ...
    def test_T82_concurrent_dedup(self):
        ...
        def insert_worker(i):
            try:
                row = self.t.insert({"user_id":"u1","mt5_ticket":"TKT_CONCURRENT","idempotency_key":f"ik_{i}"})
                results.append(row)
            except UniqueViolation:
                errors.append(i)
...
class TestIntegration:
    def test_T84_license_to_device_flow(self):
        ...
        key = gen_license_key()
        lic.insert({"license_key":key,"key_hash":hash_license_key(key),"user_id":"u1","status":"active","id":"lic1"})
    ...
    def test_T85_payment_to_license_flow(self):
        ...
        lic = make_licenses_table()
        inv.insert({"user_id":"u1","provider":"stripe","provider_ref":"pi_paid","status":"paid"})
        key = gen_license_key()
        lic.insert({"license_key":key,"key_hash":hash_license_key(key),"user_id":"u1","status":"active"})
        assert lic.count() == 1
    ...
    def test_T87_rls_complete_isolation(self):
        lic = make_licenses_table()
        k1,k2 = gen_license_key(),gen_license_key()
        lic.insert({"license_key":k1,"key_hash":hash_license_key(k1),"user_id":"u1","status":"active"})
        lic.insert({"license_key":k2,"key_hash":hash_license_key(k2),"user_id":"u2","status":"active"})
    ...
    def test_T89_migration_idempotent_concept(self):
        inv = make_invoices_table()
        row = inv.insert({"user_id":"u1","provider":"stripe","provider_ref":"pi_1"}, on_conflict="nothing")
        row2 = inv.insert({"user_id":"u1","provider":"stripe","provider_ref":"pi_1"}, on_conflict="nothing")
        assert row is not None
        assert row2 is None
        assert inv.count() == 1
    ...
    def test_T91_refresh_token_rotation_lifecycle(self):
        ...
        def make_tok(fam):
            raw = uuid.uuid4().hex
            return {"token_hash":hashlib.sha256(raw.encode()).hexdigest(),"user_id":"u1","family_id":fam}
        t1 = make_tok(family); rt.insert(t1)
        rt.update({"token_hash":t1["token_hash"]},{"revoke_reason":"rotation"})
        t2 = make_tok(family); rt.insert(t2)
        assert rt.count() == 2
    ...
    def test_T95_signal_dedup(self):
        ...
        sig.insert({"user_id":"u1","symbol":"EURUSD","direction":"BUY","minute":minute_bucket})
        sigs = sig.select(user_id="u1",symbol="EURUSD",direction="BUY",minute=minute_bucket)
        assert len(sigs) == 1

    def test_T96_complete_saas_schema_tables(self):
        tables = {"licenses","license_devices","audit_log","refresh_tokens","nonce_store","billing_invoices","billing_subscriptions","execution_orders","signals"}
        assert len(tables) >= 9
