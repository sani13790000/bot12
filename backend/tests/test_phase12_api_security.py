...
class TestErrorCodes:
    ...
    def test_t02_all_codes_have_http_status(self):
        from backend.core.error_codes import EC, _CODE_TO_HTTP
    ...
    def test_t03_all_codes_have_message(self):
        from backend.core.error_codes import EC, _CODE_TO_MSG
        codes = [v for k, v in vars(EC).items() if not k.startswith("_")]
        for code in codes:
...
class TestPagination:
    ...
    def test_t17_has_more_true_when_full_page(self):
        from backend.core.pagination import OffsetPage, build_paged_response
        assert build_paged_response([{"id": str(i)} for i in range(50)], OffsetPage(50, 0))["has_more"] is True

    def test_t18_has_more_false_when_partial(self):
        from backend.core.pagination import OffsetPage, build_paged_response
        assert build_paged_response([{"id": str(i)} for i in range(30)], OffsetPage(50, 0))["has_more"] is False
...
class TestObjectAuth:
    ...
    def test_t32_support_blocked_on_write(self):
        ...
        with pytest.raises(HTTPException):
            check_resource_owner("user-456", self._user("sup-1", "support"), allow_admin=True, require_write_admin=True)
...
class TestCORSHardening:
    ...
    def test_t43_validate_origins_strips_whitespace(self):
        ...
        result = _validate_origins(["  http://localhost:3000  ", "http://localhost:8080"], _env="development")
...
class TestSecurityMiddleware:
    ...
    def test_t58_get_real_ip_no_trusted_proxy(self):
        from backend.middleware.security_hardened import get_real_ip, _TRUSTED_PROXY_NETS
    ...
    def test_t59_get_real_ip_with_trusted_proxy(self):
        from backend.middleware.security_hardened import get_real_ip, configure_trusted_proxies
    ...
    def test_t60_apply_headers_adds_request_id(self):
        ...
        assert headers_dict.get("x-request-id") == "test-rid-123" or headers_dict.get("X-Request-ID") == "test-rid-123"
...
class TestRateLimitV2:
    ...
    def test_t66_sliding_window_blocks_over_limit(self):
        from backend.middleware.rate_limit_v2 import _SlidingWindow
        w = _SlidingWindow()
        for _ in range(10): w.is_allowed("ip:x", 10, 60)
    ...
    def test_t69_endpoint_limits_default_for_unknown(self):
        from backend.middleware.rate_limit_v2 import _get_endpoint_limit, _DEFAULT_LIMIT
...
class TestRiskRouteHardening:
    ...
    def test_t77_assess_risk_max_risk_percent_5(self):
        ...
        with pytest.raises(ValidationError):
            RiskAssessRequest(signal_id="s1", symbol="EURUSD", direction="BUY", balance=10000, equity=10000, risk_percent=10.0)

    def test_t78_assess_risk_max_risk_percent_ok(self):
        from backend.api.routes.risk_v12 import RiskAssessRequest
        req = RiskAssessRequest(signal_id="s1", symbol="EURUSD", direction="BUY", balance=10000, equity=10000, risk_percent=5.0)
    ...
    def test_t80_risk_assess_open_positions_capped(self):
        ...
        from pydantic import ValidationError
        with pytest.raises(ValidationError):
            RiskAssessRequest(signal_id="s1", symbol="EURUSD", direction="BUY", balance=10000, equity=10000, open_positions=list(range(101)))
...
class TestExceptionHandlers:
    ...
    def test_t88_rate_limit_response_has_retry_after(self):
        ...
        for _ in range(10): w.is_allowed(key, 10, 60)
...
class TestIntegration:
    def test_t89_signals_route_uses_offset_pagination(self):
        ...
        assert "offset_pagination" in inspect.getsource(list_signals) or "OffsetPage" in inspect.getsource(list_signals)
    ...
    def test_t93_error_codes_importable(self):
        from backend.core.error_codes import EC, api_error, http_status
        assert callable(api_error) and callable(http_status)

    def test_t94_pagination_importable(self):
        from backend.core.pagination import OffsetPage, CursorPage, offset_pagination, build_paged_response
        assert OffsetPage

    def test_t95_object_auth_importable(self):
        from backend.core.object_auth import check_resource_owner, assert_owns, require_self_or_admin
        assert callable(check_resource_owner)

    def test_t96_security_middleware_importable(self):
        from backend.middleware.security_hardened import HardenedSecurityMiddleware, apply_security_middleware, get_real_ip, _scan_text
