...
for _m in [
    ...
    "prometheus_client",
    "opentelemetry",
    "backend.risk.fail_mode",
    "backend.risk.volatility_filter",
    "backend.risk.news_filter",
...
def test_T13_margin_allowed():
    assert _mg().can_trade is True


def test_T14_insufficient_margin():
    assert _mg(free_margin=100, required_margin=500).can_trade is False


def test_T15_low_margin_level():
    assert _mg(margin_level=120).can_trade is False


def test_T16_exact_border():
    assert _mg(free_margin=500, required_margin=500).can_trade is True


def test_T17_zero_required():
    assert _mg(required_margin=0).can_trade is True
...
def test_T19_fail_closed_default():
    assert MarginGate().FAIL_CLOSED == "FAIL_CLOSED"
...
def test_T21_latency_populated():
    assert _mg().latency_ms >= 0.0
...
def test_T23_neg_free_margin():
    assert _mg(free_margin=-50).can_trade is False


def test_T24_margin_in_result():
    assert hasattr(_mg(), "required_margin")
...
def test_T25_normal():
    assert _ks().state == SwitchState.ARMED


def test_T26_floor_trigger():
    assert _ks(floor_usd=10000).check(current_equity=5000, opening_equity=10000).triggered


def test_T27_drawdown_trigger():
    assert _ks().check(current_equity=8000, opening_equity=10000).triggered
...
def test_T31_no_trigger():
    assert not _ks().check(current_equity=9800, opening_equity=10000).triggered
...
def test_T36_lock_attr():
    assert hasattr(_ks(), "_lock")
...
# =============================================================================
def test_T45_kill_switch_error():
    assert isinstance(KillSwitchActivatedError("k"), Exception)


def test_T46_insufficient_margin_err():
    assert isinstance(InsufficientMarginError("m"), Exception)


def test_T47_drawdown_limit_error():
    assert isinstance(DrawdownLimitError("d"), Exception)


def test_T48_trading_halted_error():
    assert isinstance(TradingHaltedError("h"), Exception)


def test_T49_exceptions_importable():
    assert all(
        [KillSwitchActivatedError, InsufficientMarginError, DrawdownLimitError, TradingHaltedError]
    )


def test_T50_lotsizer_importable():
    assert LotSizer is not None


def test_T51_margin_gate_importable():
    assert MarginGate is not None


def test_T52_kill_switch_importable():
    assert KillSwitch is not None


def test_T53_daily_limits_importable():
    assert DailyLimitsEngine is not None


def test_T54_equity_prot_importable():
    assert EquityProtectionEngine is not None
...
def test_T55_daily_ok():
    assert _dl().check_limits(10000, TodayTrades()).can_trade is True


def test_T56_daily_loss():
    assert _dl().check_limits(10000, TodayTrades(pnl_usd=-350)).can_trade is False


def test_T57_daily_trades():
    assert _dl().check_limits(10000, TodayTrades(trade_count=10)).can_trade is False


def test_T58_weekly_loss():
    assert _dl().check_limits(10000, TodayTrades(), week_pnl_usd=-750).can_trade is False


def test_T59_monthly_dd():
    assert _dl().check_limits(10000, TodayTrades(), month_pnl_usd=-1600).can_trade is False
...
def test_T65_zero_balance():
    assert not _dl().check_limits(0, TodayTrades()).can_trade


def test_T66_defaults():
    assert DailyLimitsEngine()._max_daily_trades == 10
...
def test_T68_profit_no_block():
    assert _dl().check_limits(10000, TodayTrades(pnl_usd=500)).can_trade is True
