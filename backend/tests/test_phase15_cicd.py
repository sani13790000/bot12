...
from cicd_v15 import (
    ...
    SmokeTestClient,
    find_hardcoded_secrets_in_compose,
...
class TestDeployPipeline:
    ...
    def test_staging_has_smoke_test(self):
        assert has_smoke_test(CI_CD_OLD, "deploy-staging") or has_smoke_test(
            CI_CD_HARDENED, "deploy-staging"
        )
...
class TestPrometheusConfig:
    ...
    def test_alert_rules_has_drawdown(self):
        assert (
            "DrawdownCritical" in ALERT_RULES_TEMPLATE or "drawdown" in ALERT_RULES_TEMPLATE.lower()
        )
...
class TestArtifactRegistry:
    ...
    def test_get_existing(self):
        reg = self._reg()
        reg.register("ea", "3.20", b"data", DeployEnv.PRODUCTION)
    ...
    def test_entry_created_at(self):
        t0 = time.time()
        e = self._reg().register("ea", "3.20", b"d", DeployEnv.PRODUCTION)
