...
_tsm = types.ModuleType("backend")
_tsm.core = types.ModuleType("backend.core")
_lg = types.ModuleType("backend.core.logger")
_lg.get_logger = lambda n: _FakeLogger()
_tsm.core.logger = _lg
sys.modules["backend"] = _tsm
sys.modules["backend.core"] = _tsm.core
...
class MockEP:
    def __init__(self):
        ...
        self.state.high_water_mark = 0.0
        self.state.current_drawdown_percent = 5.0
        self._halt = None
        self._reset = False

    def initialize(self, b):
        self.is_initialized = True
        self.state.high_water_mark = b

    def _set_halt(self, r):
        self._halt = r
...
class TestU7AutoInit:
    ...
    def test_no_op_if_initialized(self):
        ep = MockEP()
        ep.is_initialized = True
        ep.state.high_water_mark = 9999
        epp.auto_init_guard(ep, 9500, 10000)
...
class TestU11Strip:
    ...
    def test_keeps_safe_fields(self):
        row = {k: "v" for k in _up._PROFILE_SAFE_FIELDS}
        row["password_hash"] = "x"
...
class TestU12UpdateProfile:
    def test_valid(self):
        r = _up.UpdateProfileRequest(full_name="Bob")
        assert r.full_name == "Bob"

    def test_strips_whitespace(self):
        r = _up.UpdateProfileRequest(full_name="  Al  ")
        assert r.full_name == "Al"

    def test_empty_rejected(self):
        with pytest.raises(Exception):
            _up.UpdateProfileRequest(full_name="")


class TestU14Settings:
    def test_valid_theme(self):
        r = _up.UserSettingsRequest(theme="dark")
        assert r.theme == "dark"

    def test_invalid_theme(self):
        with pytest.raises(Exception):
            _up.UserSettingsRequest(theme="pink")

    def test_risk_bounds(self):
        with pytest.raises(Exception):
            _up.UserSettingsRequest(default_risk_pct=0.0)
        with pytest.raises(Exception):
            _up.UserSettingsRequest(default_risk_pct=11.0)
...
class TestU16Cap:
    ...
    def test_invalid(self):
        with pytest.raises(ValueError):
            sp.validate_access_token_safe("bad.token", _SECRET)
...
class TestU17Revocation:
    def test_revoked_rejected(self):
        ...
        with pytest.raises(ValueError, match="revoked"):
            sp.validate_access_token_safe(t, _SECRET)
...
class TestU19Refresh:
    ...
    def test_access_rejected(self):
        t = sp.create_access_token_safe({"sub": "u1"}, _SECRET)
        with pytest.raises(ValueError, match="Not a refresh"):
            sp.validate_refresh_token_safe(t, _SECRET)
