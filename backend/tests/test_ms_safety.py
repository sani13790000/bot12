    ...
    async def run(self, context, timeout_s=None):
        if not self.enabled:
            return AgentResult(
                ...
                vote=AgentVote(
                    score=50.0, confidence=0.0, status=AgentStatus.SKIP, reason="disabled"
                ),
            )
        ...
        try:
            ...
            return AgentResult(
                agent_name=self.name, vote=vote, elapsed_ms=(time.perf_counter() - t0) * 1000
            )
...
class VoteResult:
    ...
    confidence: float
    direction: str
    agent_results: List[AgentResult] = _f(default_factory=list)
    blocked_by: Optional[str] = None
    reasons: List[str] = _f(default_factory=list)
    elapsed_ms: float = 0.0
...
class VotingEngine:
    ...
    async def vote(self, context):
        ...
        if rv is not None:
            rv.elapsed_ms = (time.perf_counter() - t0) * 1000
            return rv
        results = await (
            self._run_parallel_safe(context)
            if self._run_parallel
            else self._run_sequential_safe(context)
        )
        r = self._aggregate(results)
    ...
    async def _check_risk_veto(self, context):
        ...
        if r.vote.status == AgentStatus.ERROR and r.vote.score == 0.0:
            return VoteResult(
                ...
                reasons=[f"Risk veto: {r.vote.reason}"],
            )
        if r.error and "timeout" in r.error.lower():
            return VoteResult(
                ...
                agent_results=[r],
                blocked_by=r.agent_name,
                reasons=["Risk timeout"],
            )
        return None

    async def _run_with_timeout(self, agent, context):
        try:
            return await asyncio.wait_for(agent.run(context), timeout=self._agent_timeout_s)
        except asyncio.TimeoutError:
            return AgentResult(
    ...
    async def _run_parallel_safe(self, context):
        ...
        raw = await asyncio.gather(
            *[self._run_with_timeout(a, context) for a in nr], return_exceptions=True
        )
        return [
            AgentResult(
                agent_name=nr[i].name,
                vote=AgentVote(
                    score=50.0,
                    confidence=0.0,
                    status=AgentStatus.ERROR,
    ...
    async def _run_sequential_safe(self, context):
        return [
            await self._run_with_timeout(a, context)
            for a in self._agents
            if a.name != _RISK_AGENT_NAME
        ]
    ...
    def _aggregate(self, results):
        ...
        ws = 0.0
        wc = 0.0
        dw = {"BUY": 0.0, "SELL": 0.0, "NEUTRAL": 0.0}
        reasons = []
        ...
        for r in results:
            ...
            cm = max(0.0, r.vote.confidence / 100.0)
            ew = bw * cm
            if r.vote.status == AgentStatus.ERROR:
                ew *= 0.5
            ws += r.vote.score * ew
            wc += r.vote.confidence * ew
            tw += ew
        ...
        fc = max(0.0, min(100.0, fc))
        bw2 = dw.get("BUY", 0.0)
        sw2 = dw.get("SELL", 0.0)
        if abs(bw2 - sw2) <= _TIE_TOLERANCE and (bw2 + sw2) > 0:
            reasons.append("TIE")
            return VoteResult(
                ...
                metadata={
                    "tie_detected": True,
                    "buy_weight": round(bw2, 4),
                    "sell_weight": round(sw2, 4),
        ...
        if fs >= self._min_score_threshold and fc >= self._min_confidence_threshold:
            dec = (
                VoteDecision.BUY
                if top == "BUY"
                else (VoteDecision.SELL if top == "SELL" else VoteDecision.NO_TRADE)
            )
        else:
            dec = VoteDecision.NO_TRADE
        return VoteResult(
            decision=dec,
            weighted_score=fs,
            confidence=fc,
...
class SlowRisk(BaseAgent):
    ...
    async def analyze(self, c):
        ...
        return AgentVote(score=85.0, confidence=90.0, direction="BUY")


class BuyAgent(BaseAgent):
    def __init__(self, n, w=0.25):
        super().__init__(n, w)

    async def analyze(self, c):
        return AgentVote(
            score=80.0, confidence=75.0, direction="BUY", status=AgentStatus.OK, reason="BUY"
        )
...
class SellAgent(BaseAgent):
    ...
    async def analyze(self, c):
        return AgentVote(
            score=80.0, confidence=75.0, direction="SELL", status=AgentStatus.OK, reason="SELL"
        )


class Crasher(BaseAgent):
    def __init__(self, n):
        super().__init__(n, 0.20)
...
class SlowAgent(BaseAgent):
    ...
    async def analyze(self, c):
        await asyncio.sleep(self._d)
        return AgentVote(score=80.0, confidence=80.0, direction="BUY")


class HiConfBuy(BaseAgent):
    def __init__(self, n, w=0.40, conf=90.0):
        super().__init__(n, w)
        self._conf = conf

    async def analyze(self, c):
        return AgentVote(score=85.0, confidence=self._conf, direction="BUY", status=AgentStatus.OK)


class LoConfSell(BaseAgent):
    def __init__(self, n, w=0.40, conf=20.0):
        super().__init__(n, w)
        self._conf = conf

    async def analyze(self, c):
        return AgentVote(score=85.0, confidence=self._conf, direction="SELL", status=AgentStatus.OK)


class ZeroConf(BaseAgent):
    def __init__(self):
        super().__init__("ZeroConf", 0.40)

    async def analyze(self, c):
        return AgentVote(score=100.0, confidence=0.0, direction="SELL")
...
async def run_tests():
    ...
    r = await VotingEngine(
        [GoodRisk(), BuyAgent("A", 0.40), BuyAgent("B", 0.40)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
    ).vote(ctx)
    ...
    print("\n-- MS-2: Tie -> NO_TRADE")
    r = await VotingEngine(
        [GoodRisk(), BuyAgent("A", 0.30), SellAgent("B", 0.30)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
    ).vote({"direction": "BUY"})
    ...
    r = await VotingEngine(
        [GoodRisk(), BuyAgent("A", 0.40), SellAgent("B", 0.20)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
    ).vote(ctx)
    check("MS-2-B no tie", r.metadata.get("tie_detected") is False)
    r = await VotingEngine(
        [GoodRisk(), SellAgent("A", 0.40), SellAgent("B", 0.40)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
    ).vote({"direction": "SELL"})
    ...
    r = await VotingEngine(
        [GoodRisk(), HiConfBuy("Hi", 0.40, 90.0), LoConfSell("Lo", 0.40, 10.0)],
        min_score_threshold=60.0,
        min_confidence_threshold=30.0,
    ).vote(ctx)
    check(
        "MS-3-A hi-conf BUY wins",
        r.metadata.get("buy_weight", 0) > r.metadata.get("sell_weight", 0),
    )
    check("MS-3-A decision BUY", r.decision == VoteDecision.BUY, f"got {r.decision}")
    r = await VotingEngine(
        [GoodRisk(), HiConfBuy("Hi", 0.40, 90.0), ZeroConf()],
        min_score_threshold=60.0,
        min_confidence_threshold=30.0,
    ).vote(ctx)
    ...
    t0 = time.time()
    r = await VotingEngine(
        [GoodRisk(), BuyAgent("Fast", 0.40), SlowAgent("Slow", 10.0)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
        agent_timeout_s=0.1,
    ).vote(ctx)
    ...
    print("\n-- MS-5: Failover")
    r = await VotingEngine(
        [GoodRisk(), BuyAgent("S", 0.35), Crasher("C"), BuyAgent("S2", 0.25)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
        agent_timeout_s=2.0,
    ).vote(ctx)
    ...
    r = await VotingEngine(
        [GoodRisk(), Crasher("C1"), Crasher("C2")],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
        agent_timeout_s=2.0,
    ).vote(ctx)
    ...
    try:
        ...
        check("MS-5-C no propagation", False, str(e))
    print("\n-- Integration")
    r = await VotingEngine(
        [GoodRisk(), BuyAgent("A", 0.25), BuyAgent("B", 0.25), BuyAgent("C", 0.10)],
        min_score_threshold=65.0,
        min_confidence_threshold=50.0,
    ).vote(ctx)
