...
class _FakeMT5:
    def __init__(self, positions=None, order_result=None, account_balance=10_000.0):
        ...
        self._order_result = order_result or _FakeOrderResult(
            success=True, order=12345, price=1.0850, volume=0.01
        )
...
class _FakeOSM:
    ...
    async def transition(self, oid, state, **kw):
        pass
...
class TestFix1Idempotency:
    def _make_svc(self, positions=None, order_success=True):
        from execution.execution_service import (
            _IDEMPOTENCY_STORE,
            _IDEMPOTENCY_TIMESTAMPS,
            ExecutionService,
        )

        _IDEMPOTENCY_STORE.clear()
        _IDEMPOTENCY_TIMESTAMPS.clear()
        mt5 = _FakeMT5(
            positions=positions or [],
            order_result=_FakeOrderResult(
                success=order_success,
                order=9999 if order_success else 0,
                price=1.085 if order_success else 0,
                volume=0.01,
                error="" if order_success else "timeout",
                retcode=0 if order_success else 10006,
            ),
        )
        return ExecutionService(
            mt5=mt5, osm=_FakeOSM(), recovery=_FakeFR(), reconciliation=_FakePR(), risk=_FakeRisk()
        )
    ...
    async def test_concurrent_only_one_executes(self):
        ...
        _IDEMPOTENCY_STORE.clear()
        _IDEMPOTENCY_TIMESTAMPS.clear()
        _INFLIGHT_SIGNALS.clear()
        svc = self._make_svc()
        sig = {"signal_id": "race1", "symbol": "EURUSD", "action": "BUY"}
        r1, r2 = await asyncio.gather(
            svc.execute_signal(sig.copy()), svc.execute_signal(sig.copy())
        )
...
class TestFix2AsyncioQueue:
    ...
    async def test_50_concurrent_no_loss(self):
        ...
        tasks = [
            eng.handle_failure(
                order_id=f"o{i}", signal_id=f"s{i}", error="connection reset", retcode=10006
            )
            for i in range(50)
        ]
    ...
    async def test_queue_full_dead_letter(self):
        ...
        s = await eng.handle_failure(
            order_id="over", signal_id="s-over", error="timeout", retcode=10004
        )
    ...
    async def test_max_retries_dead_letter(self):
        ...
        eng.set_retry_callback(cb)
        await eng.start()
        await eng.handle_failure(
            order_id="o-max", signal_id="s-max", error="timeout", retcode=10004
        )
...
class TestFix4Rolling:
    ...
    def _filter(self):
        ...
        return CorrelationFilter(
            CorrelationFilterConfig(
                max_correlated_exposure=0.80, correlation_penalty_threshold=0.60, window=20
            )
        )
    ...
    async def test_high_corr_blocked(self):
        from risk.correlation_filter import CorrelationFilter, CorrelationFilterConfig, CorrPosition

        flt = CorrelationFilter(
            CorrelationFilterConfig(
                max_correlated_exposure=0.80, correlation_penalty_threshold=0.60
            )
        )
        pos = [CorrPosition("GBPUSD", "BUY", 1.0)]
        result = await flt.check("EURUSD", "BUY", pos, 1.0)
