...
from backend.core.supply_chain import (
    VulnSeverity, PinStatus, BuildStatus, DriftKind, ScanPattern,
    UNSAFE_PATTERNS, BANNED_PACKAGES,
    DepSpec, LockfileEntry, VulnRecord, BuildRecord, DynamicLoadViolation, DriftItem,
    LockfileIntegrity, DependencyPinner, VulnerabilityScanner,
    DriftDetector, DynamicLoadScanner, BuildSigner, BuildSignatureError,
    BuildReproducer, SupplyChainAuditChain, SupplyChainAdmin,
    SupplyChainSystem, _DEFAULT_SECRET,
)
...
def make_vuln(cve_id, package, version, severity=VulnSeverity.HIGH):
    return VulnRecord(
        cve_id=cve_id, package=package,
        affected_versions=[version], severity=severity,
...
class TestLockfileIntegrity:
    def setup_method(self):
        ...
        self.entries = [make_entry("fastapi", "0.115.0"), make_entry("pydantic", "2.9.2"), make_entry("redis", "5.1.1")]
    ...
    def test_T025_version_change_detected(self):
        self.lf.record(self.entries)
        assert self.lf.verify([make_entry("fastapi", "0.200.0"), make_entry("pydantic", "2.9.2"), make_entry("redis", "5.1.1")]) is False
    def test_T026_hash_change_detected(self):
        self.lf.record(self.entries)
        assert self.lf.verify([LockfileEntry("fastapi", "0.115.0", sha256="a" * 64), make_entry("pydantic", "2.9.2"), make_entry("redis", "5.1.1")]) is False
    ...
    def test_T028_concurrent_record_and_verify(self):
        errors = []
        def worker(i):
            try: self.lf.record([make_entry(f"pkg{i}", f"1.{i}.0")])
...
class TestDependencyPinner:
    def setup_method(self): self.pinner = DependencyPinner()
...
class TestVulnerabilityScanner:
    def setup_method(self):
        ...
        self.specs = [make_spec("fastapi", "0.100.0"), make_spec("pydantic", "1.9.0"), make_spec("redis", "4.0.0")]
        self.vulns = [make_vuln("CVE-2024-001", "fastapi", "0.100.0", VulnSeverity.CRITICAL), make_vuln("CVE-2024-002", "pydantic", "1.9.0", VulnSeverity.HIGH), make_vuln("CVE-2024-003", "redis", "3.5.0", VulnSeverity.MEDIUM)]
    ...
    def test_T065_case_insensitive_package_match(self):
        ...
        assert any(v.cve_id == "CVE-CI" for v in self.scanner.scan([make_spec("fastapi", "0.100.0")]))
    def test_T066_concurrent_scan_safe(self):
        self.scanner.load_db(self.vulns)
        errors = []
        def worker():
            try: self.scanner.scan(self.specs)
            except Exception as e: errors.append(e)
    ...
    def test_T068_multiple_vulns_same_package(self):
        self.scanner.load_db([make_vuln("CVE-A", "fastapi", "0.100.0", VulnSeverity.HIGH), make_vuln("CVE-B", "fastapi", "0.100.0", VulnSeverity.CRITICAL)])
...
class TestDriftDetector:
    ...
    def test_T070_version_drift_detected(self):
        c = dict(self.current); c["fastapi"] = "0.200.0"
    ...
    def test_T071_added_package_detected(self):
        c = dict(self.current); c["evil-pkg"] = "1.0.0"
    ...
    def test_T076_hash_drift_detected(self):
        items = self.detector.detect_hash_drift([LockfileEntry("fastapi", "0.115.0", sha256="a" * 64)], {"fastapi": "b" * 64})
        assert any(i.kind == DriftKind.HASH for i in items)
    def test_T077_hash_drift_clean(self):
        h = "a" * 64
        assert self.detector.detect_hash_drift([LockfileEntry("fastapi", "0.115.0", sha256=h)], {"fastapi": h}) == []
    def test_T078_hash_drift_missing_actual_ok(self):
        assert self.detector.detect_hash_drift([LockfileEntry("fastapi", "0.115.0", sha256="a" * 64)], {}) == []
    ...
    def test_T081_multiple_drift_types(self):
        kinds = {i.kind for i in self.detector.detect(self.lockfile, {"fastapi": "0.200.0", "newpkg": "1.0.0"})}
    ...
    def test_T084_supply_chain_attack_hash_mismatch(self):
        items = self.detector.detect_hash_drift([LockfileEntry("requests", "2.28.0", sha256="legit" + "a" * 59)], {"requests": "evil" + "b" * 60})
        assert any(i.kind == DriftKind.HASH for i in items)


class TestDynamicLoadScanner:
    def setup_method(self): self.scanner = DynamicLoadScanner()
    def test_T085_detect_exec(self):
        assert any(v.pattern == "exec(" for v in self.scanner.scan_source('exec("import os")', "test.py"))
    def test_T086_detect_eval(self):
        assert any(v.pattern == "eval(" for v in self.scanner.scan_source('result = eval(user_input)', "test.py"))
    def test_T087_detect_pickle_loads(self):
        assert any(v.pattern == "pickle.loads" for v in self.scanner.scan_source('obj = pickle.loads(data)', "test.py"))
    def test_T088_detect_dynamic_import(self):
        assert any("importlib" in v.pattern for v in self.scanner.scan_source('mod = importlib.import_module(name)', "test.py"))
    ...
    def test_T090_clean_source_no_violations(self):
        assert self.scanner.scan_source('import fastapi\nfrom pydantic import BaseModel\n', "clean.py") == []
    ...
    def test_T092_violation_severity_critical_for_exec(self):
        viols = [v for v in self.scanner.scan_source('exec(code)', "test.py") if v.pattern == "exec("]
        assert viols[0].severity == VulnSeverity.CRITICAL
    def test_T093_violation_severity_high_for_subprocess(self):
        viols = [v for v in self.scanner.scan_source('p = subprocess.Popen(cmd)', "test.py") if v.pattern == "subprocess.Popen"]
        assert viols[0].severity == VulnSeverity.HIGH
    def test_T094_scan_files_multiple(self):
        assert len(self.scanner.scan_files({"a.py": "eval(x)", "b.py": "exec(y)", "c.py": "import os"})) >= 2
    ...
    def test_T097_summary_keys(self):
        assert all(sev.value in self.scanner.summary({"test.py": "eval(x)"}) for sev in VulnSeverity)
    def test_T098_custom_patterns(self):
        assert len(DynamicLoadScanner(patterns=["DANGER"]).scan_source('x = DANGER("do it")', "test.py")) >= 1
    ...
    def test_T100_marshal_loads_detected(self):
        assert any("marshal" in v.pattern for v in self.scanner.scan_source("code = marshal.loads(data)", "test.py"))


class TestBuildSigner:
    def setup_method(self):
        self.signer = BuildSigner(b"test-secret-phase28")
        self.record = BuildRecord(commit_sha="abc123", branch="main", python_ver="3.11", lockfile_hash="a" * 64, deps_count=42, built_by="ci", artifact_ids=["art-1", "art-2"])
    ...
    def test_T104_verify_tampered_false(self):
        r = self.signer.sign(self.record); r.commit_sha = "evil123"
    ...
    def test_T107_verify_or_raise_raises(self):
        r = self.signer.sign(self.record); r.branch = "evil"
        with pytest.raises(BuildSignatureError): self.signer.verify_or_raise(r)
    ...
    def test_T109_string_secret_accepted(self):
        ...
        r = copy.copy(self.record); signer.sign(r)
    ...
    def test_T111_artifact_ids_order_independent(self):
        r1 = copy.copy(self.record); r1.artifact_ids = ["art-2", "art-1"]
        r2 = copy.copy(self.record); r2.artifact_ids = ["art-1", "art-2"]
        self.signer.sign(r1); self.signer.sign(r2)
        assert r1.signature == r2.signature
    def test_T112_lockfile_hash_tamper_detected(self):
        r = self.signer.sign(self.record); r.lockfile_hash = "b" * 64
        assert self.signer.verify(r) is False
...
class TestBuildReproducer:
    ...
    def _build(self, **kw):
        kw.setdefault("commit_sha", "abc"); kw.setdefault("branch", "main")
        kw.setdefault("python_ver", "3.11"); kw.setdefault("lockfile_entries", self.entries)
        kw.setdefault("artifact_ids", []); kw.setdefault("built_by", "ci")
    ...
    def test_T119_is_reproducible_false_tampered_lockfile(self):
        b = self._build()
        assert self.reproducer.is_reproducible(b.build_id, [make_entry("evil", "0.0.1")], "abc", "3.11") is False
    def test_T120_list_builds_sorted(self):
        for i in range(3): self._build()
        builds = self.reproducer.list_builds()
        assert len(builds) >= 3
        for i in range(len(builds)-1): assert builds[i].built_at >= builds[i+1].built_at
    ...
    def test_T123_different_env_different_hash(self):
        b1 = self._build(env={"ENV": "prod"}); b2 = self._build(env={"ENV": "staging"})
...
class TestSupplyChainAuditChain:
    def setup_method(self): self.chain = SupplyChainAuditChain(b"test-secret")
...
class TestSupplyChainAdmin:
    ...
    def test_T149_generate_sbom_structure(self):
        sbom = self.sys.admin.generate_sbom(self.sys.parse_requirements(PINNED_REQS), self.entries, "build-1")
        assert "components" in sbom and sbom["sbom_version"] == "1.0"
    def test_T150_generate_sbom_sha256_present(self):
        sbom = self.sys.admin.generate_sbom(self.sys.parse_requirements(PINNED_REQS), self.entries, "b1")
    ...
    def test_T154_policy_gate_blocks_critical_vuln(self):
        passed, reasons = self.sys.admin.policy_gate("fastapi==0.100.0\n", [make_vuln("CVE-CRIT", "fastapi", "0.100.0", VulnSeverity.CRITICAL)], {}, block_on_critical_vuln=True)
        assert not passed and any("critical" in r for r in reasons)
    def test_T155_policy_gate_blocks_dynamic_load(self):
        passed, reasons = self.sys.admin.policy_gate(PINNED_REQS, [], {"bad.py": "eval(x)"}, block_on_dynamic_load=True)
    ...
    def test_T158_full_scan_detects_banned(self):
        assert self.sys.admin.full_scan(BANNED_REQS, self.entries, self.env, {})["banned_count"] >= 1
    ...
    def test_T160_sbom_build_id_present(self):
        assert self.sys.admin.generate_sbom(self.sys.parse_requirements(PINNED_REQS), self.entries, "build-999")["build_id"] == "build-999"
    def test_T161_sbom_dev_flag(self):
        assert self.sys.admin.generate_sbom([DepSpec("pytest", "8.3.3", pin_op="==", is_dev=True)], [], "b")["components"][0]["is_dev"] is True
    def test_T162_policy_gate_no_block_flags(self):
        passed, _ = self.sys.admin.policy_gate(UNPINNED_REQS, [], {"bad.py": "eval(x)"}, block_on_unpinned=False, block_on_dynamic_load=False, block_on_critical_vuln=False, block_on_banned=False)
        assert passed is True
    def test_T163_full_scan_drift_count(self):
        r = self.sys.admin.full_scan(PINNED_REQS, self.entries, {"fastapi": "0.200.0", "pydantic": "2.9.2"}, {})
...
class TestSupplyChainSystemIntegration:
    ...
    def test_T166_e2e_attack_scenario(self):
        ...
        tampered = [LockfileEntry("fastapi", "0.115.0", sha256="evil" + "a" * 60)] + self.entries[1:]
        assert self.sys.lockfile.verify(tampered) is False
        assert any(i.kind == DriftKind.HASH for i in self.sys.drift.detect_hash_drift(self.entries, {"fastapi": "evil" + "a" * 60}))
    def test_T167_e2e_dynamic_load_blocked(self):
        passed, _ = self.sys.admin.policy_gate(PINNED_REQS, [], {"backdoor.py": 'exec(open("cmd.sh").read())'}, block_on_dynamic_load=True)
        assert passed is False
    def test_T168_e2e_sbom_and_audit_chain(self):
        sbom = self.sys.admin.generate_sbom(self.sys.parse_requirements(SAMPLE_REQS), self.entries, "build-2")
    ...
    def test_T170_audit_chain_100_ops(self):
        for i in range(100): self.sys.audit.record(f"op{i}", "ci", {"i": i})
        assert self.sys.audit.verify_chain() is True
    def test_T171_concurrent_full_scans(self):
        errors = []
        def worker():
            try: self.sys.admin.full_scan(PINNED_REQS, self.entries, {"fastapi": "0.115.0", "pydantic": "2.9.2"}, {})
            except Exception as e: errors.append(e)
        threads = [threading.Thread(target=worker) for _ in range(20)]
        for t in threads: t.start()
        for t in threads: t.join()
    ...
    def test_T173_build_signature_tamper_detected(self):
        b = self._build(); b.branch = "evil-branch"
        assert not self.sys.signer.verify(b)
    def test_T174_full_scan_returns_total_deps(self):
        assert self.sys.admin.full_scan(SAMPLE_REQS, self.entries, {"fastapi": "0.115.0", "pydantic": "2.9.2"}, {})["total_deps"] == 5
    def test_T175_policy_gate_all_reasons_listed(self):
        passed, reasons = self.sys.admin.policy_gate(UNPINNED_REQS, [make_vuln("CVE-X", "fastapi", "0.100.0", VulnSeverity.CRITICAL)], {"bad.py": "eval(x)"}, block_on_unpinned=True, block_on_dynamic_load=True, block_on_critical_vuln=True)
        assert not passed and len(reasons) >= 2
    def test_T176_system_audit_chain_isolated(self):
        s1 = SupplyChainSystem(b"secret1"); s2 = SupplyChainSystem(b"secret2")
...
def sql_content():
    if not os.path.exists(SQL_PATH): pytest.skip("migration_037.sql not found")
    with open(SQL_PATH) as f: return f.read()


class TestSQLMigration:
    def test_T177_sql_file_exists(self, sql_content): assert len(sql_content) > 100
    def test_T178_begins_with_begin(self, sql_content): assert "BEGIN" in sql_content.upper()
    def test_T179_supply_chain_runs_table(self, sql_content): assert "supply_chain_runs" in sql_content
    def test_T180_lockfile_records_table(self, sql_content): assert "lockfile_records" in sql_content
    def test_T181_build_records_table(self, sql_content): assert "build_records" in sql_content
    def test_T182_vuln_scan_results_table(self, sql_content): assert "vuln_scan_results" in sql_content
    def test_T183_dynamic_load_violations_table(self, sql_content): assert "dynamic_load_violations" in sql_content
    def test_T184_rls_enabled(self, sql_content): assert "ROW LEVEL SECURITY" in sql_content.upper() or "ENABLE ROW" in sql_content.upper()
    def test_T185_tenant_id_column(self, sql_content): assert "tenant_id" in sql_content
    def test_T186_chain_hash_column(self, sql_content): assert "chain_hash" in sql_content
    def test_T187_indexes_present(self, sql_content): assert "CREATE INDEX" in sql_content.upper()
    def test_T188_immutable_trigger(self, sql_content): assert "TRIGGER" in sql_content.upper()
    def test_T189_cleanup_function(self, sql_content): assert "cleanup" in sql_content.lower() or "FUNCTION" in sql_content.upper()
    def test_T190_commit_present(self, sql_content): assert "COMMIT" in sql_content.upper()
    def test_T191_if_not_exists(self, sql_content): assert "IF NOT EXISTS" in sql_content.upper()
    def test_T192_supply_chain_audit_log_table(self, sql_content): assert "supply_chain_audit" in sql_content
...
class TestEdgeCasesAndCoverage:
    ...
    def test_T203_system_components_not_none(self):
        s = SupplyChainSystem()
        assert all(getattr(s, attr) is not None for attr in ["pinner", "scanner", "drift", "dyn", "lockfile", "signer", "builds", "audit", "admin"])
    def test_T204_banned_packages_all_lowercase_or_hyphen(self):
        for b in BANNED_PACKAGES: assert b == b.lower() or "-" in b
