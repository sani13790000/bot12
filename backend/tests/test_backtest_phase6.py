...
@pytest.fixture
def backtest_engine():
    from backend.research.backtest.engine import BacktestConfig, BacktestEngine
    ...
    def test_signal_cannot_see_future_close(self, backtest_engine, sample_candles):
        ...
        for idx, last_close in enumerate(received_last_closes):
            ...
            assert last_close == expected_close, (
                f"Lookahead bias at position {idx + 1}: got {last_close}, expected {expected_close}"
            )
...
class TestParameterRange:
    ...
    def test_parameter_range_in_optimization_config(self):
        from backend.backtest_engine.parameter_optimizer import OptimizationConfig, ParameterRange
