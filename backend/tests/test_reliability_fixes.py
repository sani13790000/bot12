...
class _FakeMT5:
    ...
    def login(self, *a, **kw) -> bool:
        return self._login_ok

    def terminal_info(self) -> Any:
        m = MagicMock()
        m.connected = self._terminal_connected
        return m

    def account_info(self) -> Any:
        return MagicMock(balance=10000.0)

    def positions_get(self, **kw) -> List:
        return []
    ...
    def order_send(self, req) -> Any:
        m = MagicMock()
        m.retcode = self.TRADE_RETCODE_DONE
        m.deal = 1
        m.order = 1
...
class TestR3DynamicSlippage(unittest.TestCase):
    def test_base_when_no_inputs(self):
        self.assertEqual(_slippage(), 10)

    def test_atr_normal_no_change(self):
        self.assertEqual(_slippage(current_atr=0.0012, avg_atr=0.0012), 10)

    def test_atr_2x_increases_deviation(self):
        self.assertEqual(_slippage(current_atr=0.0024, avg_atr=0.0012), 30)

    def test_spread_spike_increases(self):
        self.assertEqual(_slippage(current_spread=30.0, avg_spread=10.0), 40)

    def test_volatility_high_flag(self):
        self.assertEqual(_slippage(volatility_high=True), 20)

    def test_combined_capped_at_max(self):
        self.assertEqual(
            _slippage(
                current_atr=0.006,
                avg_atr=0.0012,
    ...
    def test_zero_avg_atr_falls_back(self):
        self.assertEqual(_slippage(current_atr=0.0012, avg_atr=0.0), 10)

    def test_lower_atr_no_decrease(self):
        self.assertEqual(_slippage(current_atr=0.0006, avg_atr=0.0012), 10)
...
class TestR4ReconciliationInterval(unittest.TestCase):
    def test_default_interval_is_10(self):
        ...
        os.environ.pop("RECONCILE_INTERVAL_SECONDS", None)
        self.assertEqual(int(os.environ.get("RECONCILE_INTERVAL_SECONDS", "10")), 10)

    def test_env_override(self):
        import os

        os.environ["RECONCILE_INTERVAL_SECONDS"] = "20"
        try:
            self.assertEqual(int(os.environ.get("RECONCILE_INTERVAL_SECONDS", "10")), 20)
        finally:
            os.environ.pop("RECONCILE_INTERVAL_SECONDS", None)

    def test_clamp_below_min(self):
        def clamp(s):
            return max(5, min(s, 300))
    ...
    def test_clamp_above_max(self):
        ...
        self.assertEqual(clamp(999), 300)
        self.assertEqual(clamp(301), 300)
    ...
    def test_valid_interval_unchanged(self):
        ...
        self.assertEqual(clamp(30), 30)

    def test_set_interval_runtime(self):
        current = [10]

        def set_interval(s):
            current[0] = max(5, min(s, 300))

        set_interval(30)
        self.assertEqual(current[0], 30)
...
class TestR5SemiAutoMemoryLeak(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_waiting_not_evicted(self):
        ...
        for k in [k for k in list(pending) if pending[k].status in terminal]:
            pending.pop(k)
        self.assertIn(s.signal_id, pending)

    async def test_callbacks_cleared(self):
        cbs = {"s1": lambda x: None}
        cbs.pop("s1", None)
    ...
    async def test_pending_count_decreases(self):
        ...
        for i, s in enumerate(sigs):
            if i < 8:
                s.status = St.APPROVED if i < 5 else St.REJECTED
        terminal = {St.APPROVED, St.REJECTED, St.EXPIRED, St.EXECUTED}
        for sid in [s.signal_id for s in sigs if s.status in terminal]:
