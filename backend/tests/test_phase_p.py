...
class TestWeightAdjusterNumpyFix:
    ...
    def test_normalize_sums_to_one(self):
        from backend.intelligence.weight_adjuster import IndicatorWeights
        w = IndicatorWeights(
            smc_weight=np.float64(1.0), price_action_weight=np.float64(1.0),
            htf_alignment_weight=np.float64(1.0), session_weight=np.float64(1.0),
            ltf_weight=np.float64(1.0),
        ).normalize()
        total = w.smc_weight + w.price_action_weight + w.htf_alignment_weight + w.session_weight + w.ltf_weight
        assert abs(total - 1.0) < 1e-9

    def test_apply_delta_clamped(self):
        from backend.intelligence.weight_adjuster import IndicatorWeights, _MAX_DELTA_PER_CYCLE
        w = IndicatorWeights()
    ...
    def test_min_weight_enforced(self):
        from backend.intelligence.weight_adjuster import IndicatorWeights, _MIN_WEIGHT
        w = IndicatorWeights()
...
class TestSignalServiceDBFilters:
    ...
    def test_direction_passed_to_db(self):
        rows = [{"id": "1", "symbol": "EURUSD", "direction": "BUY", "score": 70, "expires_at": None}]
        svc, mock_db = self._make_service(rows)
    ...
    def test_min_score_filters_results(self):
        rows = [{"id": "1", "score": 40, "expires_at": None}, {"id": "2", "score": 80, "expires_at": None}]
        svc, mock_db = self._make_service(rows)
...
class TestPortfolioRiskRollingCorrelation:
    def _make_risk(self, sym, direction, entry, sl, balance=10000.0):
        ...
        return OpenTradeRisk(
            symbol=sym, direction=TradeDirection(direction),
            lot_size=0.1, entry_price=entry, stop_loss=sl, account_balance=balance,
        )
...
class TestCircuitBreakerAsyncLock:
    ...
    def test_closed_to_open_transition(self):
        from backend.circuit_breaker import CircuitBreaker, BreakerConfig, State
        async def _test():
    ...
    def test_registry_cap(self):
        from backend.circuit_breaker import get_breaker, _MAX_REGISTRY_SIZE, _registry
        async def _test():
            _registry.clear()
    ...
    def test_half_open_timeout_resets(self):
        from backend.circuit_breaker import CircuitBreaker, BreakerConfig, State, _HALF_OPEN_TIMEOUT_S
        async def _test():
...
class TestPhasePIntegration:
    def test_validate_pipeline(self):
        from backend.core.validators import validate_symbol, validate_lot_size, validate_direction
        assert validate_symbol("xauusd") == "XAUUSD"
    ...
    def test_weight_adjuster_full_cycle(self):
        import tempfile, os
        from backend.intelligence.weight_adjuster import IndicatorWeights
        w = IndicatorWeights(smc_weight=np.float64(0.5), price_action_weight=np.float64(0.3)).normalize()
        ...
        total = w2.smc_weight + w2.price_action_weight + w2.htf_alignment_weight + w2.session_weight + w2.ltf_weight
        assert abs(total - 1.0) < 1e-9
