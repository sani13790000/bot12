...
"""
import copy, hashlib, hmac, json, time, uuid
...
from core.compliance import (
    DocumentType, DocumentStatus, ConsentStatus, JurisdictionCode,
    AuditAction, SAAS_REQUIRED_DOCS, ACCEPTANCE_REQUIRED, REQUIRES_REASON,
    GENESIS_CONST,
    ComplianceError, MissingReasonError, DocumentNotFoundError,
    ConsentRequiredError, ConsentDeclinedError, RefundDeniedError, CancellationError,
    DocumentVersion, ConsentRecord, RetentionRule, RefundRequest, CancellationRequest,
    ComplianceAuditChain, DocumentStore, ConsentStore,
    DisclosureEngine, RetentionPolicyEngine, DEFAULT_RETENTION_RULES,
    RefundPolicyEngine, RefundPolicyConfig,
    CancellationPolicyEngine, CancellationPolicyConfig,
    SaaSReadinessChecker, SaaSReadinessReport,
    ComplianceAdmin, LegalDocumentFactory, build_compliance_system,
)


def make_doc(doc_type=DocumentType.TOS, version="1.0.0", status=DocumentStatus.ACTIVE,
             jurisdiction=JurisdictionCode.GLOBAL, effective_date=None,
             content="Test legal content.", actor="admin") -> DocumentVersion:
    return DocumentVersion(
        doc_id=str(uuid.uuid4()), doc_type=doc_type, version=version,
        title=f"Test {doc_type.value} v{version}",
        content=content, content_hash=hashlib.sha256(content.encode()).hexdigest(),
        status=status, effective_date=effective_date or time.time(),
        jurisdiction=jurisdiction, created_by=actor)


def make_consent(user_id="user1", tenant_id="t1", doc_type=DocumentType.TOS,
                 doc_id=None, status=ConsentStatus.ACCEPTED, expires_at=None) -> ConsentRecord:
    return ConsentRecord(
        consent_id=str(uuid.uuid4()), user_id=user_id, tenant_id=tenant_id,
        doc_id=doc_id or str(uuid.uuid4()), doc_type=doc_type, doc_version="1.0.0",
        status=status, ip_address="1.2.3.4", user_agent="TestAgent/1.0",
        accepted_at=time.time(), expires_at=expires_at)
...
class TestDocumentTypeEnums:
    def test_T001_all_10_doc_types_exist(self):
        ...
        for t in ["tos","privacy","risk_disclaimer","license_terms","refund_policy",
                  "retention_policy","cancellation_policy","data_processing_agreement",
                  "cookie_policy","aml_kyc_policy"]:
            assert t in types
        assert len(types) == 10
    ...
    def test_T004_requires_reason_actions(self):
        for a in [AuditAction.DOC_SUPERSEDED, AuditAction.DOC_ARCHIVED,
...
class TestComplianceAuditChain:
    ...
    def test_T021_verify_chain_multiple_entries(self):
        c = ComplianceAuditChain()
        for _ in range(10): c.record(AuditAction.CONSENT_RECORDED, actor="u")
        assert c.verify_chain() is True
    ...
    def test_T029_query_by_action(self):
        ...
        assert all(e.action == AuditAction.DOC_PUBLISHED for e in c.query(action=AuditAction.DOC_PUBLISHED))
    ...
    def test_T031_query_limit(self):
        c = ComplianceAuditChain()
        for _ in range(20): c.record(AuditAction.DOC_PUBLISHED, actor="a")
        assert len(c.query(limit=5)) == 5

    def test_T032_thread_safe_100_concurrent(self):
        c = ComplianceAuditChain(); errors = []
        def w():
            try: c.record(AuditAction.CONSENT_RECORDED, actor="u")
...
class TestDocumentStore:
    ...
    def test_T045_supersede_requires_reason(self):
        s = DocumentStore(); o = make_doc(); n = make_doc(); s.add(o); s.add(n)
        with pytest.raises(MissingReasonError): s.supersede(o.doc_id, n.doc_id, "a", "")
    ...
    def test_T047_archive_requires_reason(self):
        s = DocumentStore(); d = make_doc(); s.add(d)
        with pytest.raises(MissingReasonError): s.archive(d.doc_id, "a", "")
    ...
    def test_T049_count(self):
        s = DocumentStore(); assert s.count() == 0; s.add(make_doc()); assert s.count() == 1

    def test_T050_thread_safe_add(self):
        s = DocumentStore(); docs = [make_doc() for _ in range(50)]
    ...
    def test_T051_jurisdiction_global_always_matches(self):
        s = DocumentStore(); s.add(make_doc(jurisdiction=JurisdictionCode.GLOBAL))
    ...
    def test_T052_jurisdiction_specific_matches(self):
        s = DocumentStore(); s.add(make_doc(jurisdiction=JurisdictionCode.EU))
        assert s.active_for_type(DocumentType.TOS, JurisdictionCode.EU) is not None

    def test_T053_supersede_missing_doc_raises(self):
        with pytest.raises(DocumentNotFoundError): DocumentStore().supersede("x","y","a","r")

    def test_T054_archive_missing_doc_raises(self):
        with pytest.raises(DocumentNotFoundError): DocumentStore().archive("x","a","r")
...
class TestConsentStore:
    def test_T057_record_and_get(self):
        s = ConsentStore(); c = make_consent(); s.record(c)
        assert s.get(c.consent_id) is not None

    def test_T058_latest_for_returns_most_recent(self):
        s = ConsentStore()
        c1 = make_consent(user_id="u1"); c1.accepted_at = time.time()-100
        c2 = make_consent(user_id="u1"); c2.accepted_at = time.time()
        s.record(c1); s.record(c2)
        assert s.latest_for("u1", DocumentType.TOS).consent_id == c2.consent_id

    def test_T059_user_has_accepted_true(self):
        s = ConsentStore(); s.record(make_consent(user_id="u1", status=ConsentStatus.ACCEPTED))
    ...
    def test_T061_user_has_accepted_false_if_declined(self):
        s = ConsentStore(); s.record(make_consent(status=ConsentStatus.DECLINED))
        assert s.user_has_accepted("user1", DocumentType.TOS) is False

    def test_T062_withdraw_requires_reason(self):
        s = ConsentStore(); c = make_consent(); s.record(c)
        with pytest.raises(MissingReasonError): s.withdraw(c.consent_id, "", "a")
    ...
    def test_T064_withdraw_missing_raises(self):
        with pytest.raises(ComplianceError): ConsentStore().withdraw("x", "r", "a")

    def test_T065_pending_for_user_all_accepted(self):
        assert full_consent_store().pending_for_user("user1","t1",ACCEPTANCE_REQUIRED,full_doc_store()) == []

    def test_T066_pending_for_user_missing_consent(self):
        assert len(ConsentStore().pending_for_user("u","t",ACCEPTANCE_REQUIRED,full_doc_store())) == len(ACCEPTANCE_REQUIRED)

    def test_T067_tenant_isolation(self):
        s = ConsentStore(); s.record(make_consent(user_id="u1", tenant_id="t1"))
    ...
    def test_T068_count(self):
        s = ConsentStore(); assert s.count() == 0; s.record(make_consent()); assert s.count() == 1

    def test_T069_thread_safe_record(self):
        s = ConsentStore(); cs = [make_consent() for _ in range(50)]
    ...
    def test_T071_expired_consent_not_valid(self):
        s = ConsentStore(); s.record(make_consent(expires_at=time.time()-1))
        assert s.user_has_accepted("user1", DocumentType.TOS) is False
    ...
    def test_T073_multiple_doc_types_independent(self):
        s = ConsentStore(); s.record(make_consent(doc_type=DocumentType.TOS))
    ...
    def test_T074_pending_ignores_no_active_doc(self):
        ds = DocumentStore(); ds.add(make_doc(doc_type=DocumentType.TOS))
        pending = ConsentStore().pending_for_user("u","t",{DocumentType.TOS,DocumentType.PRIVACY},ds)
...
class TestDisclosureEngine:
    def _setup(self, grace_days=7):
        audit = ComplianceAuditChain(); ds = full_doc_store(); cs = ConsentStore()
    ...
    def test_T077_fully_accepted_user_allowed(self):
        ...
        for dt in ACCEPTANCE_REQUIRED: cs.record(make_consent(user_id="u1", doc_type=dt))
    ...
    def test_T079_old_user_missing_consent_blocked(self):
        ...
        with pytest.raises(ConsentRequiredError): e.check_access("u","t",time.time()-9999)

    def test_T080_grace_period_expired_blocked(self):
        e, ds, cs, _ = self._setup(grace_days=0)
        with pytest.raises(ConsentRequiredError): e.check_access("u","t",time.time()-1)
    ...
    def test_T083_record_consent_no_active_doc_raises(self):
        ...
        with pytest.raises(DocumentNotFoundError): e.record_consent("u","t",DocumentType.TOS,"1","A")
    ...
    def test_T085_block_audited(self):
        ...
        try: e.check_access("u","t",time.time()-9999)
        except ConsentRequiredError: pass
        assert len(audit.query(action=AuditAction.DISCLOSURE_BLOCK)) == 1

    def test_T086_hook_called_on_block(self):
        e, ds, cs, _ = self._setup(grace_days=0); hooked = []
        e.add_hook(lambda u,t,m: hooked.append(u))
        try: e.check_access("u1","t",time.time()-9999)
        except ConsentRequiredError: pass
        assert "u1" in hooked
    ...
    def test_T088_fully_accepted_audited_as_allowed(self):
        e, ds, cs, audit = self._setup()
        for dt in ACCEPTANCE_REQUIRED: cs.record(make_consent(user_id="u", doc_type=dt))
    ...
    def test_T089_check_access_missing_list_contains_types(self):
        ...
        for m in missing: assert isinstance(m, DocumentType)

    def test_T090_multiple_users_isolated(self):
        e, ds, cs, _ = self._setup()
        for dt in ACCEPTANCE_REQUIRED: cs.record(make_consent(user_id="u1",tenant_id="t",doc_type=dt))
        allowed, _ = e.check_access("u1","t",time.time()-86400)
        assert allowed is True
        with pytest.raises(ConsentRequiredError): e.check_access("u2","t",time.time()-9999999)
    ...
    def test_T094_grace_period_exactly_at_boundary(self):
        ...
        with pytest.raises(ConsentRequiredError): e.check_access("u","t",time.time()-86401)

    def test_T095_consent_required_error_lists_missing(self):
        e, ds, cs, _ = self._setup(grace_days=0)
        try: e.check_access("u1","t",time.time()-9999999)
        except ConsentRequiredError as ex: assert "u1" in str(ex)
...
class TestRetentionPolicyEngine:
    def test_T097_default_rules_loaded(self):
        ...
        for cat in ["user_pii","trading_logs","audit_logs"]: assert e.get(cat) is not None
    ...
    def test_T105_set_rule_audited(self):
        audit = ComplianceAuditChain(); e = RetentionPolicyEngine(audit=audit)
    ...
    def test_T107_default_rules_immutable_on_init(self):
        e1 = RetentionPolicyEngine(); e2 = RetentionPolicyEngine()
...
class TestRefundPolicyEngine:
    def _e(self, **kw):
        ...
        return RefundPolicyEngine(config=RefundPolicyConfig(**kw) if kw else RefundPolicyConfig(), audit=audit), audit

    def test_T113_request_refund_creates_record(self):
        e, _ = self._e(); req = e.request_refund("u","t",5000,"USD","reason",time.time())
        assert req.request_id != "" and req.status == "pending"

    def test_T114_request_refund_requires_reason(self):
        e, _ = self._e()
        with pytest.raises(MissingReasonError): e.request_refund("u","t",5000,"USD","",time.time())
    ...
    def test_T116_evaluate_outside_window_denied(self):
        ...
        with pytest.raises(RefundDeniedError): e.evaluate(req.request_id,"admin")
    ...
    def test_T119_deny_audited(self):
        ...
        try: e.evaluate(req.request_id,"admin")
        except RefundDeniedError: pass
        assert len(audit.query(action=AuditAction.REFUND_DENIED)) == 1

    def test_T120_get_returns_request(self):
        e, _ = self._e(); req = e.request_refund("u","t",500,"USD","r",time.time())
    ...
    def test_T122_missing_request_raises(self):
        e, _ = self._e()
        with pytest.raises(ComplianceError): e.evaluate("x","admin")

    def test_T123_default_window_14_days(self):
        e, _ = self._e(); assert e._config.window_days == 14
    ...
    def test_T126_whitespace_reason_raises(self):
        e, _ = self._e()
        with pytest.raises(MissingReasonError): e.request_refund("u","t",500,"USD","   ",time.time())
...
class TestCancellationPolicyEngine:
    def _e(self, **kw):
        audit = ComplianceAuditChain()
        return CancellationPolicyEngine(config=CancellationPolicyConfig(**kw) if kw else CancellationPolicyConfig(), audit=audit), audit

    def test_T129_request_cancellation_creates_record(self):
        e, _ = self._e(); req = e.request_cancellation("u","t","switching")
        assert req.request_id != "" and req.status == "pending"

    def test_T130_request_requires_reason(self):
        e, _ = self._e()
        with pytest.raises(MissingReasonError): e.request_cancellation("u","t","")
    ...
    def test_T132_immediate_cancellation_blocked_by_default(self):
        ...
        with pytest.raises(CancellationError): e.request_cancellation("u","t","r",immediate=True)
    ...
    def test_T136_double_confirm_raises(self):
        e, _ = self._e(); req = e.request_cancellation("u","t","r")
        e.confirm_cancellation(req.request_id,"admin")
        with pytest.raises(CancellationError): e.confirm_cancellation(req.request_id,"admin")
    ...
    def test_T138_abort_confirmed_raises(self):
        e, _ = self._e(); req = e.request_cancellation("u","t","r")
        e.confirm_cancellation(req.request_id,"admin")
        with pytest.raises(CancellationError): e.abort_cancellation(req.request_id)

    def test_T139_confirm_missing_raises(self):
        e, _ = self._e()
        with pytest.raises(CancellationError): e.confirm_cancellation("x","a")

    def test_T140_abort_missing_raises(self):
        e, _ = self._e()
        with pytest.raises(CancellationError): e.abort_cancellation("x")

    def test_T141_data_deletion_flag(self):
        e, _ = self._e(); req = e.request_cancellation("u","t","r",data_deletion=True)
        assert req.data_deletion is True

    def test_T142_request_audited(self):
        e, audit = self._e(); e.request_cancellation("u","t","r")
        assert len(audit.query(action=AuditAction.CANCEL_REQUESTED)) == 1

    def test_T143_default_notice_30_days(self):
        e, _ = self._e(); assert e._config.notice_days == 30
...
class TestSaaSReadinessChecker:
    ...
    def test_T146_missing_docs_fails(self):
        sys_ = build_compliance_system(); sys_["doc_store"]._docs.clear()
    ...
    def test_T149_to_dict_serializable(self):
        ...
        for k in ["passed","score","missing_docs"]: assert k in d

    def test_T150_check_audited(self):
        sys_ = build_compliance_system(); sys_["saas_checker"].check()
    ...
    def test_T152_tampered_chain_fails(self):
        ...
        if entries: entries[0].chain_hash = "a" * 64
    ...
    def test_T159_refund_window_zero_fails(self):
        r = build_compliance_system(refund_config=RefundPolicyConfig(window_days=0))["saas_checker"].check()
        assert r.score < 100

    def test_T160_all_saas_docs_in_active_store(self):
        sys_ = build_compliance_system()
        active_types = {d.doc_type for d in sys_["doc_store"].list_active()}
        for req in SAAS_REQUIRED_DOCS: assert req in active_types

    def test_T161_checker_fields(self):
        r = SaaSReadinessReport(passed=True, score=100, missing_docs=[], outdated_docs=[], gaps=[], recommendations=[])
    ...
    def test_T162_check_twice_consistent(self):
        ...
        r1 = checker.check(); r2 = checker.check()
        assert r1.passed == r2.passed and r1.score == r2.score

    def test_T163_cancellation_negative_notice_fails(self):
        r = build_compliance_system(cancellation_config=CancellationPolicyConfig(notice_days=-1))["saas_checker"].check()
        assert r.score < 100

    def test_T164_full_system_build_returns_all_components(self):
        sys_ = build_compliance_system()
        for k in ["audit","doc_store","consent_store","retention","refund",
                  "cancellation","disclosure","admin","saas_checker","factory"]:
            assert k in sys_


class TestComplianceAdmin:
    def _a(self):
        audit = ComplianceAuditChain(); ds = DocumentStore(); cs = ConsentStore()
    ...
    def test_T166_publish_supersedes_old(self):
        ...
        new = a.publish_document(DocumentType.TOS,"2.0.0","TOS v2","new",time.time(),"admin",supersede_reason="update")
    ...
    def test_T169_archive_requires_reason(self):
        a, ds, cs, _ = self._a()
        doc = a.publish_document(DocumentType.COOKIE,"1.0","C","c",time.time(),"admin")
        with pytest.raises(MissingReasonError): a.archive_document(doc.doc_id,"admin","")
    ...
    def test_T172_audit_summary_structure(self):
        ...
        a.publish_document(DocumentType.TOS,"1.0","T","c",time.time(),"admin")
        s = a.audit_summary()
        for k in ["total_entries","chain_valid","docs_active","consents_total"]: assert k in s
    ...
    def test_T174_publish_all_10_types(self):
        ...
        for dt in DocumentType:
            a.publish_document(dt,"1.0",f"T","c",time.time(),"admin")
    ...
    def test_T176_supersede_audited(self):
        ...
        a.publish_document(DocumentType.TOS,"2.0","T","new",time.time(),"admin",supersede_reason="update")
    ...
    def test_T178_audit_summary_consent_count(self):
        a, ds, cs, _ = self._a(); cs.record(make_consent())
        assert a.audit_summary()["consents_total"] == 1

    def test_T179_bulk_check_multi_missing(self):
        a, ds, cs, _ = self._a()
        for dt in ACCEPTANCE_REQUIRED: a.publish_document(dt,"1.0","T","c",time.time(),"admin")
...
class TestLegalDocumentFactory:
    ...
    def test_T195_content_hash_stable(self):
        f = LegalDocumentFactory()
        _, c1 = f.generate(DocumentType.TOS); _, c2 = f.generate(DocumentType.TOS)
...
class TestIntegrationFlows:
    ...
    def test_T198_new_user_onboarding_flow(self):
        sys_ = build_compliance_system(); engine = sys_["disclosure"]
        uid = "new_user_001"; tid = "t1"; created = time.time()
        allowed, missing = engine.check_access(uid, tid, created)
        assert allowed is True
        for dt in ACCEPTANCE_REQUIRED: engine.record_consent(uid, tid, dt, "1.2.3.4", "B")
    ...
    def test_T201_audit_chain_integrity_after_all_ops(self):
        sys_ = build_compliance_system(); engine = sys_["disclosure"]
        for dt in ACCEPTANCE_REQUIRED: engine.record_consent("u","t",dt,"1","A")
    ...
    def test_T202_100_users_concurrent_consent(self):
        sys_ = build_compliance_system(); errors = []
        def accept(uid):
            try:
                for dt in ACCEPTANCE_REQUIRED: sys_["disclosure"].record_consent(uid,"t",dt,"1","A")
            except Exception as e: errors.append(e)
    ...
    def test_T203_version_update_flow(self):
        sys_ = build_compliance_system()
        new = sys_["admin"].publish_document(DocumentType.TOS,"2.0.0","TOS v2","Updated terms",time.time(),"admin",supersede_reason="Annual review")
    ...
    def test_T205_all_audit_actions_used(self):
        ...
        for a in [AuditAction.DOC_PUBLISHED, AuditAction.CONSENT_RECORDED,
                  AuditAction.REFUND_ISSUED, AuditAction.CANCEL_REQUESTED, AuditAction.SAAS_CHECK]:
            assert a in actions
    ...
    def test_T206_sql_migration_tables(self):
        ...
        for t in ["legal_documents","consent_records","compliance_audit_log"]: assert t in sql

    def test_T207_sql_migration_rls(self):
        sql_path = os.path.join(os.path.dirname(__file__),
            "../../supabase/migrations/20260628_039_phase30_compliance.sql")
        if not os.path.exists(sql_path): pytest.skip("SQL file not found")
        sql = open(sql_path).read()
    ...
    def test_T209_build_system_isolation(self):
        s1 = build_compliance_system(secret="s1"); s2 = build_compliance_system(secret="s2")
    ...
    def test_T211_disclosure_engine_grace_0_blocks_immediately(self):
        sys_ = build_compliance_system()
        engine = DisclosureEngine(sys_["doc_store"], sys_["consent_store"], sys_["audit"], grace_days=0)
        with pytest.raises(ConsentRequiredError): engine.check_access("new","t",time.time()-1)
    ...
    def test_T212_saas_readiness_is_production_ready(self):
        ...
        for cat in ["user_pii","trading_logs","audit_logs","financial_records","payment_records"]:
