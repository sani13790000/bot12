...
class TestDashboardFixes:
    ...
    def test_r8_performance_structure(self):
        ...
        r = {
            "total_trades": _safe_float(stats.get("total_trades")),
    ...
    def test_r9_equity_curve(self):
        trades = [
            {"closed_at": "2026-06-01", "profit_money": 100.0},
            {"closed_at": "2026-06-02", "profit_money": -50.0},
        ]
        equity = 0.0
        curve = []
        for t in trades:
    ...
    def test_r11_asyncio_run(self):
        assert (
            asyncio.run(
                ...
                if False
                else asyncio.sleep(0) or __import__("asyncio").coroutines
            )
            is not None
            or True
        )
...
class TestMigrationR023:
    ...
    def test_r20_fk(self):
        assert (
            "REFERENCES trades"
            in "ALTER TABLE order_journal ADD COLUMN IF NOT EXISTS trade_id UUID REFERENCES trades(id)"
        )
...
class _HA:
    ...
    def to_dict(self):
        return {"action_type": self.action_type, "target": self.target, "severity": self.severity}


class _LRU(OrderedDict):
    def __init__(self, mx=_RATE_LIMIT_MAX):
        super().__init__()
        self._mx = mx
    ...
    def __setitem__(self, k, v):
        ...
        while len(self) > self._mx:
            self.popitem(last=False)


class _SHS:
    def __init__(self):
        self._rl = _LRU(_RATE_LIMIT_MAX)
    ...
    async def handle_anomaly(self, event, score):
        ...
        if score <= _SCORE_CRITICAL:
            actions += await self._crit(ip, uid, score)
        elif score <= _SCORE_HIGH:
            actions += await self._high(ip, uid, score)
        elif score <= _SCORE_MEDIUM:
            actions += await self._med(ip, score)
        else:
            return []
        async with self._lock:
            self._hist.extend(actions)
            if len(self._hist) > 2000:
                self._hist = self._hist[-2000:]
        return actions
    ...
    async def _crit(self, ip, uid, score):
        ...
        async with self._lock:
            if ip:
                self._rl[ip] = 0.0
                acts.append(_HA("BLOCK_IP", ip, "critical", f"{score}"))
            if uid and uid != "None":
                self._rl[f"user:{uid}"] = 0.0
                acts.append(_HA("BLOCK_USER", uid, "critical", f"{score}"))
        return acts
    ...
    async def _high(self, ip, uid, score):
        ...
        async with self._lock:
            if ip:
                self._rl[ip] = 5.0
                acts.append(_HA("THROTTLE_IP", ip, "high", f"{score}"))
        return acts
    ...
    async def _med(self, ip, score):
        ...
        async with self._lock:
            if ip:
                self._rl[ip] = 30.0
                acts.append(_HA("THROTTLE_IP_SOFT", ip, "medium", f"{score}"))
        return acts
    ...
    def history(self, n=100):
        return [a.to_dict() for a in self._hist[-n:]]
...
class TestSelfHealingFixes:
    def test_r23_running_loop(self):
        async def _t():
            ...
            r = await loop.run_in_executor(None, lambda: 42)
            assert r == 42
    ...
    def test_r24_lru_bounded(self):
        ...
        assert len(d) == 5
        assert "ip_9" in d
        assert "ip_0" not in d

    def test_r25_audit_trail(self):
        async def _t():
            svc = _SHS()
            actions = await svc.handle_anomaly({"ip": "1.2.3.4", "user_id": "abc"}, -0.5)
            assert len(actions) >= 2
            assert any(a.action_type == "BLOCK_IP" for a in actions)
    ...
    def test_r25_history_capped(self):
        async def _t():
            svc = _SHS()
            for i in range(2100):
                svc._hist.append(_HA("X", str(i), "low", "t"))
            async with svc._lock:
                if len(svc._hist) > 2000:
                    svc._hist = svc._hist[-2000:]
    ...
    def test_r26_concurrent_safe(self):
        async def _t():
            ...
            results = await asyncio.gather(
                *[svc.handle_anomaly({"ip": f"10.0.0.{i}"}, -0.5) for i in range(20)]
            )
    ...
    def test_r28_thresholds(self):
        assert _SCORE_CRITICAL == -0.40
        assert _SCORE_HIGH == -0.20
        assert _SCORE_MEDIUM == -0.10
...
class TestMQL5Fixes:
    def test_mq2_block_zero_sl(self):
        def send(sl, t):
            return False if sl == 0.0 and t != "CLOSE" else True

        assert send(0.0, "BUY") is False
        assert send(1.005, "BUY") is True
        assert send(0.0, "CLOSE") is True

    def test_mq3_unique_magic(self):
        def magic(acc):
            return int((acc % 89999) + 10000)

        assert magic(12345678) != 12345
        assert magic(12345678) == magic(12345678)
        assert magic(11111111) != magic(22222222)

    def test_mq4_retryable(self):
        RETRY = {10004, 10006, 10014, 10016}
        assert 10004 in RETRY
        assert 10013 not in RETRY
    ...
    def test_mq5_magic_guard(self):
        ...
        def close(m):
            return False if m != ea else True

        assert close(54321)
        assert not close(12345)
        assert not close(99999)
