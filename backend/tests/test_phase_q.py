...
# Q-12
from backend.risk.equity_protection import EquityProtectionEngine, EquityProtectionConfig, ProtectionLevel
...
def test_q13_weekly_next_reset():
    r = DailyLimitsEngine(max_weekly_loss_pct=5.0).check_limits(10000.0, TodayTrades(1,0.0,0.0), week_pnl_usd=-600.0)
    assert not r.can_trade and r.next_reset is not None

def test_q13_monthly_next_reset():
    r = DailyLimitsEngine(max_monthly_dd_pct=10.0).check_limits(10000.0, TodayTrades(1,0.0,0.0), month_pnl_usd=-1100.0)
    assert not r.can_trade and r.next_reset is not None
...
def test_q15_safe_div_zero(): assert _safe_div(5.0, 0.0) == 0.0
def test_q15_safe_div_normal(): assert abs(_safe_div(6.0, 3.0) - 2.0) < 1e-9
def test_q15_safe_div_custom(): assert _safe_div(1.0, 0.0, -1.0) == -1.0

def test_q15_no_zero_division():
    class ZR:
        sharpe_ratio=0.0; profit_factor=0.0; win_rate=0.0; net_profit_pct=0.0; total_trades=5; max_drawdown_pct=5.0
    class GR:
        sharpe_ratio=1.5; profit_factor=1.8; win_rate=0.6; net_profit_pct=12.0; total_trades=20; max_drawdown_pct=4.0
    async def mock_bt(a,b,c,d): return ZR(), GR()
    wf = WalkForwardAdvanced(is_months=3, oos_months=1, step_months=1)
    result = run(wf.run(datetime(2023,1,1,tzinfo=timezone.utc), datetime(2023,12,31,tzinfo=timezone.utc), mock_bt))
