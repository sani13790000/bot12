...
class TestDeadCodeRemoved(unittest.TestCase):
    ...
    def test_volatility_filter_compiles(self):
        with open(_BASE / "volatility_filter.py") as f:
            src = f.read()
    ...
    def test_correlation_lock_is_live(self):
        with open(_BASE / "correlation_filter.py") as f:
            src = f.read()
    ...
    def test_exposure_optional_used(self):
        ...
        with open(_BASE / "exposure_control.py") as f:
            src = f.read()
        tree = ast.parse(src)
        self.assertTrue(any(isinstance(n, ast.Name) and n.id == "Optional" for n in ast.walk(tree)))
...
class TestOrchestratorGates(unittest.TestCase):
    ...
    def test_per_gate_override(self):
        orch = RiskOrchestrator(
            fail_mode_equity="FAIL_OPEN",
            fail_mode_daily=FailMode.FAIL_CLOSED,
            fail_mode_volatility="FAIL_OPEN",
            fail_mode_correlation=FailMode.FAIL_CLOSED,
            fail_mode_lot="FAIL_OPEN",
            fail_mode_exposure=FailMode.FAIL_CLOSED,
        )
    ...
    def test_equity_fail_open_allows(self):
        orch = RiskOrchestrator(equity_guard=_raising_guard(), fail_mode_equity=FailMode.FAIL_OPEN)
        self.assertIn("EQUITY_FAIL_OPEN", _run(orch).gates_passed)

    def test_equity_fail_closed_blocks(self):
        orch = RiskOrchestrator(
            equity_guard=_raising_guard(), fail_mode_equity=FailMode.FAIL_CLOSED
        )
        self.assertFalse(_run(orch).approved)

    def test_daily_fail_open_allows(self):
        orch = RiskOrchestrator(daily_limits=_raising_guard(), fail_mode_daily=FailMode.FAIL_OPEN)
        self.assertIn("DAILY_LIMITS_FAIL_OPEN", _run(orch).gates_passed)

    def test_correlation_fail_open_allows(self):
        orch = RiskOrchestrator(
            correlation_filter=_raising_guard(), fail_mode_correlation=FailMode.FAIL_OPEN
        )
        self.assertIn("CORRELATION_FAIL_OPEN", _run(orch).gates_passed)

    def test_every_exception_logged(self):
        orch = RiskOrchestrator(
            equity_guard=_raising_guard(),
            daily_limits=_raising_guard(),
