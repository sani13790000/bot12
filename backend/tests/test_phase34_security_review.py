...
220 tests T001-T220
"""
import time, threading, uuid, pytest
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from core.security_review import (
    RiskLevel, VulnCategory, CheckStatus, AuditAction,
    SECURITY_HEADERS_REQUIRED, REQUIRES_REASON, SENSITIVE_FIELDS,
    SecurityAuditChain, MissingReasonError,
    SecurityFinding, CheckResult,
    SecurityHeadersEnforcer,
    CORSPolicy, CORSEnforcer, CORSViolationError,
    RateLimitRule, RateLimiter, RateLimitExceededError,
    InjectionScanner, InjectionDetectedError,
    IDORChecker, IDORDetectedError,
    ReplayChecker, ReplayDetectedError,
    InformationLeakScanner,
    AuthCheckConfig, AuthRBACChecker,
    SecurityLogger,
    CryptoChecker,
    RiskItem, RiskRegister,
    PenTestScenario, PenTestResult, PenTestHarness,
    SecurityReviewReport, SecurityReviewEngine,
...
ENDPOINTS = [
    '/api/auth/login', '/api/auth/refresh', '/api/license',
    '/api/signals', '/api/billing', '/api/webhook',
    '/api/admin', '/api/ea/heartbeat',
]
...
class TestSecurityAuditChain:
    ...
    def test_T033_different_secrets_invalid(self):
        ...
        e  = a2.record(AuditAction.CHECK_RUN, actor='x')
        a2._secret = b'wrong'
...
class TestIDORChecker:
    ...
    def test_T103_run_check_same_tenant(self):
        r = self.idor.run_check([{
            'actor_tenant':'T','actor_user':'u',
            'resource_tenant':'T','resource_id':'r'}])
        assert r.status == CheckStatus.PASS

    def test_T104_run_check_cross_tenant_fail(self):
        r = self.idor.run_check([{
            'actor_tenant':'A','actor_user':'u',
            'resource_tenant':'B','resource_id':'r'}])
        assert r.status == CheckStatus.FAIL

    def test_T105_cwe_639_in_finding(self):
        r = self.idor.run_check([{
            'actor_tenant':'X','actor_user':'u',
            'resource_tenant':'Y','resource_id':'r'}])
...
class TestIntegrationFlows:
    ...
    def test_T198_risk_register_mitigate(self):
        risk = self.sys_['risk']
        risk.add(RiskItem(
            risk_id='R1', category=VulnCategory.SESSION,
            risk=RiskLevel.LOW, title='Test risk',
    ...
    def test_T200_full_review_pass(self):
        ...
        report = engine.run_full_review(
            ...
            idor_cases        = [{'actor_tenant':'T','actor_user':'u',
                                  'resource_tenant':'T','resource_id':'r'}],
            replay_cases      = [],
    ...
    def test_T202_pen_test_sql_inject_blocked(self):
        ...
        s = PenTestScenario('S1', 'SQL Inject', VulnCategory.INJECTION,
                            {'value': "' UNION SELECT * FROM users--"}, True, 'sqli')
        r = pentest.run_scenario(s)
        assert r.passed is True

    def test_T203_pen_test_idor_blocked(self):
        pentest = self.sys_['pentest']
        s = PenTestScenario('S2', 'IDOR', VulnCategory.IDOR,
                            {'actor_tenant': 'X', 'actor_user': 'u',
                             'resource_tenant': 'Y', 'resource_id': 'r1'},
                            True, 'cross-tenant')
        r = pentest.run_scenario(s)
        assert r.passed is True
    ...
    def test_T204_pen_test_good_auth_allowed(self):
        pentest = self.sys_['pentest']
        s = PenTestScenario('S3', 'Good Auth', VulnCategory.AUTH_BYPASS,
                            good_token(), False, 'valid')
        r = pentest.run_scenario(s)
        assert r.passed is True
...
class TestEdgeCasesAndAcceptance:
    ...
    def test_T208_security_finding_defaults(self):
        f = SecurityFinding(
            finding_id='f1', category=VulnCategory.CORS,
            risk=RiskLevel.HIGH, title='T', description='D',
    ...
    def test_T209_build_system_returns_all_keys(self):
        sys_ = make_system()
        for key in ['audit','headers','cors','rl','inj','idor',
                    'replay','leak','auth','logger','crypto',
                    'risk','pentest','engine']:
    ...
    def test_T210_build_system_shared_audit(self):
        ...
        sys_['engine'].run_full_review(
            headers=good_headers(), injection_samples=[],
            idor_cases=[], replay_cases=[], leak_responses=[],
            auth_tokens=[good_token()], endpoints_checked=[],
            crypto_configs=[], pen_scenarios=[])
        assert len(audit) > 0
    ...
    def test_T211_risk_register_accept(self):
        ...
        risk.add(RiskItem(
            risk_id='R2', category=VulnCategory.LOGGING,
            risk=RiskLevel.LOW, title='Minor log issue',
            description='desc', mitigation='fix'))
    ...
    def test_T212_risk_register_list_open(self):
        risk = RiskRegister()
        risk.add(RiskItem(
            risk_id='R3', category=VulnCategory.CRYPTO,
            risk=RiskLevel.MEDIUM, title='Open risk',
            description='desc', mitigation='fix'))
    ...
    def test_T213_pen_test_replay_blocked(self):
        ...
        nonce   = str(uuid.uuid4())
        replay.check(nonce, now, now=now)
        s = PenTestScenario('S-replay', 'Replay',
                            VulnCategory.REPLAY,
                            {'nonce': nonce, 'timestamp': now, 'now': now + 1},
                            True, 'replay')
        r = pentest.run_scenario(s)
    ...
    def test_T214_pen_test_leak_blocked(self):
        ...
        pentest = PenTestHarness(
            AuthRBACChecker(), IDORChecker(), ReplayChecker(),
            InjectionScanner(), InformationLeakScanner(), audit)
        s = PenTestScenario('S-leak', 'Leak',
                            VulnCategory.INFORMATION_LEAK,
                            {'password': 'hunter2'}, True, 'leak')
        r = pentest.run_scenario(s)
        assert r.passed is True
    ...
    def test_T216_full_review_all_attack_vectors(self):
        ...
        scenarios = [
            PenTestScenario('S1', 'SQL inject',
                            VulnCategory.INJECTION,
                            {'value': "' UNION SELECT * FROM users--"},
                            True, 'sqli'),
            PenTestScenario('S2', 'XSS inject',
                            VulnCategory.INJECTION,
                            {'value': '<script>document.cookie</script>'},
                            True, 'xss'),
            PenTestScenario('S3', 'Valid auth',
                            VulnCategory.AUTH_BYPASS,
                            good_token(), False, 'good token'),
            PenTestScenario('S4', 'No-sub auth bypass',
                            VulnCategory.AUTH_BYPASS,
                            {'exp': time.time() + 3600, 'tenant_id': 't1'},
                            True, 'missing sub'),
            PenTestScenario('S5', 'IDOR block',
                            VulnCategory.IDOR,
                            {'actor_tenant': 'X', 'actor_user': 'u',
                             'resource_tenant': 'Y', 'resource_id': 'r1'},
                            True, 'cross-tenant'),
            PenTestScenario('S6', 'Secret leak',
                            VulnCategory.INFORMATION_LEAK,
                            {'password': 'hunter2'},
                            True, 'password field'),
        ]
        ...
        risk.mitigate('RESIDUAL-1', 'security-team',
                      'session_id rotated on login')
        assert risk.summary()['mitigated'] >= 1
