...
@dataclass
class _CacheEntry:
    correlation: float; timestamp: float; source: str


@dataclass
class CorrelationResult:
    symbol_a: str; symbol_b: str; correlation: float
    source: str; window_bars: int; cached: bool
    high_vol_a: bool = False; high_vol_b: bool = False

    @property
    def is_highly_correlated(self) -> bool: return abs(self.correlation) >= 0.75
    @property
    def regime(self) -> str:
...
class RollingCorrelationEngine:
    ...
    def __init__(self, window: int = _DEFAULT_WINDOW, ttl_s: float = _DEFAULT_TTL_S) -> None:
        self._window = window; self._ttl_s = ttl_s
    ...
    async def get_correlation(self, a: str, b: str) -> CorrelationResult:
        key = _canonical(a, b)
        async with self._lock: return self._get_or_compute(key)

    def _get_or_compute(self, key: Tuple[str, str]) -> CorrelationResult:
        a, b = key; now = time.monotonic()
        cached = self._cache.get(key)
        if cached and (now - cached.timestamp) < self._ttl_s:
            pw_a = self._prices.get(a); pw_b = self._prices.get(b)
            return CorrelationResult(symbol_a=a, symbol_b=b, correlation=cached.correlation,
                source=cached.source, window_bars=self._window, cached=True,
        ...
        if pw_a and pw_b and pw_a.ready and pw_b.ready:
            ...
            return CorrelationResult(symbol_a=a, symbol_b=b, correlation=corr,
                source="rolling", window_bars=self._window, cached=False,
        ...
        return CorrelationResult(symbol_a=a, symbol_b=b, correlation=static,
            source=source, window_bars=0, cached=False)
    ...
    async def portfolio_correlation_matrix(self, symbols: List[str]) -> Dict[str, Dict[str, float]]:
        ...
        async with self._lock:
            for a in symbols:
                matrix[a] = {}
                for b in symbols:
                    matrix[a][b] = 1.0 if a == b else self._get_or_compute(_canonical(a, b)).correlation
        return matrix

    async def get_high_correlation_pairs(self, symbols: List[str], threshold: float = 0.75) -> List[Tuple[str, str, float]]:
        pairs = []
    ...
    def stats(self) -> Dict[str, int]:
        return {"tracked_symbols": len(self._prices), "cache_entries": len(self._cache),
                "window_bars": self._window, "ttl_s": int(self._ttl_s)}
...
async def get_correlation_engine(window: int = _DEFAULT_WINDOW, ttl_s: float = _DEFAULT_TTL_S) -> RollingCorrelationEngine:
    global _engine
    if _engine is None:
        async with _engine_lock:
            if _engine is None:
                _engine = RollingCorrelationEngine(window=window, ttl_s=ttl_s)
                logger.info("RollingCorrelationEngine initialized (window=%d, ttl=%.0fs)", window, ttl_s)
    return _engine
