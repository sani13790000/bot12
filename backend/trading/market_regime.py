...
class MarketRegimeDetector:
    ...
    async def detect(self, symbol: str) -> RegimeResult:
        ...
        if len(bars) < self._cfg.min_bars_required:
            return RegimeResult(symbol=symbol, regime=MarketRegime.UNKNOWN,
                adx=0.0, atr=0.0, atr_zscore=0.0, bb_width_pct=0.0, confidence=0.0,
                bars_used=len(bars),
    ...
    async def bulk_detect(self, symbols):
        ...
        for sym, task in tasks.items():
            try:
                ...
                results[sym] = RegimeResult(symbol=sym, regime=MarketRegime.UNKNOWN,
                    adx=0.0, atr=0.0, atr_zscore=0.0, bb_width_pct=0.0, confidence=0.0)
        return results

    def get_trading_multiplier(self, regime: MarketRegime) -> float:
        return {MarketRegime.TRENDING_UP:1.0, MarketRegime.TRENDING_DOWN:1.0,
                MarketRegime.RANGING:0.6, MarketRegime.HIGH_VOL:0.5,
                MarketRegime.LOW_VOL:0.8, MarketRegime.UNKNOWN:0.3}[regime]
    ...
    def _compute(self, symbol, bars):
        ...
        adx=self._calc_adx(H,L,C); atr=self._calc_atr(H,L,C)
        atr_z=self._calc_atr_zscore(H,L,C); bb_w=self._calc_bb_width(C)
        bb_wp=bb_w/C[-1] if C[-1]>0 else 0.0
        regime,conf=self._classify(adx,atr_z,bb_wp)
        return RegimeResult(symbol=symbol,regime=regime,adx=adx,atr=atr,
            atr_zscore=atr_z,bb_width_pct=bb_wp,confidence=conf,bars_used=len(bars),
    ...
    def _classify(self,adx,atr_z,bb_w):
        ...
        if adx>=self._cfg.adx_trend_threshold:
            return MarketRegime.TRENDING_UP,min(1.0,0.5+(adx-self._cfg.adx_trend_threshold)/40.0)
    ...
    def _calc_atr_zscore(self,H,L,C):
        trs=[max(H[i]-L[i],abs(H[i]-C[i-1]),abs(L[i]-C[i-1])) for i in range(1,len(H))]
        if len(trs)<10: return 0.0
        m=sum(trs)/len(trs); v=sum((x-m)**2 for x in trs)/len(trs)
    ...
    def _calc_adx(self,H,L,C):
        ...
        def sm(data,n):
            ...
            r=[sum(data[:n])]
            for i in range(n,len(data)): r.append(r[-1]-r[-1]/n+data[i])
            return r
        st=sm(tr,p);sp=sm(pdm,p);sm2=sm(mdm,p)
        if not st or not sp or not sm2: return 0.0
        dx=[]
        for s2,p2,m2 in zip(st,sp,sm2):
            if s2==0: continue
            pdi=100*p2/s2; mdi=100*m2/s2; den=pdi+mdi
    ...
    def _calc_bb_width(self,closes):
        p=self._cfg.bb_period; w=closes[-p:] if len(closes)>=p else closes
        if not w: return 0.0
        m=sum(w)/len(w); s=math.sqrt(sum((x-m)**2 for x in w)/len(w))
