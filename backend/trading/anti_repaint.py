...
class RepaintRisk(str,Enum):
    CLEAN="CLEAN"; SUSPICIOUS="SUSPICIOUS"; CONFIRMED="CONFIRMED"; UNKNOWN="UNKNOWN"

@dataclass
class FeatureSnapshot:
    name:str; value:float; timestamp:float; bar_index:int

@dataclass
class SignalRecord:
    signal_id:str; generated_at:float; bar_close_time:float; bar_index:int
    features:List[FeatureSnapshot]=field(default_factory=list)
    direction:str=""; score:float=0.0; input_hash:str=""

@dataclass(frozen=True)
class RepaintResult:
    signal_id:str; risk:RepaintRisk; confidence:float
...
class AntiRepaintValidator:
    ...
    def capture(self,signal_id,bar_close_time,bar_index,features,direction="",score=0.0):
        ...
        r=SignalRecord(signal_id,time.time(),bar_close_time,bar_index,features,direction,score,h)
        self._history[signal_id]=r
        return r

    def validate(self,record:SignalRecord)->RepaintResult:
        issues=[]; metadata={}; risk=RepaintRisk.CLEAN
        ...
        if lag<-self._tolerance:
            ...
            issues.append(f"SUSPICIOUS: signal generated only {lag:.3f}s after bar close (expected >={self._delay}s)")
            if risk==RepaintRisk.CLEAN: risk=RepaintRisk.SUSPICIOUS
        future=[f for f in record.features if f.timestamp>record.bar_close_time+self._tolerance]
        if future:
            names=[f.name for f in future]
            issues.append(f"FUTURE DATA: features {names} timestamp > bar_close")
            risk=RepaintRisk.CONFIRMED; metadata["future_features"]=names
        ...
        if wrong:
            ...
            risk=RepaintRisk.CONFIRMED; metadata["index_violation"]=names
        ...
        conf={RepaintRisk.CLEAN:1.0,RepaintRisk.SUSPICIOUS:0.5,RepaintRisk.CONFIRMED:0.95,RepaintRisk.UNKNOWN:0.1}[risk]
        if risk!=RepaintRisk.CLEAN:
            logger.warning("anti-repaint %s -> %s | %s",record.signal_id,risk.value,"; ".join(issues))
