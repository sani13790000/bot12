...
async def _get_voting_engine():
    ...
    async with _voting_engine_lock:
        ...
        try:
            ...
            _voting_engine_instance = VotingEngine(
                agents=agents,
...
class DecisionService:
        ...
        **kwargs: Any,
        ...
        try:
            input_data = DecisionInput(
                ...
                timeframe=timeframe,
                user_id=user_id,
                **{k: v for k, v in kwargs.items() if k in DecisionInput.__dataclass_fields__},
            )
        ...
        if voting_engine:
            try:
                vote_result = await voting_engine.vote(
                    symbol=symbol, direction=direction, user_id=user_id
                )
                result["vote"] = {
                    "direction": vote_result.direction,
                    "score": vote_result.score,
                    "confidence": vote_result.confidence,
                    "consensus": vote_result.consensus_pct,
                }
            except Exception as exc:
    ...
    async def get_decision_history(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        try:
            ...
            logger.error("get_decision_history failed: %s", exc)
            return []
    ...
    def invalidate_cache(self, user_id: str, symbol: Optional[str] = None) -> int:
        ...
        keys_to_remove = [
            ...
            if k.startswith(prefix) and (symbol is None or f":{symbol}:" in k)
        ]
