...
class SignalProcessor:
    ...
    async def process(self, signal: RawSignal) -> ProcessResult:
        ...
        logger.info("[SignalProcessor] process %s %s conf=%.2f", signal.direction, signal.symbol, signal.confidence)
        ...
        logger.info("[SignalProcessor] accepted %s %s → ticket=%s (%.0f ms)", signal.direction, signal.symbol, ticket, elapsed)
    ...
    async def _voting_confirms(self, signal: RawSignal) -> bool:
        ...
        try:
            votes = await asyncio.wait_for(
                self._voting_engine.collect_votes(symbol=signal.symbol, timeframe=signal.timeframe, direction=signal.direction, confidence=signal.confidence),
                timeout=10.0,
            )
            approve = sum(1 for v in votes if v.get("approve"))
            total = len(votes)
            result = approve > total / 2
            logger.info("[SignalProcessor] VotingEngine %d/%d approve ₒ %s", approve, total, result)
            return result
        except asyncio.TimeoutError:
            logger.warning("[SignalProcessor] VotingEngine timeout")
            await self._alert_timeout(signal)
            return True
    ...
    async def _execute(self, signal: RawSignal) -> Dict[str, Any]:
        ...
        try:
            ...
            ts = TradeSignal(symbol=signal.symbol, direction=signal.direction, volume=signal.meta.get("volume", 0.01), entry=signal.entry_price, sl=signal.sl_price, tp=signal.tp_price, strategy=signal.strategy, confidence=signal.confidence)
    ...
    async def _persist(self, signal: RawSignal, ticket: Optional[int]) -> None:
        if self._db is None:
            try:
                ...
                return
        try:
            await self._db.insert("signals", {"symbol": signal.symbol, "direction": signal.direction, "confidence": signal.confidence, "entry_price": signal.entry_price, "sl_price": signal.sl_price, "tp_price": signal.tp_price, "strategy": signal.strategy, "ticket": ticket, "created_at": datetime.now(timezone.utc).isoformat()})
        except Exception as exc:
            logger.warning("[SignalProcessor] DB persist failed: %s", exc)

    def _reject(self, signal: RawSignal, reason: str, t0: float) -> ProcessResult:
        elapsed = (datetime.now(timezone.utc).timestamp() - t0) * 1000
        logger.info("[SignalProcessor] rejected %s %s — reason: %s", signal.direction, signal.symbol, reason)
        return ProcessResult(accepted=False, signal=signal, reason=reason, elapsed_ms=elapsed)

    async def _alert_timeout(self, signal) -> None:
        """FIX K-5: Telegram alert if VotingEngine times out."""
        try:
            from backend.telegram.notifier import get_notifier
            notifier = get_notifier()
            symbol = getattr(signal, "symbol", "unknown")
            await notifier.send(f"\u26a0\ufe0f *VotingEngine Timeout*\nSymbol: `{symbol}`\nSignal processed without vote (timeout > 10s).\nCheck agent health.")
        except Exception as exc:
            logger.warning("[SignalProcessor] alert_timeout failed: %s", exc)
