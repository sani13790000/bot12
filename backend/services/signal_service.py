...
class SignalService:
    ...
    async def create_signal(
        ...
        if not result.data:
            raise RuntimeError(f"insert empty for id={sid}")
    ...
    async def list_signals(
        ...
        return {
            "signals": signals,
            "total": getattr(r, "count", len(signals)),
            "page": page,
            "page_size": page_size,
        }
    ...
    async def update_signal_status(self, signal_id, user_id, new_status, updated_at=None):
        ...
        q = (
            self._db.table("signals")
            .update({"status": new_status.upper(), "updated_at": now.isoformat()})
            .eq("id", signal_id)
            .eq("user_id", user_id)
        )
        if updated_at is not None:
            q = q.eq("updated_at", updated_at.isoformat())
        r = await self._db_execute(q.execute)
        return bool(r.data)
