...
class SelfHealingService:
            ...
            if ip:
                ...
                actions.append(
                    HealingAction(
                        ...
                        "critical",
                        f"score={score}",
                        auto_expire_at=datetime.now(timezone.utc)
                        + timedelta(seconds=_BLOCK_TTL_CRITICAL),
                    )
                )
            ...
            if ip:
                ...
                actions.append(
                    HealingAction(
                        ...
                        f"score={score}",
                        auto_expire_at=datetime.now(timezone.utc)
                        + timedelta(seconds=_BLOCK_TTL_HIGH),
                    )
                )
        return actions
            ...
            if ip:
                ...
                actions.append(
                    HealingAction(
                        ...
                        f"score={score}",
                        auto_expire_at=datetime.now(timezone.utc)
                        + timedelta(seconds=_BLOCK_TTL_MEDIUM),
                    )
                )
        return actions
    ...
    async def _log_actions(self, actions, score):
        for a in actions:
            log.warning(
                "SelfHeal[%s] target=%s severity=%s score=%.3f",
                a.action_type,
                a.target,
                a.severity,
                score,
