    ...
    def verify_chain(self) -> bool:
        ...
        for e in entries:
            expected = self._make_hash(
                prev, e.entry_id, e.action, e.actor, e.domain, e.reason, e.detail, e.ts
            )
...
class ReleaseCheckEngine:
    ...
    def check_trading_execution(self, metrics):
        ...
        if fill_rate < 95.0:
            r = CheckResult(
                CheckDomain.TRADING_EXECUTION,
                "trading_fill_rate",
                CheckStatus.FAIL,
                f"Fill rate {fill_rate}% < 95%",
                phase="P07",
            )
        elif slippage > 2.0:
            r = CheckResult(
            ...
            r = CheckResult(
                ...
                "trading_execution",
                CheckStatus.PASS,
                f"fill={fill_rate}% slippage={slippage}% drawdown={drawdown}%",
                phase="P07",
            )
        self._emit(r)
        return r
    ...
    def check_license_saas(self, metrics):
        if not metrics.get("device_binding_active", True):
            r = CheckResult(
                CheckDomain.LICENSE_SAAS,
                "device_binding_missing",
                CheckStatus.FAIL,
                "Device binding not active",
                phase="P10",
            )
        elif not metrics.get("license_expiry_enforced", True):
            r = CheckResult(
                CheckDomain.LICENSE_SAAS,
                "license_expiry_not_enforced",
                CheckStatus.FAIL,
            ...
            r = CheckResult(
                CheckDomain.LICENSE_SAAS,
                "offline_protection_missing",
                CheckStatus.WARN,
                "Offline protection missing",
                phase="P10",
            )
        else:
            r = CheckResult(
    ...
    def check_auth_rbac(self, metrics):
        if not metrics.get("jwt_rotation_active", True):
            r = CheckResult(
                CheckDomain.AUTH_RBAC,
                "jwt_rotation_inactive",
                CheckStatus.FAIL,
                "JWT rotation not active",
                cwe="CWE-613",
                phase="P11",
            )
    ...
    def check_api_versioning(self, metrics):
        if not metrics.get("v1_sunset_enforced", True):
            r = CheckResult(
                CheckDomain.API_VERSIONING,
                "v1_sunset_not_enforced",
                CheckStatus.WARN,
                "V1 sunset not enforced",
                phase="P26",
            )
        elif not metrics.get("response_migration_active", True):
            r = CheckResult(
                CheckDomain.API_VERSIONING,
                "response_migration_inactive",
                CheckStatus.FAIL,
                "Response migration not active",
                phase="P26",
            )
        elif not metrics.get("deprecation_headers_sent", True):
            r = CheckResult(
                CheckDomain.API_VERSIONING,
                "deprecation_headers_missing",
                CheckStatus.WARN,
            ...
            r = CheckResult(
                ...
                "api_versioning",
                CheckStatus.PASS,
                "V1 sunset + migration + headers OK",
                phase="P26",
            )
        self._emit(r)
        return r
    ...
    def check_feature_flags(self, metrics):
        if not metrics.get("kill_override_active", True):
            r = CheckResult(
                CheckDomain.FEATURE_FLAGS,
                "kill_override_missing",
                CheckStatus.FAIL,
                "Kill override not implemented",
                phase="P24",
            )
        elif not metrics.get("flag_changes_audited", True):
            r = CheckResult(
                CheckDomain.FEATURE_FLAGS,
                "flag_changes_not_audited",
                CheckStatus.FAIL,
                "Flag changes not audited",
                phase="P24",
            )
        elif not metrics.get("tenant_scoped", True):
            r = CheckResult(
                CheckDomain.FEATURE_FLAGS,
                "flags_not_tenant_scoped",
                CheckStatus.WARN,
                "Flags not tenant-scoped",
                phase="P24",
            )
        else:
            r = CheckResult(
    ...
    def check_supply_chain(self, metrics):
        if not metrics.get("dependencies_pinned", True):
            ...
            r = CheckResult(
                CheckDomain.SUPPLY_CHAIN,
                "lockfile_not_verified",
                CheckStatus.FAIL,
                "Lockfile not verified",
                phase="P28",
            )
        elif not metrics.get("no_critical_vulns", True):
            r = CheckResult(
                CheckDomain.SUPPLY_CHAIN,
                "critical_vulns_found",
                CheckStatus.FAIL,
                "Critical CVEs found",
                cwe="CWE-1357",
                phase="P28",
    ...
    def check_secret_rotation(self, metrics):
        if not metrics.get("rotation_policy_active", True):
            r = CheckResult(
                CheckDomain.SECRET_ROTATION,
                "rotation_policy_inactive",
                CheckStatus.FAIL,
                "Rotation policy not active",
                cwe="CWE-320",
                phase="P29",
            )
            ...
            r = CheckResult(
                ...
                CheckStatus.FAIL,
                "Grace period not configured",
                phase="P29",
            )
        elif not metrics.get("compromise_plan_ready", True):
            r = CheckResult(
                CheckDomain.SECRET_ROTATION,
                "compromise_plan_missing",
                CheckStatus.WARN,
                "Compromise plan missing",
                phase="P29",
            )
        else:
            r = CheckResult(
    ...
    def check_customer_lifecycle(self, metrics):
        if not metrics.get("onboarding_flow_active", True):
            r = CheckResult(
                CheckDomain.CUSTOMER_LIFECYCLE,
                "onboarding_not_automated",
                CheckStatus.WARN,
                "Onboarding not automated",
                phase="P32",
            )
        elif not metrics.get("renewal_reminders_active", True):
            r = CheckResult(
                CheckDomain.CUSTOMER_LIFECYCLE,
                "renewal_reminders_inactive",
                CheckStatus.WARN,
                "Renewal reminders inactive",
                phase="P32",
            )
        elif not metrics.get("heartbeat_fail_notify", True):
            r = CheckResult(
                CheckDomain.CUSTOMER_LIFECYCLE,
                "heartbeat_fail_not_notified",
                CheckStatus.WARN,
                "Heartbeat fail not notified",
                phase="P32",
            )
        else:
            r = CheckResult(
                ...
                "customer_lifecycle",
                CheckStatus.PASS,
                "Onboarding + renewal + heartbeat OK",
                phase="P32",
            )
        self._emit(r)
        return r
    ...
    def check_security_hardening(self, metrics):
        if not metrics.get("no_critical_findings", True):
            r = CheckResult(
                CheckDomain.SECURITY_HARDENING,
            ...
            r = CheckResult(
                CheckDomain.SECURITY_HARDENING,
                "replay_protection_inactive",
                CheckStatus.FAIL,
                "Replay protection inactive",
                cwe="CWE-294",
                phase="P34",
            )
        else:
...
class MigrationVerifyResult:
    ...
    def is_ok(self) -> bool:
        return self.status == MigrationStatus.VERIFIED


class MigrationVerifier:
    EXPECTED_MIGRATIONS = [
        ("001", "initial_schema", "P01"),
        ("002", "partitioning", "P02"),
        ("003", "missing_tables", "P03"),
        ("004", "stabilization", "P04"),
        ("005", "phase3_dedup", "P03"),
        ("006", "ml_realism", "P06"),
        ("007", "phase6_backtest", "P06"),
        ("008", "phase7_execution", "P07"),
        ("009", "phase8_db_hardening", "P08"),
        ("010", "phase9_observability", "P09"),
        ("026", "phase10_billing", "P10"),
        ("027", "phase13_saas_schema", "P13"),
        ("028", "phase19_rls_tenant", "P19"),
        ("029", "phase21_audit_chain", "P21"),
        ("030", "phase22_rate_limit", "P22"),
        ("031", "phase23_backup_dr", "P23"),
        ("032", "phase24_observability", "P24"),
        ("033", "phase24_feature_flags", "P24"),
        ("034", "phase25_artifact", "P25"),
        ("035", "phase26_api_versioning", "P26"),
        ("036", "phase27_integration", "P27"),
        ("037", "phase28_supply_chain", "P28"),
        ("038", "phase29_secret_rotation", "P29"),
        ("039", "phase30_compliance", "P30"),
        ("040", "phase31_analytics", "P31"),
        ("041", "phase32_lifecycle", "P32"),
        ("042", "phase33_support", "P33"),
        ("043", "phase34_security", "P34"),
    ]
    ...
    def _build_expected(self):
        for seq, name, phase in self.EXPECTED_MIGRATIONS:
            rec = MigrationRecord(
                seq=int(seq), filename=f"migration_{seq}_{name}.sql", phase=phase, checksum=""
            )
    ...
    def verify(self, actual_filenames) -> MigrationVerifyResult:
        ...
        verified = total - len(missing) - len(checksum_fails)
        status = (
            MigrationStatus.VERIFIED
            if not missing and not checksum_fails and not out_of_order
            else MigrationStatus.MISSING
            if missing
            else MigrationStatus.OUT_OF_ORDER
            if out_of_order
...
class RollbackPlanner:
    ROLLBACK_PLANS = {
        "P24_FEATURE_FLAGS": {
            "steps": [
                RollbackStep(
                    ...
                    "Set kill override",
                    True,
                    "curl /api/feature-flags/health",
                ),
                RollbackStep(
                    2, "REVERT_DB_MIGRATION", "Run migration_033 DOWN", False, "psql check"
                ),
                RollbackStep(
                    3, "DEPLOY_PREV_VERSION", "Deploy previous image", True, "curl /api/health"
                ),
                RollbackStep(
                    4, "VERIFY_FLAGS_ABSENT", "Confirm 404", True, "curl /api/feature-flags"
                ),
            ],
            "estimated_mins": 10,
        ...
        "P26_API_VERSIONING": {
            "steps": [
                RollbackStep(
                    ...
                    True,
                    "curl -H Accept-Version:v2 /api/signals",
                ),
                RollbackStep(
                    2, "REVERT_DB_MIGRATION", "Run migration_035 DOWN", False, "psql check"
                ),
        ...
        "P27_INTEGRATION_SECURITY": {
            "steps": [
                RollbackStep(
                    1, "DISABLE_SIG_VERIFY", "Set SKIP_SIGNATURE=1", True, "curl /api/webhook/test"
                ),
                RollbackStep(
                    2, "OPEN_CIRCUIT_BREAKERS", "Reset circuit breakers", True, "curl /api/health"
                ),
        ...
        "P29_SECRET_ROTATION": {
            "steps": [
                RollbackStep(
                    1, "PAUSE_ROTATION", "Set ROTATION_PAUSED=1", True, "curl /api/health/keys"
                ),
                RollbackStep(
                    2, "RESTORE_PREV_KEYS", "Load from vault", False, "curl /api/auth/token/test"
                ),
        ...
        "P30_COMPLIANCE": {
            "steps": [
                RollbackStep(
                    1, "DISABLE_CONSENT_GATE", "Set REQUIRE_CONSENT=0", True, "curl /api/health"
                ),
    ...
    def __init__(self, audit=None):
        ...
        for key, cfg in self.ROLLBACK_PLANS.items():
            self._plans[key] = RollbackPlan(
                phase=key,
                steps=cfg["steps"],
                estimated_mins=cfg["estimated_mins"],
                requires_downtime=cfg["requires_downtime"],
            )

    def get_plan(self, phase):
        return self._plans.get(phase)
    ...
    def trigger_rollback(self, phase, actor, reason):
        if not reason or not reason.strip():
            raise MissingReasonError("reason mandatory")
        plan = self._plans.get(phase)
        if plan is None:
            raise ValueError(f"No plan for {phase}")
        if self._audit is not None:
            self._audit.record(
                "ROLLBACK_TRIGGERED", actor, phase, reason=reason, detail={"steps": len(plan.steps)}
            )
        return plan
    ...
    def verify_rollback(self, phase):
        plan = self._plans.get(phase)
        if plan is None:
            return {"has_plan": False, "all_reversible": False}
        return {
            "has_plan": True,
...
class StagingSignOff:
    ...
    def submit(self, approver, role, status, comment=None, reason=None):
        ...
        if self._audit is not None:
            action = (
                "SIGN_OFF_APPROVED" if status == SignOffStatus.APPROVED else "SIGN_OFF_REJECTED"
            )
            self._audit.record(
                action, approver, "staging_sign_off", reason=reason, detail={"role": role}
            )
        return rec
    ...
    def is_quorum_met(self):
        with self._lock:
            return (
                sum(1 for r in self._records.values() if r.status == SignOffStatus.APPROVED)
                >= self._quorum
            )
    ...
    def get_summary(self):
        ...
        pending = [role for role in self.REQUIRED_ROLES if not any(r.role == role for r in records)]
        return {
            "total": len(records),
            "approved": approved,
            "rejected": rejected,
            "quorum": self._quorum,
            "quorum_met": approved >= self._quorum,
...
class ReleaseRiskRegister:
    ...
    def _seed_known_risks(self):
        known = [
            RiskItem(
                "R001",
                "CSP unsafe-inline in dynamic pages",
            ...
            RiskItem(
                "R002",
                "Rate limit IP trust with load balancer",
            ...
            RiskItem(
                "R005",
                "Service-to-service RBAC not enforced",
            ...
            RiskItem(
                "R006",
                "GDPR right-to-erasure not automated",
            ...
            RiskItem(
                "R008",
                "Backup restore not tested in prod-equivalent env",
    ...
    def mitigate(self, risk_id, actor, reason, mitigation=""):
        ...
        with self._lock:
            ...
            risk.mitigation = mitigation or reason
            risk.ts_closed = time.time()
        if self._audit is not None:
            self._audit.record(
                "RISK_MITIGATED", actor, risk.domain, reason=reason, detail={"risk_id": risk_id}
            )
        return risk
    ...
    def accept(self, risk_id, actor, reason):
        ...
        if self._audit is not None:
            self._audit.record(
                "RISK_ACCEPTED", actor, risk.domain, reason=reason, detail={"risk_id": risk_id}
            )
        return risk
    ...
    def summary(self):
        with self._lock:
            all_items = list(self._risks.values())
...
class GoNoGoResult:
    ...
    open_critical_risks: list
    sign_off_summary: dict
    migration_ok: bool
    audit_chain_ok: bool
    recommendation: str
    ...
    def to_dict(self):
        return {
            ...
            "sign_off": self.sign_off_summary,
            "migration_ok": self.migration_ok,
            "audit_ok": self.audit_chain_ok,
...
class GoNoGoEngine:
    ...
    def evaluate(self, checks, migration_result, sign_off, risk_register, audit_chain=None):
        reasons = []
        blocking_checks = [c for c in checks if c.is_blocking()]
        ...
        if sign_off.has_rejection():
            reasons.append("sign_off_rejected")
        if not sign_off_summary["quorum_met"]:
            reasons.append(
                f"sign_off_quorum_not_met: {sign_off_summary['approved']}/{sign_off_summary['quorum']}"
            )
        if open_crits:
            reasons.append(f"open_critical_risks: {len(open_crits)}")
        if not audit_ok:
            reasons.append("audit_chain_tampered")
        ...
        if reasons:
            ...
            rec = (
                "All checks PASS. Quorum met. No critical risks. Audit intact.\n"
        ...
        result = GoNoGoResult(
            ...
            open_critical_risks=open_crits,
...
class ReleaseReportBuilder:
    ...
    def format_text(self, report):
        ...
        decision_line = (
            "[GO] GO - APPROVED FOR PRODUCTION"
            if d == Decision.GO
            else "[COND] CONDITIONAL GO"
            if d == Decision.CONDITIONAL_GO
        ...
        lines += [
            f"  Total: {rs.get('total', 0)}  Open: {rs.get('open', 0)}  Mitigated: {rs.get('mitigated', 0)}"
        ]
        lines += [
            "=" * 72,
            "PROJECT STATISTICS",
...
def build_release_gate(
    secret="release-gate-secret-v35", company="Bot12 Technologies Ltd", quorum=2
):
