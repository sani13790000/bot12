"""backend/core/config.py v7 - Round 6 Restored + Extended
Restored from last known-good commit (94dc074f24fb)
Plus all Round 5 fixes:
  - REDIS_PASSWORD + REDIS_URL_WITH_AUTH property
  - MT5_GATEWAY_URL field
  - TELEGRAM_ADMIN_IDS field
  - 18 missing env vars from .env.example
BUG-R6: Restored from CONFIG_CONTENT_PLACEHOLDER
"""
...
class Settings(BaseSettings):
    ...
    REDIS_PASSWORD: str = ""  # BUG-R5 FIX: was missing
    REDIS_MAX_CONNECTIONS: int = Field(default=10, ge=1, le=100)

    @property
    def REDIS_URL_WITH_AUTH(self) -> str:
        """BUG-R5 FIX: Inject password into Redis URL if provided."""
    ...
    MT5_GATEWAY_URL: str = "http://localhost:8080"  # ARCH-R4 FIX
    ...
    TELEGRAM_ADMIN_IDS: List[int] = []  # BUG-R5 FIX
    ...
    def _validate_jwt_secret(cls, v: str) -> str:
        ...
        if v.lower() in _DANGEROUS_SECRETS:
            log.warning(
                "[config] JWT_SECRET_KEY is weak. Set a strong secret for production."
            )
        return v
...
# Module-level singleton for `from .config import settings`
