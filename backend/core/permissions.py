    ...
    Role.ADMIN: frozenset(
        {
            ...
            P.BILLING_CHECKOUT,
            P.BILLING_REFUND,
            P.BILLING_MANAGE_PLANS,
            P.BILLING_WEBHOOK_INGEST,
            P.RISK_READ_OWN,
            P.RISK_READ_ANY,
            P.RISK_HALT,
            P.RISK_RESUME,
            ...
            P.SETTINGS_WRITE,
            P.METRICS_READ,
            P.METRICS_PROMETHEUS,
            P.ALERT_MANAGE,
            P.TRACE_READ,
            P.TENANT_READ_OWN,
            P.TENANT_READ_ANY,
            P.TENANT_MANAGE,
            P.TENANT_CROSS_ACCESS,
            P.RELEASE_DOWNLOAD,
            P.RELEASE_PUBLISH,
            P.RELEASE_REVOKE,
        }
...
PLAN_PERMISSIONS: Dict[str, FrozenSet[str]] = {
    ...
    "pro": frozenset(
        ROLE_PERMISSIONS[Role.CUSTOMER]
        if False
        else {
            P.PROFILE_READ_OWN,
            P.PROFILE_WRITE_OWN,
...
ENDPOINT_REGISTRY: List[EndpointSpec] = [
    ...
    EndpointSpec(
        "POST", "/api/v1/license/{lic_id}/transfer", P.LICENSE_TRANSFER, "Transfer license"
    ),
    ...
    EndpointSpec(
        "POST", "/api/v1/trades/{trade_id}/cancel", P.TRADE_CANCEL_OWN, "Cancel own trade", True
    ),
    ...
    EndpointSpec(
        "POST", "/api/v1/signals/{sig_id}/cancel", P.SIGNAL_CANCEL_OWN, "Cancel own signal", True
    ),
...
class RBACEngineV2:
    ...
    def check_resource(self, ctx: AuthContext, perm: str, owner_id: Optional[str]) -> bool:
        ...
        if not result:
            _log.warning(
                "RBAC owner-deny: user=%s role=%s perm=%s", ctx.user_id[:8], ctx.role, perm
            )
        return result
