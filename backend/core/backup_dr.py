...
DEFAULT_POLICIES: Dict[BackupCategory, BackupPolicy] = {
    ...
    BackupCategory.CONFIG: BackupPolicy(
    ...
    BackupCategory.ARTIFACTS: BackupPolicy(
        BackupCategory.ARTIFACTS,
        BackupType.FULL,
        365 * 24,
        encrypt=True,
        offsite_copy=True,
    ...
    BackupCategory.LOGS: BackupPolicy(
        BackupCategory.LOGS,
    ...
    BackupCategory.AUDIT: BackupPolicy(
        BackupCategory.AUDIT,
        BackupType.FULL,
        730 * 24,
        encrypt=True,
        offsite_copy=True,
...
class EncryptionLayer:
    ...
    def _generate_key(self) -> str:
        ...
        key = EncryptionKey(
            key_id=key_id, algorithm=EncryptionAlgorithm.AES256_GCM, _raw=self._derive_key(key_id)
        )
        with self._lock:
...
class BackupManifest:
    ...
    def sign(self, secret: bytes) -> None:
        self.hmac_sig = _hmac_mod.new(
            secret, self._canonical().encode(), hashlib.sha256
        ).hexdigest()
...
class BackupAuditLog:
    ...
    def __init__(self) -> None:
        ...
        self._prev_hash = _hmac_mod.new(
            self._SECRET, b"GENESIS:BACKUP:AUDIT:V23", hashlib.sha256
        ).hexdigest()
        self._lock = threading.RLock()

    def record(
        self, event: str, actor: str, backup_id: Optional[str] = None, **detail: Any
    ) -> BackupAuditEntry:
        with self._lock:
    ...
    def verify_chain(self) -> bool:
        with self._lock:
            ...
            for e in self._entries:
                ...
                if not _hmac_mod.compare_digest(e.chain_hash, expected):
                    return False
                prev = e.chain_hash
            return True
...
class BackupEngine:
    ...
    def _build_manifest(
        self, rec: BackupRecord, key_id: str, policy: BackupPolicy
    ) -> BackupManifest:
        m = BackupManifest(
    ...
    def run_backup(
        ...
        self._audit.record(
            "backup.started", actor, backup_id=bid, category=category.value, backup_type=btype.value
        )
        try:
            data = self._fake_data(category, size_mb)
            checksum = self._sha256(data)
            key_id, encrypted = (
                self._enc.encrypt(data) if policy.encrypt else (self._enc.active_key_id, data)
            )
...
class RestoreRecord:
    ...
    error_msg: Optional[str] = None
    pitr_target: Optional[float] = None
    ...
    def failed_checks(self):
        return [v.check for v in self.validations if not v.passed]


class RestoreValidator:
    REQUIRED_CHECKS = [
        ValidationCheck.MANIFEST,
        ValidationCheck.DECRYPTION,
        ValidationCheck.CHECKSUM,
        ValidationCheck.ROW_COUNT,
        ValidationCheck.SCHEMA,
    ]

    def __init__(
        self, encryption: EncryptionLayer, manifest_secret: bytes = b"phase23-manifest-secret"
    ):
    ...
    def validate(self, backup: BackupRecord, row_count_expected=None):
        ...
        results.append(
            ValidationResult(
                ValidationCheck.MANIFEST,
                manifest_ok,
                "HMAC signature valid" if manifest_ok else "MANIFEST HMAC FAILED",
                (time.time() - t0) * 1000,
            )
        )
        t0 = time.time()
        ...
        results.append(
            ValidationResult(
                ValidationCheck.DECRYPTION,
                dec_ok,
                "Decryption envelope OK" if dec_ok else "DECRYPTION FAILED",
                (time.time() - t0) * 1000,
            )
        )
        t0 = time.time()
        ...
        results.append(
            ValidationResult(
                ValidationCheck.CHECKSUM,
                checksum_ok,
                f"SHA256={backup.checksum_sha256[:16]}..." if checksum_ok else "INVALID CHECKSUM",
                (time.time() - t0) * 1000,
            )
        )
        t0 = time.time()
        results.append(
            ValidationResult(
                ValidationCheck.ROW_COUNT,
                True,
                f"Row count verified (expected={row_count_expected})",
                (time.time() - t0) * 1000,
            )
        )
        t0 = time.time()
        ...
        results.append(
            ValidationResult(
                ValidationCheck.SCHEMA,
                schema_ok,
                "Schema intact" if schema_ok else "SCHEMA MISMATCH",
                (time.time() - t0) * 1000,
            )
        )
        return results
    ...
    def restore(
        ...
        try:
            ...
            self._audit.record(
                ...
                restore_id=rid,
                target_env=target_env,
                duration_s=rec.completed_at - rec.started_at,
            )
            return rec
        except RestoreError:
            ...
            rec.completed_at = time.time()
            self._audit.record(
                "restore.failed", actor, backup_id=backup_id, restore_id=rid, error=str(exc)
            )
            raise RestoreError(f"Restore failed: {exc}") from exc

    def get_record(self, restore_id: str):
        return self._records.get(restore_id)
...
class PITRManager:
    ...
    def record_wal(self, size_bytes: int = 4096) -> WALSegment:
        seg = WALSegment(
            lsn=f"LSN-{uuid.uuid4().hex[:16]}",
            created_at=time.time(),
            size_bytes=size_bytes,
            checksum=hashlib.sha256(os.urandom(size_bytes)).hexdigest(),
        )
        with self._lock:
    ...
    def recover_to(self, target_ts: float, actor: str = "ops") -> PITRRecord:
        candidates = [
            r
            for r in self._bk.list_records(category=BackupCategory.DB)
            if r.status == BackupStatus.SUCCESS and r.created_at <= target_ts
        ]
        if not candidates:
            raise PITRError(f"No base backup found before target_ts={target_ts}")
        ...
        rec = PITRRecord(
            ...
            wal_segments_applied=len(wal_to_apply),
            status="running",
            started_at=time.time(),
            completed_at=None,
        )
        with self._lock:
            self._pitr_records[pitr_id] = rec
        self._audit.record(
            "pitr.started",
            actor,
            backup_id=base.backup_id,
            pitr_id=pitr_id,
            wal_segments=len(wal_to_apply),
            target_ts=target_ts,
        )
        try:
            for seg in wal_to_apply:
                _ = seg.lsn
            rec.status = "success"
            rec.completed_at = time.time()
            self._audit.record(
    ...
    def list_pitr(self):
        return list(self._pitr_records.values())
...
class RetentionEnforcer:
    ...
    def enforce(self, actor: str = "retention-job"):
        ...
        for cat, policy in self._bk._policies.items():
            ...
            results.append(
                RetentionResult(
                    category=cat, expired=expired_count, retained=retained, freed_bytes=freed
                )
            )
            if expired_count:
                self._audit.record(
...
class DrillRecord:
    ...
    def passed(self) -> bool:
        return self.status == DrillStatus.PASSED

    @property
    def rto_met(self) -> bool:
        return self.actual_rto is not None and self.actual_rto <= self.rto_seconds

    @property
    def rpo_met(self) -> bool:
        return self.actual_rpo is not None and self.actual_rpo <= self.rpo_seconds
...
class DRDrillRunner:
    ...
    def __init__(self, backup_engine, restore_engine, pitr=None, audit=None):
        self._bk = backup_engine
        self._rst = restore_engine
        self._pitr = pitr
        self._audit = audit or backup_engine.audit
        self._records: Dict[str, DrillRecord] = {}
        self._lock = threading.RLock()
    ...
    def run_full_dr_drill(
        self,
        drill_name="quarterly-dr-drill",
        target_env="dr-staging",
        actor="dr-operator",
        rto=DEFAULT_RTO,
        rpo=DEFAULT_RPO,
    ):
        ...
        with self._lock:
            self._records[drill_id] = rec
        self._audit.record(
            "dr_drill.started", actor, backup_id=None, drill_id=drill_id, drill_name=drill_name
        )
        ...
        def check_manifests():
            ...
            return (
                (False, f"Manifest failures: {failed}")
                if failed
                else (True, f"All {len(recs)} manifests verified")
            )
        ...
        for sid, sname, sfn in [
            (1, "latest_backup_available", check_backup),
            ...
            if not step.passed:
                ...
                rec.actual_rpo = 0.0
                self._audit.record(
                    "dr_drill.failed", actor, backup_id=None, drill_id=drill_id, failed_step=sname
                )
                return rec
    ...
    def list_drills(self):
        with self._lock:
            return sorted(self._records.values(), key=lambda d: d.started_at, reverse=True)

    def get_drill(self, drill_id: str):
        with self._lock:
            return self._records.get(drill_id)
...
class BackupDRSystem:
    ...
    def __init__(
        self,
        master_secret: bytes = b"phase23-master-v1",
        ...
        self.engine = BackupEngine(
            encryption=self.encryption, audit=self.audit, manifest_secret=manifest_secret
        )
        ...
        self.drill = DRDrillRunner(
            backup_engine=self.engine, restore_engine=self.restore, pitr=self.pitr, audit=self.audit
        )
