...
P11-CFG-1: SECRETS_MASTER_KEY required در production
P11-CFG-2: ALLOWED_ORIGINS strict — نه فقط wildcard check
P11-CFG-3: CORS allow_credentials=True فقط با explicit origins
P11-CFG-4: Content-Security-Policy config
P11-CFG-5: LOG_REDACTION_ENABLED flag
P11-CFG-6: ENCRYPTION_AT_REST_KEY برای DB field encryption
P11-CFG-7: SESSION_COOKIE_SECURE, SAMESITE flags
P11-CFG-8: Webhook secrets (Stripe, ZarinPal)
"""
...
class Settings(BaseSettings):
    ...
    def _validate_origins(cls, v: List[str]) -> List[str]:
        """P11-CFG-2: No wildcard, valid URL format in staging/production."""
        env = os.environ.get("ENVIRONMENT", "development").lower()
