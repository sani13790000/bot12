...
class RefreshTokenRotationStore:
    ...
    def _oldest_session_hash(self, user_id: str) -> Optional[str]:
        now = time.time()
        active = [r for r in self._store.values()
                  if r.user_id == user_id and not r.used and r.expires_at > now]
        if not active:
            return None
    ...
    def issue(self, user_id: str, family_id: Optional[str] = None) -> str:
        ...
        self._store[th] = _RTRecord(
            token_hash=th, user_id=user_id, family_id=fid,
            issued_at=now, expires_at=now + self._ttl_days * 86400,
        )
    ...
    def rotate(self, old_token: str) -> Optional[str]:
        ...
        if now > record.expires_at:
            ...
            return None
        if record.used:
            self._log("REUSE_DETECTED", record.user_id,
                      f"family={record.family_id[:8]} -- revoking all")
    ...
    def _revoke_family(self, user_id: str, family_id: str) -> None:
        to_del = [th for th, r in self._store.items()
                  if r.user_id == user_id and r.family_id == family_id]
        for th in to_del:
            del self._store[th]
    ...
    def get_audit(self, user_id: Optional[str] = None, limit: int = 100) -> List[dict]:
        ...
        return [{"ts": e.ts, "event": e.event, "user_id": e.user_id, "detail": e.detail}
