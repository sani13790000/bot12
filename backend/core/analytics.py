...
class MissingReasonError(AnalyticsError):
    pass


class AuditChainTampered(AnalyticsError):
    pass
...
class AnalyticsAuditChain:
    ...
    def query(self, action=None, kpi_key: str = "", limit: int = 50) -> List[_AuditEntry]:
        ...
        if action is not None:
            res = [e for e in res if e.action == str(action)]
        if kpi_key:
            res = [e for e in res if e.kpi_key == kpi_key]
...
class KPIStore:
    ...
    def record(self, point: KPIDataPoint) -> None:
        with self._lock:
            self._data[f"{point.tenant_id}:{point.kpi_key}"].append(point)

    def latest(self, kpi_key: str, tenant_id: str = "global", n: int = 1) -> List[KPIDataPoint]:
        with self._lock:
            data = list(self._data[f"{tenant_id}:{kpi_key}"])
        data.sort(key=lambda p: p.recorded_at, reverse=True)
        return data[:n]

    def series(
        self, kpi_key: str, tenant_id: str = "global", since: float = 0.0, limit: int = 100
    ) -> List[KPIDataPoint]:
        with self._lock:
            data = list(self._data[f"{tenant_id}:{kpi_key}"])
...
class KPIEngine:
    ...
    def _record(self, kpi_key, value, tenant_id, meta):
        cat_map = {
            "revenue": KPICategory.REVENUE,
            "subscription": KPICategory.SUBSCRIPTION,
            "churn": KPICategory.CHURN,
            "payment": KPICategory.PAYMENT,
            "risk": KPICategory.RISK,
            "heartbeat": KPICategory.HEARTBEAT,
            "fraud": KPICategory.FRAUD,
            "license": KPICategory.LICENSE,
            "system": KPICategory.SYSTEM,
        }
        category = cat_map.get(kpi_key.value.split(".")[0], KPICategory.SYSTEM)
        pt = KPIDataPoint(
            id=str(uuid.uuid4()),
            kpi_key=kpi_key.value,
        ...
        if self._audit is not None:
            self._audit.record(
                AuditAction.KPI_RECORDED, kpi_key=kpi_key.value, value=value, tenant_id=tenant_id
            )
        return pt

    def record_mrr(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.MRR, amount, tenant_id, meta)

    def record_arr(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.ARR, amount, tenant_id, meta)
    ...
    def record_nrr(self, rate, tenant_id="global", **meta):
        return self._record(KPIKey.NRR, rate, tenant_id, meta)

    def record_arpu(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.ARPU, amount, tenant_id, meta)

    def record_ltv(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.LTV, amount, tenant_id, meta)

    def record_cac(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.CAC, amount, tenant_id, meta)
    ...
    def compute_ltv_cac_ratio(self, tenant_id="global"):
        ...
        if not ltv or not cac or cac[0].value == 0:
            return None
        return round(ltv[0].value / cac[0].value, 2)

    def record_active_subs(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.ACTIVE_SUBS, float(count), tenant_id, meta)

    def record_new_subs(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.NEW_SUBS, float(count), tenant_id, meta)

    def record_cancelled_subs(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.CANCELLED_SUBS, float(count), tenant_id, meta)

    def record_trial_conversion(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.TRIAL_CONVERSIONS, float(count), tenant_id, meta)

    def record_plan_upgrade(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.PLAN_UPGRADES, float(count), tenant_id, meta)

    def record_plan_downgrade(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.PLAN_DOWNGRADES, float(count), tenant_id, meta)

    def record_churn_rate(self, rate, tenant_id="global", **meta):
        return self._record(KPIKey.CHURN_RATE, rate, tenant_id, meta)

    def record_revenue_churn(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.REVENUE_CHURN, amount, tenant_id, meta)
    ...
    def record_payment_success_rate(self, rate, tenant_id="global", **meta):
        return self._record(KPIKey.PAYMENT_SUCCESS_RATE, rate, tenant_id, meta)

    def record_payment_failure(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.PAYMENT_FAILURES, float(count), tenant_id, meta)

    def record_payment_recovery(self, amount, tenant_id="global", **meta):
        return self._record(KPIKey.PAYMENT_RECOVERY, amount, tenant_id, meta)

    def record_dunning_active(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.DUNNING_ACTIVE, float(count), tenant_id, meta)

    def record_risk_block(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.RISK_BLOCKS, float(count), tenant_id, meta)

    def record_kill_switch_event(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.KILL_SWITCH_EVENTS, float(count), tenant_id, meta)

    def record_drawdown_breach(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.DRAWDOWN_BREACHES, float(count), tenant_id, meta)

    def record_heartbeat_miss(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.HEARTBEAT_MISS, float(count), tenant_id, meta)

    def record_heartbeat_ok(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.HEARTBEAT_OK, float(count), tenant_id, meta)

    def record_fraud_flag(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.FRAUD_FLAGS, float(count), tenant_id, meta)

    def record_abuse_ban(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.ABUSE_BANS, float(count), tenant_id, meta)

    def record_replay_attack(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.REPLAY_ATTACKS, float(count), tenant_id, meta)

    def record_auth_failure(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.AUTH_FAILURES, float(count), tenant_id, meta)

    def record_license_issued(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.LICENSE_ISSUED, float(count), tenant_id, meta)

    def record_license_revoked(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.LICENSE_REVOKED, float(count), tenant_id, meta)

    def record_license_active(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.LICENSE_ACTIVE, float(count), tenant_id, meta)

    def record_license_expired(self, count=1, tenant_id="global", **meta):
        return self._record(KPIKey.LICENSE_EXPIRED, float(count), tenant_id, meta)

    def record_api_error_rate(self, rate, tenant_id="global", **meta):
        return self._record(KPIKey.API_ERROR_RATE, rate, tenant_id, meta)

    def record_p95_latency(self, ms, tenant_id="global", **meta):
        return self._record(KPIKey.P95_LATENCY_MS, ms, tenant_id, meta)

    def record_active_tenants(self, count, tenant_id="global", **meta):
        return self._record(KPIKey.ACTIVE_TENANTS, float(count), tenant_id, meta)
...
class AnomalyDetector:
    ...
    def set_threshold(self, kpi_key, warn, crit, actor="admin", reason=""):
        with self._lock:
            self._thresholds[kpi_key] = {"warn": warn, "crit": crit}
        if self._audit is not None:
            self._audit.record(
                AuditAction.THRESHOLD_SET,
                actor=actor,
                kpi_key=kpi_key,
                reason=reason,
                warn=warn,
    ...
    def feed(self, kpi_key, value, tenant_id="global"):
        ...
        with self._lock:
            hist = list(self._history[tk])
            self._history[tk].append(value)
        anomaly = self._check_zscore(kpi_key, value, hist, tenant_id)
        if anomaly is None:
            anomaly = self._check_threshold(kpi_key, value, tenant_id)
        if anomaly is not None:
            with self._lock:
                self._events.append(anomaly)
            if self._audit is not None:
                self._audit.record(
                    AuditAction.ANOMALY_RAISED,
                    kpi_key=kpi_key,
    ...
    def _check_zscore(self, kpi_key, value, hist, tenant_id):
        ...
        return AnomalyEvent(
            ...
            message=f"Z-score {z:.2f} on {kpi_key}",
        )
    ...
    def _check_threshold(self, kpi_key, value, tenant_id):
        ...
        if th is None:
            return None
        crit = th.get("crit", float("inf"))
        warn = th.get("warn", float("inf"))
        if value >= crit:
    ...
    def check_flatline(self, kpi_key, last_seen_ts, expected_interval_s, tenant_id="global"):
        ...
        ev = AnomalyEvent(
            ...
            tenant_id=tenant_id,
            detected_at=time.time(),
            message=f"Flatline on {kpi_key}: no data for {age:.0f}s",
        )
        with self._lock:
    ...
    def recent_anomalies(self, limit=20, severity=None, kpi_key=""):
        ...
        if kpi_key:
            events = [e for e in events if e.kpi_key == kpi_key]
        return list(reversed(events))[:limit]

    def reset_baseline(self, kpi_key, tenant_id="global", actor="admin", reason=""):
        tk = f"{tenant_id}:{kpi_key}"
        with self._lock:
            self._history[tk].clear()
        if self._audit is not None:
            self._audit.record(
                AuditAction.BASELINE_RESET,
                actor=actor,
                kpi_key=kpi_key,
                reason=reason,
                tenant_id=tenant_id,
...
class BusinessHealthChecker:
    GATES = [
        {
            ...
            "kpi": KPIKey.PAYMENT_SUCCESS_RATE,
            "warn": 95.0,
            "crit": 85.0,
    ...
    def check(self, tenant_id="global", actor="admin"):
        ...
        for g in self.GATES:
            kpi = g["kpi"]
            pts = self._store.latest(kpi.value, tenant_id, n=1)
            if not pts:
                gates.append(
                    HealthGate(
                        name=g["name"],
        ...
        overall = (
            ...
            if critical > 0
            else (HealthStatus.DEGRADED if warning > 0 else HealthStatus.HEALTHY)
        )
        report = BusinessHealthReport(
            ...
            tenant_id=tenant_id,
            anomalies=self._detector.recent_anomalies(limit=10),
        )
        if self._audit is not None:
            self._audit.record(
                AuditAction.DASHBOARD_READ,
                actor=actor,
                kpi_key="health_check",
    ...
    def to_dict(self):
        return {
            ...
            "health": {
                ...
                "critical_count": self.health.critical_count,
...
class AdminDashboard:
    ...
    def snapshot(self, tenant_id="global", actor="admin"):
        ...
        snap = DashboardSnapshot(
            tenant_id=tenant_id,
            period=period,
            mrr=self._get(KPIKey.MRR, tenant_id),
            arr=self._get(KPIKey.ARR, tenant_id),
            nrr=self._get(KPIKey.NRR, tenant_id),
            active_subs=int(self._get(KPIKey.ACTIVE_SUBS, tenant_id)),
            new_subs=int(self._get(KPIKey.NEW_SUBS, tenant_id)),
            churn_rate=self._get(KPIKey.CHURN_RATE, tenant_id),
        ...
        if self._audit is not None:
            self._audit.record(
                AuditAction.KPI_SNAPSHOT,
                actor=actor,
                kpi_key="dashboard_snapshot",
                tenant_id=tenant_id,
                mrr=snap.mrr,
    ...
    def mrr_trend(self, tenant_id="global", periods=6):
        return [
            {"period": p.period, "mrr": p.value, "ts": p.recorded_at}
            for p in self._store.series(KPIKey.MRR.value, tenant_id, limit=periods)
        ]

    def churn_trend(self, tenant_id="global", periods=6):
        return [
            {"period": p.period, "churn_rate": p.value, "ts": p.recorded_at}
            for p in self._store.series(KPIKey.CHURN_RATE.value, tenant_id, limit=periods)
        ]
    ...
    def fraud_summary(self, tenant_id="global"):
        return {
            "fraud_flags": int(self._get(KPIKey.FRAUD_FLAGS, tenant_id)),
            "abuse_bans": int(self._get(KPIKey.ABUSE_BANS, tenant_id)),
            "replay_attacks": int(self._get(KPIKey.REPLAY_ATTACKS, tenant_id)),
            "auth_failures": int(self._get(KPIKey.AUTH_FAILURES, tenant_id)),
            "anomalies": [
                e
                for e in self._detector.recent_anomalies(limit=20)
                if e.kpi_key
                in (
                    KPIKey.FRAUD_FLAGS.value,
                    KPIKey.ABUSE_BANS.value,
                    KPIKey.REPLAY_ATTACKS.value,
                    KPIKey.AUTH_FAILURES.value,
                )
            ],
        }
    ...
    def risk_summary(self, tenant_id="global"):
        return {
            "risk_blocks": int(self._get(KPIKey.RISK_BLOCKS, tenant_id)),
            "kill_switch_events": int(self._get(KPIKey.KILL_SWITCH_EVENTS, tenant_id)),
            "drawdown_breaches": int(self._get(KPIKey.DRAWDOWN_BREACHES, tenant_id)),
            "heartbeat_miss": int(self._get(KPIKey.HEARTBEAT_MISS, tenant_id)),
        }
    ...
    def payment_summary(self, tenant_id="global"):
        return {
            "payment_success_rate": self._get(KPIKey.PAYMENT_SUCCESS_RATE, tenant_id),
            "payment_failures": int(self._get(KPIKey.PAYMENT_FAILURES, tenant_id)),
            "dunning_active": int(self._get(KPIKey.DUNNING_ACTIVE, tenant_id)),
            "payment_recovery": self._get(KPIKey.PAYMENT_RECOVERY, tenant_id),
        }
    ...
    def multi_tenant_summary(self):
        return [
            {
                "tenant_id": t,
                "mrr": self._get(KPIKey.MRR, t),
                "active_subs": int(self._get(KPIKey.ACTIVE_SUBS, t)),
                "churn_rate": self._get(KPIKey.CHURN_RATE, t),
            }
            for t in self._store.all_tenants()
            if t != "global"
        ]
...
def build_analytics_system(secret="", zscore_threshold=3.0, thresholds=None):
    ...
    detector = AnomalyDetector(
        zscore_threshold=zscore_threshold, thresholds=thresholds, audit=audit
    )
    health_checker = BusinessHealthChecker(store=store, detector=detector, audit=audit)
    dashboard = AdminDashboard(
        store=store, engine=engine, detector=detector, health_checker=health_checker, audit=audit
    )
    return {
        "audit": audit,
        "store": store,
