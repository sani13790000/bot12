        ...
        **detail: Any,
        ...
        if action in REQUIRES_REASON:
            if not reason or not reason.strip():
                raise MissingReasonError(f"reason is mandatory for action={action.value}")
        ts_now = time.time()
        with self._lock:
        ...
        with self._lock:
            ...
            for e in reversed(self._entries):
                if actor and e.actor != actor:
                    continue
                if action and e.action != action.value:
                    continue
    ...
    def start_session(
        ...
        *,
        ...
        if not role.can_impersonate:
            ...
            raise InsufficientRoleError(f"role={role.value} cannot impersonate; ADMIN required")
        if not reason or not reason.strip():
        ...
        with self._lock:
            for s in self._sessions.values():
                if s.actor == actor and s.is_active:
                    raise ImpersonationDeniedError(f"agent {actor} already has an active session")
...
class DeviceResetManager:
    ...
    def reset_device(
        ...
        if not role.can_reset_device:
            if self._audit is not None:
                self._audit.record(
                    SupportAction.DEVICE_RESET_DENIED,
                    actor,
                    tenant_id,
                    device_id,
                    reason=f"insufficient_role:{role.value}",
    ...
    def _check_daily_limit(self, user_id: str, tenant_id: str) -> None:
        ...
        with self._lock:
            count = sum(
                1
                for r in self._records
                if r.user_id == user_id and r.tenant_id == tenant_id and r.ts >= cutoff
            )
...
class SubscriptionExtender:
    ...
    def __init__(
        self, audit: SupportAuditChain, *, auto_approve_max: int = 30, hard_max: int = 365
    ) -> None:
        self._audit = audit
    ...
    def extend(
        ...
        *,
        ...
        if not role.can_extend:
            if self._audit is not None:
                self._audit.record(
                    SupportAction.SUB_EXTEND_DENIED,
                    actor,
                    tenant_id,
                    user_id,
                    reason=f"insufficient_role:{role.value}",
        ...
        if days > self._auto_max and approved_by is None:
            if self._audit is not None:
                self._audit.record(
                    ...
                    tenant_id,
                    user_id,
                    reason=f"approval_required_for_days>{self._auto_max}",
                )
...
class ArtifactResender:
    ...
    def resend(
        self, actor: str, role: SupportRole, artifact_id: str, user_id: str, tenant_id: str
    ) -> ResendRecord:
        if not role.can_resend_artifact:
            if self._audit is not None:
                self._audit.record(
                    SupportAction.ARTIFACT_RESEND_DENIED,
                    actor,
                    tenant_id,
                    artifact_id,
                    reason=f"insufficient_role:{role.value}",
        ...
        if self._audit is not None:
            self._audit.record(
                SupportAction.ARTIFACT_RESENT,
                actor,
                tenant_id,
                artifact_id,
                reason="support_resend",
                user_id=user_id,
                resend_id=rec.resend_id,
            )
        return rec

    def history(
        self, *, user_id: Optional[str] = None, artifact_id: Optional[str] = None
    ) -> List[ResendRecord]:
        with self._lock:
            return [
                r
...
class AccountRecoveryManager:
    ...
    def start_recovery(
        self, actor: str, role: SupportRole, user_id: str, tenant_id: str, reason: str
    ) -> RecoveryCase:
        ...
        if self._audit is not None:
            self._audit.record(
                SupportAction.ACCOUNT_RECOVERY_START,
                actor,
                tenant_id,
                user_id,
                reason=reason,
    ...
    def advance_step(
        self, case_id: str, step: RecoveryStep, actor: str, *, note: str = ""
    ) -> RecoveryCase:
        with self._lock:
        ...
        if self._audit is not None:
            action = (
                SupportAction.ACCOUNT_RECOVERY_COMPLETE
                if step == RecoveryStep.COMPLETE
...
class SupportTicketEngine:
    ...
    def claim_ticket(self, ticket_id: str, actor: str, role: SupportRole) -> SupportTicket:
        ...
        if self._audit is not None:
            self._audit.record(
                SupportAction.TICKET_CLAIMED, actor, t.tenant_id, ticket_id, reason="agent_claim"
            )
        return t
    ...
    def resolve_ticket(self, ticket_id: str, actor: str, resolution: str) -> SupportTicket:
        ...
        if self._audit is not None:
            self._audit.record(
                SupportAction.TICKET_RESOLVED, actor, t.tenant_id, ticket_id, reason=resolution
            )
        return t

    def escalate_ticket(
        self, ticket_id: str, actor: str, escalated_to: str, reason: str
    ) -> SupportTicket:
        if not reason or not reason.strip():
            raise MissingReasonError("reason is mandatory for escalation")
        t = self._get_and_transition(ticket_id, TicketStatus.ESCALATED)
        with self._lock:
            t.escalated_to = escalated_to
            t.updated_at = time.time()
        if self._audit is not None:
            self._audit.record(
                SupportAction.TICKET_ESCALATED,
    ...
    def close_ticket(self, ticket_id: str, actor: str) -> SupportTicket:
        ...
        if self._audit is not None:
            self._audit.record(
                SupportAction.TICKET_CLOSED, actor, t.tenant_id, ticket_id, reason="closed"
            )
        return t
...
class SupportViewEngine:
    def __init__(
        self, audit: SupportAuditChain, *, data_store: Optional[Dict[str, Any]] = None
    ) -> None:
        self._audit = audit
    ...
    def get_customer_view(
        self, actor: str, role: SupportRole, user_id: str, tenant_id: str
    ) -> CustomerView:
        if not role.can_view:
            raise InsufficientRoleError("insufficient role")
        if self._audit is not None:
            self._audit.record(
                SupportAction.VIEW_ACCESSED,
                actor,
                tenant_id,
                user_id,
    ...
    def get_license_view(
        self, actor: str, role: SupportRole, license_id: str, tenant_id: str
    ) -> LicenseView:
        if not role.can_view:
            raise InsufficientRoleError("insufficient role")
        if self._audit is not None:
...
class SupportAdmin:
    def __init__(
        ...
        tickets: SupportTicketEngine,
        recovery: AccountRecoveryManager,
    ) -> None:
        self._audit = audit
...
def build_support_system(
    *,
    ...
    sessions = SupportSessionManager(
        audit, require_mfa=require_mfa, max_ttl=max_session_ttl, default_ttl=default_session_ttl
    )
