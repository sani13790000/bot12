...
REQUIRES_REASON = {
    InterventionKind.DEVICE_REVOKE,
    InterventionKind.ACCOUNT_SUSPEND,
    InterventionKind.ACCOUNT_RECOVER,
    InterventionKind.MFA_RESET,
    InterventionKind.IMPERSONATION_GRANT,
    InterventionKind.IMPERSONATION_START,
    InterventionKind.REFUND_MANUAL,
    InterventionKind.CREDIT_MANUAL,
    InterventionKind.SUBSCRIPTION_DOWNGRADE,
    InterventionKind.ARTIFACT_REISSUE,
}
...
class ImpersonationError(SupportError):
    pass


class InterventionDeniedError(SupportError):
    pass
...
class CustomerViewBuilder:
    ...
    def build(self, raw: Dict[str, Any]) -> CustomerView:
        return CustomerView(
            ...
            device_limit=raw.get("device_limit", 3),
...
class DeviceResetHandler:
    ...
    def reset(self, agent: SupportAgent, device_id: str, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if dev is None:
                raise InterventionDeniedError(f"Device {device_id} not found")
            dev["reset_count"] += 1
            dev["active"] = True
            result = {"device_id": device_id, "reset_count": dev["reset_count"], "status": "reset"}
            self._audit.record(
                kind=InterventionKind.DEVICE_RESET,
    ...
    def revoke(self, agent: SupportAgent, device_id: str, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.DEVICE_REVOKE,
                actor_id=agent.agent_id,
                target_id=device_id,
                tenant_id=dev["tenant_id"],
                reason=reason,
...
class SubscriptionExtender:
    ...
    def extend(self, agent: SupportAgent, user_id: str, days: int, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if sub is None:
                raise InterventionDeniedError(f"No subscription for {user_id}")
            if days <= 0:
            ...
            result = {
                ...
                "days_added": days,
                "old_expires": old_exp,
                "new_expires": sub["expires_at"],
                "extensions": sub["extensions"],
            }
            self._audit.record(
                kind=InterventionKind.SUBSCRIPTION_EXTEND,
                actor_id=agent.agent_id,
                target_id=user_id,
    ...
    def get(self, user_id: str) -> Optional[Dict[str, Any]]:
        with self._lock:
            return dict(self._subs[user_id]) if user_id in self._subs else None
...
class ArtifactResendHandler:
    def __init__(self, audit: SupportAuditChain) -> None:
        ...
        self._reissues: List[Dict[str, Any]] = []
    ...
    def resend(
        ...
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.ARTIFACT_RESEND,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=tenant_id,
                reason=reason,
    ...
    def reissue(
        self, agent: SupportAgent, user_id: str, tenant_id: str, artifact_id: str, reason: str
    ) -> Dict[str, Any]:
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.ARTIFACT_REISSUE,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=tenant_id,
                reason=reason,
    ...
    def send_history(self, user_id: str) -> List[Dict[str, Any]]:
        with self._lock:
            return [s for s in self._sends if s["user_id"] == user_id]
...
class AccountRecoveryHandler:
    ...
    def recover(self, agent: SupportAgent, user_id: str, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if acc is None:
                raise InterventionDeniedError(f"Account {user_id} not found")
            acc["status"] = "active"
            acc["suspended_reason"] = None
            result = {"user_id": user_id, "status": "recovered"}
            self._audit.record(
                kind=InterventionKind.ACCOUNT_RECOVER,
                actor_id=agent.agent_id,
                target_id=user_id,
    ...
    def suspend(self, agent: SupportAgent, user_id: str, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.ACCOUNT_SUSPEND,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=acc["tenant_id"],
                reason=reason,
    ...
    def unsuspend(self, agent: SupportAgent, user_id: str, reason: str = "") -> Dict[str, Any]:
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.ACCOUNT_UNSUSPEND,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=acc["tenant_id"],
                reason=reason,
    ...
    def reset_password(self, agent: SupportAgent, user_id: str, reason: str = "") -> Dict[str, Any]:
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.PASSWORD_RESET,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=acc["tenant_id"],
                reason=reason,
    ...
    def reset_mfa(self, agent: SupportAgent, user_id: str, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.MFA_RESET,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=acc["tenant_id"],
                reason=reason,
...
class ImpersonationManager:
    ...
    def grant(
        self, admin: SupportAgent, agent_id: str, target_user_id: str, tenant_id: str, reason: str
    ) -> Dict[str, Any]:
        with self._lock:
            ...
            result = {
                ...
                "granted_by": admin.agent_id,
            }
            self._audit.record(
                kind=InterventionKind.IMPERSONATION_GRANT,
                actor_id=admin.agent_id,
    ...
    def start(
        ...
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.IMPERSONATION_START,
                actor_id=agent.agent_id,
                target_id=target_user_id,
                tenant_id=tenant_id,
                reason=reason,
    ...
    def log_action(self, session_id: str, action: str) -> None:
        with self._lock:
            ...
            if session is None:
                raise ImpersonationError(f"Session {session_id} not found")
            if not session.is_active():
                raise ImpersonationError(f"Session {session_id} is not active")
    ...
    def end(self, agent: SupportAgent, session_id: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if session.agent_id != agent.agent_id:
                raise PermissionDeniedError("Only the owning agent can end a session")
            ...
            self._audit.record(
                kind=InterventionKind.IMPERSONATION_END,
                actor_id=agent.agent_id,
    ...
    def revoke(self, admin: SupportAgent, session_id: str, reason: str) -> Dict[str, Any]:
        with self._lock:
            ...
            if session is None:
                raise ImpersonationError(f"Session {session_id} not found")
            session.status = ImpersonationStatus.REVOKED
            session.ended_at = time.time()
            result = {"session_id": session_id, "status": "revoked", "revoked_by": admin.agent_id}
            self._audit.record(
                kind=InterventionKind.IMPERSONATION_REVOKE,
                actor_id=admin.agent_id,
                target_id=session.target_user_id,
                tenant_id=session.tenant_id,
                reason=reason,
...
class BillingInterventionHandler:
    ...
    def issue_refund(
        self, agent: SupportAgent, user_id: str, tenant_id: str, amount_cents: int, reason: str
    ) -> Dict[str, Any]:
        with self._lock:
            if amount_cents <= 0:
            ...
            self._audit.record(
                kind=InterventionKind.REFUND_MANUAL,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=tenant_id,
                reason=reason,
    ...
    def issue_credit(
        ...
        with self._lock:
            ...
            self._audit.record(
                kind=InterventionKind.CREDIT_MANUAL,
                actor_id=agent.agent_id,
                target_id=user_id,
                tenant_id=tenant_id,
                reason=reason,
    ...
    def total_refunded(self, user_id: str) -> int:
        with self._lock:
            return sum(r["amount_cents"] for r in self._refunds if r["user_id"] == user_id)


class SupportViewEngine:
    def __init__(
        self, audit: SupportAuditChain, guard: PermissionGuard, builder: CustomerViewBuilder
    ) -> None:
        self._audit = audit
        self._guard = guard
    ...
    def view_customer(self, agent: SupportAgent, raw: Dict[str, Any]) -> CustomerView:
        ...
        self._audit.record(
            kind=InterventionKind.VIEW_CUSTOMER,
            actor_id=agent.agent_id,
    ...
    def view_audit_trail(
        self, agent: SupportAgent, target_id: str, tenant_id: str, limit: int = 50
    ) -> List[_AuditEntry]:
        self._guard.check(agent, InterventionKind.VIEW_AUDIT_TRAIL)
        self._audit.record(
            kind=InterventionKind.VIEW_AUDIT_TRAIL,
            actor_id=agent.agent_id,
            target_id=target_id,
            tenant_id=tenant_id,
            reason="support_audit_view",
        )
        return self._audit.query(target_id=target_id, limit=limit)

    def view_device_list(
        self, agent: SupportAgent, user_id: str, tenant_id: str, device_handler
    ) -> List[Dict[str, Any]]:
        self._guard.check(agent, InterventionKind.VIEW_DEVICE_LIST)
        self._audit.record(
            kind=InterventionKind.VIEW_DEVICE_LIST,
            actor_id=agent.agent_id,
            target_id=user_id,
            tenant_id=tenant_id,
            reason="support_device_view",
        )
        return device_handler.list_devices(user_id)

    def view_license_status(
        self, agent: SupportAgent, user_id: str, tenant_id: str, info: Dict[str, Any]
    ) -> Dict[str, Any]:
        self._guard.check(agent, InterventionKind.VIEW_LICENSE_STATUS)
        self._audit.record(
            kind=InterventionKind.VIEW_LICENSE_STATUS,
            actor_id=agent.agent_id,
            target_id=user_id,
            tenant_id=tenant_id,
            reason="support_license_view",
        )
...
class ControlledInterventionEngine:
    ...
    def _make_record(
        self, kind, agent, user_id, tenant_id, reason, detail, status=InterventionStatus.EXECUTED
    ):
    ...
    def reset_device(self, agent, device_id, reason=""):
        ...
        return self._make_record(
            InterventionKind.DEVICE_RESET,
            agent,
            detail.get("user_id", device_id),
            agent.tenant_id,
            reason,
            detail,
        )
    ...
    def revoke_device(self, agent, device_id, reason):
        ...
        return self._make_record(
            InterventionKind.DEVICE_REVOKE, agent, device_id, agent.tenant_id, reason, detail
        )
    ...
    def extend_subscription(self, agent, user_id, days, reason=""):
        ...
        return self._make_record(
            InterventionKind.SUBSCRIPTION_EXTEND, agent, user_id, agent.tenant_id, reason, detail
        )
    ...
    def resend_artifact(self, agent, user_id, tenant_id, artifact_id, channel="email", reason=""):
        ...
        return self._make_record(
            InterventionKind.ARTIFACT_RESEND, agent, user_id, tenant_id, reason, detail
        )
    ...
    def reissue_artifact(self, agent, user_id, tenant_id, artifact_id, reason):
        ...
        return self._make_record(
            InterventionKind.ARTIFACT_REISSUE, agent, user_id, tenant_id, reason, detail
        )
    ...
    def recover_account(self, agent, user_id, reason):
        ...
        return self._make_record(
            InterventionKind.ACCOUNT_RECOVER, agent, user_id, agent.tenant_id, reason, detail
        )
    ...
    def suspend_account(self, agent, user_id, reason):
        ...
        return self._make_record(
            InterventionKind.ACCOUNT_SUSPEND, agent, user_id, agent.tenant_id, reason, detail
        )
    ...
    def reset_password(self, agent, user_id, reason=""):
        ...
        return self._make_record(
            InterventionKind.PASSWORD_RESET, agent, user_id, agent.tenant_id, reason, detail
        )
    ...
    def reset_mfa(self, agent, user_id, reason):
        ...
        return self._make_record(
            InterventionKind.MFA_RESET, agent, user_id, agent.tenant_id, reason, detail
        )
    ...
    def issue_refund(self, agent, user_id, tenant_id, amount_cents, reason):
        ...
        return self._make_record(
            InterventionKind.REFUND_MANUAL, agent, user_id, tenant_id, reason, detail
        )
...
class SupportAdminDashboard:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_impersonations": len(self._impersonation.active_sessions()),
        ...
        return [
            {
                ...
                "actions_taken": len(s.actions_taken),
            }
...
def build_support_system(secret: str = _AUDIT_SECRET) -> Dict[str, Any]:
    ...
    builder = CustomerViewBuilder()
    devices = DeviceResetHandler(audit)
    subs = SubscriptionExtender(audit)
    artifacts = ArtifactResendHandler(audit)
    accounts = AccountRecoveryHandler(audit)
    imperso = ImpersonationManager(audit)
    billing = BillingInterventionHandler(audit)
    engine = ControlledInterventionEngine(
        audit=audit,
        guard=guard,
        store=store,
        devices=devices,
