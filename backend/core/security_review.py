...
class SecurityAuditChain:
    def __init__(self, secret: str = "sec-review-secret-v34"):
        ...
        genesis_mac = hmac.new(
            self._secret, b"GENESIS:SECURITY:REVIEW:CHAIN:V34", hashlib.sha256
        ).hexdigest()
    ...
    def record(
        self, action: AuditAction, actor: str, reason: Optional[str] = None, **detail: Any
    ) -> _AuditEntry:
        ...
        with self._lock:
            ...
            entry = _AuditEntry(
                seq=seq, action=action, actor=actor, detail=detail, ts=ts_now, chain_hash=chain_hash
            )
    ...
    def query(
        self, action: Optional[AuditAction] = None, actor: Optional[str] = None, limit: int = 100
    ) -> List[_AuditEntry]:
...
class HeaderViolationError(ValueError):
    pass


class CORSViolationError(ValueError):
    pass
...
class IDORDetectedError(ValueError):
    pass
...
class CORSPolicy:
    ...
    allowed_headers: Set[str] = field(
        default_factory=lambda: {"Content-Type", "Authorization", "X-Tenant-ID"}
    )
...
class ReplayChecker:
    ...
    def check(self, nonce: str, timestamp: float, now: Optional[float] = None) -> bool:
        now = now or time.time()
    ...
    def _evict(self, now: float) -> None:
        if self._window <= 0:
            return
        expired = [n for n, t in self._times.items() if now - t > self._window]
        for n in expired:
    ...
    def run_check(self, cases: List[Dict[str, Any]]) -> CheckResult:
        ...
        for c in cases:
            try:
                self.check(c.get("nonce", ""), c.get("timestamp", time.time()), now=c.get("now"))
...
class AuthCheckConfig:
    ...
    allowed_roles: Set[str] = field(
        default_factory=lambda: {"admin", "operator", "viewer", "support"}
    )
...
class SecurityLogger:
    def __init__(
        self, audit: Optional[SecurityAuditChain] = None, redact_fields: Optional[Set[str]] = None
    ):
        self._audit = audit
    ...
    def log(self, level: str, message: str, **context: Any) -> LogEntry:
        safe_ctx = {k: "[REDACTED]" if k.lower() in self._redact else v for k, v in context.items()}
        entry = LogEntry(ts=time.time(), level=level, message=message, context=safe_ctx)
        with self._lock:
            self._log.append(entry)
        return entry

    def run_check(self) -> CheckResult:
        issues = [e for e in self._log if any(f in e.message.lower() for f in SENSITIVE_FIELDS)]
        findings: List[SecurityFinding] = []
        for issue in issues:
            findings.append(
                SecurityFinding(
                    ...
                    description="Log message may contain sensitive info",
...
class CryptoChecker:
    ...
    MIN_KEY_BITS = {"rsa": 2048, "dsa": 2048, "ec": 256, "aes": 128, "hmac": 128}
...
class RiskRegister:
    ...
    def mitigate(self, risk_id: str, actor: str, reason: str) -> None:
        ...
        if self._audit is not None:
            self._audit.record(
                AuditAction.RISK_MITIGATED, actor=actor, reason=reason, risk_id=risk_id
            )
    ...
    def accept(self, risk_id: str, actor: str, reason: str) -> None:
        ...
        if self._audit is not None:
            self._audit.record(
                AuditAction.RISK_ACCEPTED, actor=actor, reason=reason, risk_id=risk_id
            )
    ...
    def list_open(self) -> List[RiskItem]:
        with self._lock:
            return [i for i in self._items.values() if not i.mitigated and not i.accepted]
...
class PenTestHarness:
    ...
    def run_scenario(self, s: PenTestScenario) -> PenTestResult:
        try:
            cat = s.category
            blocked = False
            if cat in (VulnCategory.AUTH_BYPASS, VulnCategory.RBAC_BYPASS, VulnCategory.SESSION):
            ...
            self._audit.record(
                AuditAction.CHECK_RUN,
                actor="pentest_harness",
                scenario_id=s.scenario_id,
                passed=str(passed),
            )
            return PenTestResult(
                s.scenario_id, s.name, s.category, passed, "blocked" if blocked else "allowed"
            )
        except Exception as exc:
            return PenTestResult(s.scenario_id, s.name, s.category, False, f"ERROR: {exc}")
...
def build_security_review_system(
    ...
    policy = CORSPolicy(
        allowed_origins=allowed_origins or {"https://app.bot12.io"}, allow_credentials=True
    )
    ...
    engine = SecurityReviewEngine(
        headers, cors, rl, inj, idor, replay, leak, auth, logger, crypto, risk, pentest, audit
    )
    return {
        "audit": audit,
        "headers": headers,
