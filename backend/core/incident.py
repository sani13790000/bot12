...
class Incident:
    ...
    def add_event(self, actor_id: str, action: str, **detail: Any) -> None:
        self.timeline.append(TimelineEvent(
            ts=time.time(), actor_id=actor_id, action=action, detail=dict(detail)
        ))
...
class AlertRouter:
    ...
    def route(self, message: str, severity: IncidentSeverity,
        ...
        with self._lock:
            ...
            record = {"ts": time.time(), "message": message,
                      "severity": severity.value, "channels": results,
                      "dedup_key": dedup_key}
...
class KillSwitchV22:
    ...
    def activate(self, target: KillSwitchTarget, target_id: str,
                 reason: IncidentReason, actor_id: str, tenant_id: str,
                 reason_note: str = "", severity: IncidentSeverity = IncidentSeverity.P2_HIGH,
                 incident_id: Optional[str] = None,
                 ttl_seconds: Optional[float] = None) -> KillSwitchEntry:
        if not reason_note:
            raise ValueError("reason_note is mandatory for kill switch activation")
        with self._lock:
            ks_id = str(uuid.uuid4())
            entry = KillSwitchEntry(
                ks_id=ks_id, target=target, target_id=target_id,
                reason=reason, reason_note=reason_note, severity=severity,
                actor_id=actor_id, tenant_id=tenant_id,
                incident_id=incident_id, ttl_seconds=ttl_seconds,
            )
            ...
            self._audit_action("kill_switch.activated", actor_id, tenant_id,
                               reason.value, target=target.value,
                               target_id=target_id, ks_id=ks_id, note=reason_note)
            self._router.route(
    ...
    def reset(self, ks_id: str, actor_id: str, tenant_id: str,
              reason_note: str = "") -> bool:
        if not reason_note:
        ...
        with self._lock:
            ...
            if entry.target == KillSwitchTarget.GLOBAL:
                still_global = any(
                    ...
                    for e in self._entries.values() if not e.is_expired())
                self._global_active = still_global
            self._audit_action("kill_switch.reset", actor_id, tenant_id,
                               IncidentReason.RECOVERY.value,
                               ks_id=ks_id, target=entry.target.value,
                               target_id=entry.target_id, note=reason_note)
            return True

    def reset_all(self, target: KillSwitchTarget, target_id: str,
                  actor_id: str, tenant_id: str, reason_note: str) -> int:
        if not reason_note:
    ...
    def check(self, target: KillSwitchTarget, target_id: str) -> None:
        with self._lock:
            ...
            if self._global_active:
                raise KillSwitchError(KillSwitchTarget.GLOBAL, "global",
    ...
    def list_active(self, target: Optional[KillSwitchTarget] = None,
        ...
        with self._lock:
            ...
            for e in self._entries.values():
                ...
                if target and e.target != target: continue
    ...
    def _audit_action(self, event: str, user_id: str, tenant_id: str,
...
class IncidentManager:
    def __init__(self, kill_switch: Optional[KillSwitchV22] = None,
                 alert_router: Optional[AlertRouter] = None,
                 audit_logger=None) -> None:
        self._lock      = threading.RLock()
    ...
    def open_incident(self, title: str, severity: IncidentSeverity,
                      reason: IncidentReason, tenant_id: str, reporter_id: str,
                      tags: Optional[List[str]] = None, runbook_id: Optional[str] = None,
                      auto_kill: Optional[Tuple[KillSwitchTarget, str]] = None,
                      reason_note: str = "") -> Incident:
        if auto_kill and not reason_note:
            raise ValueError("reason_note is mandatory when auto_kill is specified")
        with self._lock:
            inc_id = str(uuid.uuid4())
            inc = Incident(
                incident_id=inc_id, title=title, severity=severity,
                reason=reason, tenant_id=tenant_id, reporter_id=reporter_id,
                tags=tags or [], runbook_id=runbook_id,
            )
            ...
            if auto_kill:
                ...
                ks_entry = self._ks.activate(
                    target=target, target_id=target_id, reason=reason,
                    actor_id=reporter_id, tenant_id=tenant_id,
                    reason_note=reason_note or title, severity=severity,
                    incident_id=inc_id,
                )
                inc.kill_switch_ids.append(ks_entry.ks_id)
                inc.add_event(reporter_id, "kill_switch_activated",
                              ks_id=ks_entry.ks_id, target=target.value)
            self._router.route(
                message=f"[{severity.value}] INCIDENT OPENED: {title}",
                severity=severity, dedup_key=f"inc:{inc_id}",
                context={"incident_id": inc_id, "reason": reason.value},
            )
            self._do_audit("incident.opened", reporter_id, tenant_id,
                           reason.value, incident_id=inc_id, title=title)
            return inc

    def transition(self, incident_id: str, new_state: IncidentState,
                   actor_id: str, note: str = "") -> Incident:
        with self._lock:
            ...
            inc.state = new_state
            ts = time.time()
            if new_state == IncidentState.ACKNOWLEDGED: inc.acknowledged_at = ts
            elif new_state == IncidentState.CONTAINED:  inc.contained_at = ts
            elif new_state == IncidentState.RESOLVED:   inc.resolved_at = ts
            elif new_state == IncidentState.CLOSED:     inc.closed_at = ts
            inc.add_event(actor_id, "state_changed",
                          old=old_state.value, new=new_state.value, note=note)
            self._do_audit("incident.state_changed", actor_id, inc.tenant_id,
    ...
    def list_incidents(self, tenant_id: Optional[str] = None,
    ...
    def get_timeline(self, incident_id: str) -> List[Dict[str, Any]]:
        with self._lock:
            inc = self._get(incident_id)
            return [{"ts": e.ts, "actor_id": e.actor_id,
    ...
    def _do_audit(self, event: str, user_id: str, tenant_id: str,
                  reason: str, **detail: Any) -> None:
        if self._audit is None:
            return
        try:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "steps": [{"order": s.order, "title": s.title,
                       "is_automated": s.is_automated, "cmd": s.cmd}
                      for s in self.steps],
        }


_RUNBOOKS = [
    Runbook("RB-001-ABUSE", "Abuse / Policy Violation",
            "User detected abusing API, license sharing, or rate limits.",
            IncidentSeverity.P2_HIGH, IncidentReason.ABUSE_DETECTED,
            tags=["security","license","user"],
            steps=[
                RunbookStep(1,"Identify abusing user/device","Query audit log for user_id.",True,"GET /admin/audit?user_id={user_id}"),
                RunbookStep(2,"Kill switch device","Activate kill switch for specific device.",True,"POST /admin/incident/kill-switch"),
                RunbookStep(3,"Revoke license","Revoke license with reason=abuse_detected.",True,"POST /admin/licenses/{id}/revoke"),
                RunbookStep(4,"Block user account","Block login and API access.",True,"POST /admin/users/{id}/block"),
    ...
    Runbook("RB-002-COMPROMISE", "Credential Compromise / Data Breach",
            "Credentials or sensitive data suspected to be compromised.",
            IncidentSeverity.P1_CRITICAL, IncidentReason.CREDENTIAL_COMPROMISE,
            tags=["security","p1","breach"],
            steps=[
                RunbookStep(1,"Activate GLOBAL kill switch","Immediately halt all trading.",True,"POST /admin/incident/kill-switch (target=global)"),
                RunbookStep(2,"Rotate all secrets","Rotate JWT secret, DB password, API keys."),
                RunbookStep(3,"Force logout all sessions","Invalidate all active JWTs.",True,"POST /admin/users/force-logout-all"),
                RunbookStep(4,"Revoke all licenses","Revoke all active licenses.",True),
                RunbookStep(5,"Forensic audit export","Export full audit chain.",True,"GET /admin/audit/export.jsonl"),
    ...
    Runbook("RB-003-DRAWDOWN", "Drawdown / Risk Limit Breach",
            "Equity drawdown or flash crash exceeds configured limits.",
            IncidentSeverity.P1_CRITICAL, IncidentReason.DRAWDOWN_LIMIT,
            tags=["risk","trading","p1"],
            steps=[
                RunbookStep(1,"Kill switch trading bot","Activate kill switch for affected bot.",True,"POST /admin/incident/kill-switch (target=bot)"),
                RunbookStep(2,"Close all open positions","Execute close-all on MT5.",True,"EA: CLOSE_ALL_POSITIONS"),
                ...
                RunbookStep(6,"Resume with approval","Reset kill switch after admin approval.",True,"POST /admin/incident/kill-switch/{id}/reset"),
            ]),
    Runbook("RB-004-BILLING", "Billing / Payment Failure",
            "Payment failure or chargeback leading to license suspension.",
            IncidentSeverity.P3_MEDIUM, IncidentReason.PAYMENT_FAILURE,
            tags=["billing","license"],
            steps=[
                RunbookStep(1,"Detect payment failure","Webhook from payment provider.",True),
                RunbookStep(2,"Suspend license","Suspend with grace period.",True,"POST /admin/licenses/{id}/suspend"),
    ...
    Runbook("RB-005-OUTAGE", "Service Outage / System Failure",
            "Backend API, database, or critical service is unreachable.",
            IncidentSeverity.P1_CRITICAL, IncidentReason.SYSTEM_OUTAGE,
            tags=["infrastructure","p1","sre"],
            steps=[
                RunbookStep(1,"Health check all services","Run health probes.",True,"GET /health/ready"),
                RunbookStep(2,"Check error rate","Review Prometheus metrics.",True),
                RunbookStep(3,"Rollback if recent deploy","Rollback if < 1hr old.",False,"docker compose up api@previous"),
                RunbookStep(4,"Scale or restart","Restart unhealthy containers.",True,"docker compose restart api"),
                RunbookStep(5,"Database failover","Switch to read-replica or restore."),
                RunbookStep(6,"Incident communication","Post status update."),
            ]),
    Runbook("RB-006-RECOVERY", "Post-Incident Recovery",
            "Systematic recovery and verification after any incident.",
            IncidentSeverity.P3_MEDIUM, IncidentReason.RECOVERY,
            tags=["recovery","verification"],
            steps=[
                RunbookStep(1,"Verify audit chain","Run chain integrity check.",True,"GET /admin/audit/verify"),
                RunbookStep(2,"Reset kill switches","Reset specific kill switches.",True,"POST /admin/incident/kill-switch/{id}/reset"),
    ...
    def check_escalations(self) -> List[str]:
        ...
        with self._lock:
            for inc in self._mgr._incidents.values():
                if inc.state in (IncidentState.OPEN,) and \
                        inc.incident_id not in self._escalated:
                    ...
                    if elapsed > sla:
                        ...
                        self._router.route(
                            message=(f"SLA BREACH: Incident {inc.incident_id[:8]} "
...
def get_alert_router() -> AlertRouter:
    global _router
    if _router is None: _router = AlertRouter()
    return _router
...
def get_incident_manager(audit_logger=None) -> IncidentManager:
    ...
    if _manager is None:
        _manager = IncidentManager(
            ...
            alert_router=get_alert_router(), audit_logger=audit_logger)
    return _manager

def get_runbook_registry() -> RunbookRegistry:
    global _runbooks
    if _runbooks is None: _runbooks = RunbookRegistry()
    return _runbooks
