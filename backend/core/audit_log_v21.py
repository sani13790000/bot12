...
P21-FIX-BUG:  chain if chain is not None (not 'chain or') — empty chain was replaced
"""
...
class MissingReasonError(ValueError):
    """Raised when a sensitive event is recorded without a reason."""


class TamperDetectedError(RuntimeError):
    """Raised when chain verification detects a hash mismatch."""
...
class AuditChain:
    ...
    def record(self, event, user_id="", tenant_id="default",
               actor="", reason="", detail=None, ip=None) -> AuditRecord:
        if event in REQUIRES_REASON and not (reason and reason.strip()):
            raise MissingReasonError(f"Event '{event.value}' requires a non-empty reason")
        detail = detail or {}
        ts  = time.time()
        rid = str(uuid.uuid4())
        ...
        with self._lock:
            ...
            rec = AuditRecord(
                id=rid, seq=seq, event=event.value,
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_hash":      records[-1].chain_hash if records else self._genesis(),
...
class AuditLogger:
    ...
    def __init__(self, chain=None) -> None:
        # CRITICAL: use 'is not None' — empty chain is falsy (__len__==0)
    ...
    def rbac_escalation_attempt(self, user_id, tenant_id="default", **kw):
        return self._record(AuditEvent.RBAC_ESCALATION,
                            user_id=user_id, tenant_id=tenant_id, **kw)
