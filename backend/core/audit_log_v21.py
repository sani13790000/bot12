...
class AuditRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "detail": self.detail, "chain_hash": self.chain_hash,
...
class AuditChain:
    ...
    def _canonical(self, rid: str, ev: str, ts: float, uid: str, tid: str,
                   reason: str, detail: Dict[str, Any]) -> bytes:
        payload = {
            "id": rid, "event": ev, "ts": str(ts),
            "user_id": uid, "tenant_id": tid, "reason": reason,
            "detail": json.dumps(detail, sort_keys=True),
        }
        return json.dumps(payload, sort_keys=True).encode()

    def _compute(self, prev: str, rid: str, ev: str, ts: float,
                 uid: str, tid: str, reason: str, detail: Dict) -> str:
        msg = (prev + ":" + self._canonical(rid, ev, ts, uid, tid, reason, detail).decode()).encode()
        return self._hmac(msg)

    def record(self, event: Any, user_id: str = "", tenant_id: str = "default",
               actor_id: str = "", ip: str = "", reason: str = "",
               detail: Optional[Dict] = None) -> AuditRecord:
        ...
        with self._lock:
            ...
            self._prev = ch
            rec = AuditRecord(
                id=rid, seq=seq, event=ev, severity=sev_val, ts=ts,
                user_id=user_id, tenant_id=tenant_id, actor_id=actor_id,
                ip=ip, reason=reason, detail=det,
                chain_hash=ch, prev_hash=prev,
            )
    ...
    def verify_chain(self) -> bool:
        ...
        prev = self._genesis
        for r in recs:
            expected = self._compute(prev, r.id, r.event, r.ts,
    ...
    def query(self, user_id: Optional[str] = None, tenant_id: Optional[str] = None,
              event: Optional[str] = None, severity: Optional[str] = None,
    ...
    def export_csv(self) -> str:
        ...
        fields = ["seq", "id", "event", "severity", "ts", "user_id",
                  "tenant_id", "actor_id", "ip", "reason", "chain_hash"]
...
class AuditLogger:
    ...
    def __init__(self, chain: Optional[AuditChain] = None,
                 secret: Union[str, bytes] = _DEFAULT_SECRET) -> None:
