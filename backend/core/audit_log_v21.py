...
class AuditLogger:
    ...
    def detect_tamper(self)->List[int]: return self._chain.detect_tamper()
    def summary(self)->Dict[str,Any]: return self._chain.summary()
    ...
    def trade_duplicate_blocked(self,u,**kw): return self.record(AuditEvent.TRADE_DUPLICATE_BLOCKED,user_id=u,**kw)
    def signal_emit(self,u,**kw): return self.record(AuditEvent.SIGNAL_EMIT,user_id=u,**kw)
    def signal_dedup_blocked(self,u,**kw): return self.record(AuditEvent.SIGNAL_DEDUP_BLOCKED,user_id=u,**kw)
    ...
    def risk_drawdown_critical(self,u,**kw): return self.record(AuditEvent.RISK_DRAWDOWN_CRITICAL,user_id=u,**kw)
    def admin_audit_export(self,user_id="admin",**kw): return self.record(AuditEvent.ADMIN_AUDIT_EXPORT,user_id=user_id,**kw)
    def admin_chain_verify(self,user_id="admin",**kw): return self.record(AuditEvent.ADMIN_CHAIN_VERIFY,user_id=user_id,**kw)
    def recon_mismatch(self,u,**kw): return self.record(AuditEvent.RECON_MISMATCH,user_id=u,**kw)
