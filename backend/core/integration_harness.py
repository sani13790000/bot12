...
class License:
    ...
    def is_active(self) -> bool:
        return self.status == LicenseState.ACTIVE and (
            self.expires_at == 0 or time.time() < self.expires_at
        )
...
class BillingEngine:
    ...
    def process_webhook(self, provider: str, provider_ref: str, event: str, sub_id: str) -> dict:
...
class SignalService:
    ...
    def emit(
        self, user_id: str, symbol: str, direction: str, expires_in: float = 300.0
    ) -> Optional[Signal]:
    ...
    def get_signals(self, user_id: str) -> List[Signal]:
        return [s for s in self._signals if s.user_id == user_id and not s.is_expired()]
...
class KillSwitch:
    def __init__(self, max_drawdown_pct: float = 10.0, equity_floor_usd: float = 1000.0) -> None:
        self._active = False
        self._reason = ""
        self._actor = ""
...
class ReconciliationEngine:
    ...
    def reconcile(self, local: List[Position], broker: List[Position]) -> List[dict]:
        ...
        for sym in symbols:
            ...
            if lp is None:
                mismatches.append(
                    {"symbol": sym, "type": "ghost", "broker_qty": bp.qty, "local_qty": 0.0}
                )
            elif bp is None:
                mismatches.append(
                    {"symbol": sym, "type": "missing", "broker_qty": 0.0, "local_qty": lp.qty}
                )
...
_ROLE_PERMS: Dict[str, set] = {
    ...
    "admin": {"read:own", "write:own", "read:any", "write:any", "admin:action", "revoke:license"},
    "super_admin": {
        "read:own",
        "write:own",
        "read:any",
...
class ProductionReadinessGate:
    ...
    def run(self) -> ReadinessResult:
        ...
        chk("jwt_secret_length", len(jwt) >= 32, f"JWT_SECRET_KEY too short ({len(jwt)} < 32)")
        chk(
            "jwt_secret_not_weak",
        ...
        chk(
            "field_enc_key_present",
            len(fk) >= 32,
            f"FIELD_ENCRYPTION_KEY missing or too short ({len(fk)})",
        )

        if env == "production":
            chk("debug_off", not self._cfg.get("DEBUG", False), "DEBUG must be False in production")

        if env == "production":
            chk(
                "hsts_enabled",
                self._cfg.get("HSTS_ENABLED", False),
...
class E2EFlowSimulator:
    ...
    def issue_token(self, user_id: str, role: str = "customer", exp_offset: int = 3600) -> str:
        payload = {
            "sub": user_id,
            "role": role,
    ...
    def webhook(self, provider: str, provider_ref: str, event: str, sub_id: str) -> dict:
    ...
    def create_license(self, user_id: str, plan: str = "pro", max_devices: int = 1) -> License:
        import hashlib
    ...
    def emit_signal(self, user_id: str, symbol: str, direction: str) -> Optional[Signal]:
        try:
            self.kill_sw.check()
        ...
        if sig:
            self._log("signal_emitted", user_id=user_id, signal_id=sig.signal_id, symbol=symbol)
        else:
    ...
    def insert_trade(
        ...
        self.kill_sw.check()
        trade = self.trades.insert(
            user_id, symbol, direction, lot_size, idempotency_key, mt5_ticket
        )
    ...
    def run_reconciliation(self, local: List[Position], broker: List[Position]) -> List[dict]:
...
class SmokeTestSuite:
    ...
    def run_all(self) -> List[SmokeResult]:
        ...
        self._run(
            "checkout_pro",
            lambda: (lambda sub: assert_true(sub.is_active))(self._sim.checkout(uid, "pro")),
        )

        sub_id = next(s.sub_id for s in self._sim.billing._subs.values() if s.user_id == uid)
        self._run(
            "webhook_idempotency",
            lambda: (
                lambda r1, r2: (
        ...
        self._run(
            "license_heartbeat", lambda: assert_true(self._sim.heartbeat(lic.license_id, "dev_001"))
        )

        self._run(
            "signal_emit_and_dedup",
            lambda: (
                lambda s1, s2: (
        ...
        self._run(
            "trade_idempotency",
            lambda: (lambda t1, t2: assert_true(t1.trade_id == t2.trade_id))(
        ...
        self._run(
            "reconciliation",
            lambda: (lambda mm: assert_true(len(mm) > 0))(
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "avg_latency_ms": (
                sum(r.latency_ms for r in self._results) / max(len(self._results), 1)
            ),
        }
...
class RegressionGuard:
    ...
    def check_phase13_db_migration() -> bool:
        path = "/home/definable/phase13_db/migrations/20260626_027_phase13_saas_schema.sql"
        try:
            with open(path) as f:
                content = f.read()
