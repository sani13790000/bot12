...
class IntegrationPolicyError(Exception):
    pass
...
DEFAULT_POLICIES: Dict[IntegrationKind, IntegrationPolicy] = {
    ...
    IntegrationKind.TELEGRAM: IntegrationPolicy(
        ...
        scheme=SignatureScheme.PLAIN_TOKEN,
        timeout_seconds=5.0,
        max_retries=2,
        replay_window_s=120,
        circuit_threshold=5,
    ...
    IntegrationKind.AUTH_PROVIDER: IntegrationPolicy(
        kind=IntegrationKind.AUTH_PROVIDER,
        scheme=SignatureScheme.HMAC_SHA256,
        timeout_seconds=10.0,
        max_retries=1,
        replay_window_s=300,
    ...
    def check_and_record(self, event_id: str, timestamp_ms: Optional[int] = None) -> bool:
        ...
        if timestamp_ms is not None:
            ...
            if abs(now - provider_ts) > self._window:
                raise ReplayError(
                    f"Timestamp outside replay window: delta={abs(now - provider_ts):.1f}s window={self._window}s"
                )
        with self._lock:
            self._evict(now)
    ...
    def verify(self, scheme: SignatureScheme, payload: bytes, signature: str) -> bool:
        ...
        if scheme == SignatureScheme.HMAC_SHA256:
            ...
            if not _hmac.compare_digest(expected, signature.lower()):
                raise SignatureError("HMAC-SHA256 mismatch")
            return True
            ...
            if key in self._store:
                ...
                if stored_ph != ph:
                    raise IdempotencyConflict(
                        f"Idempotency key {key!r} used with different payload"
                    )
...
class RetryPolicy:
    def __init__(
        self, max_retries: int = 3, base_ms: int = 200, max_ms: int = 30_000, jitter: bool = True
    ):
...
class SafeIntegrationCall:
        ...
        with self._lock:
            if kind not in self._breakers:
                ...
                self._breakers[kind] = CircuitBreaker(
                    threshold=p.circuit_threshold, timeout_s=p.circuit_timeout_s
                )
    ...
    def verify_inbound(
        self, event: IntegrationEvent, raw_body: bytes, actor: str = "inbound"
    ) -> bool:
        ...
        try:
            ...
            self._audit.record(
                ...
                event.event_id,
                actor=actor,
                detail={"scheme": policy.scheme.value},
            )
            return True
    ...
    def call(
        self,
        event: IntegrationEvent,
        handler: Callable[[IntegrationEvent], Any],
        actor: str = "system",
        ...
        if verify_signature and policy.scheme != SignatureScheme.NONE:
            try:
                ...
                return CallResult(
                    result=IntegrationResult.SIG_INVALID,
        ...
        if policy.replay_window_s > 0:
            try:
                ...
                return CallResult(
                    result=IntegrationResult.REPLAY_BLOCKED,
                    event_id=event.event_id,
                    error=str(e),
                    latency_ms=(time.time() - t0) * 1000,
                )
        if event.idempotency_key:
            try:
                ...
                if cached is not None:
                    self._audit.record(
                        AuditAction.IDEMPOTENT_HIT,
                        event.kind,
                        event.event_id,
                        actor=actor,
                        detail={"key": event.idempotency_key},
                    )
                    return CallResult(
                        result=IntegrationResult.IDEMPOTENT_HIT,
                        event_id=event.event_id,
                        response=cached,
                        cached=True,
                        latency_ms=(time.time() - t0) * 1000,
        ...
        if not breaker.allow_call():
            ...
            return CallResult(
                result=IntegrationResult.CIRCUIT_OPEN,
                event_id=event.event_id,
                error="Circuit breaker OPEN",
                latency_ms=(time.time() - t0) * 1000,
            )
        retry = RetryPolicy(
            max_retries=policy.max_retries, base_ms=policy.retry_base_ms, max_ms=policy.retry_max_ms
        )
        ...
        while attempt <= retry.max_retries:
            try:
                ...
                if new_state == CircuitState.OPEN and attempt == 0:
                    self._audit.record(
                        AuditAction.CIRCUIT_TRIPPED, event.kind, event.event_id, actor=actor
                    )
                if not retry.should_retry(attempt + 1):
                    break
                attempt += 1
        self._dlq.append(
            DeadLetterItem(event=event, reason=str(last_exc), attempts=attempt + 1, ts=time.time())
        )
        self._audit.record(
            AuditAction.DEAD_LETTERED,
            event.kind,
            event.event_id,
            actor=actor,
            detail={"reason": str(last_exc), "attempts": attempt + 1},
        )
        return CallResult(
            result=IntegrationResult.DEAD_LETTERED,
            event_id=event.event_id,
            attempts=attempt + 1,
            error=str(last_exc),
            latency_ms=(time.time() - t0) * 1000,
    ...
    @property
    def dlq(self) -> List[DeadLetterItem]:
    ...
    def reset_circuit(self, kind: IntegrationKind, reason: str) -> None:
        ...
        self._audit.record(
            AuditAction.CIRCUIT_RESET, kind, "admin", actor="admin", detail={"reason": reason}
        )
...
class IntegrationAdmin:
    def __init__(
        self,
        registry: IntegrationRegistry,
        caller: SafeIntegrationCall,
        audit: IntegrationAuditChain,
    ):
        self._registry = registry
        self._caller = caller
        self._audit = audit
    ...
    def revoke_key(self, kind: IntegrationKind, reason: str) -> None:
        if not reason or not reason.strip():
            raise MissingReasonError("Reason required to revoke key")
    ...
    def inspect_circuits(self) -> Dict[str, str]:
        return {
            kind.value: self._caller.circuit_state(kind).value
            for kind in self._registry.list_kinds()
        }

    def drain_dlq(self, reason: str) -> List[DeadLetterItem]:
        if not reason or not reason.strip():
            raise MissingReasonError("Reason required to drain DLQ")
        ...
        self._audit.record(
            ...
            "admin-drain",
            actor="admin",
            detail={"drained": len(items), "reason": reason},
        )
        return items
