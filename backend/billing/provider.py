...
class ZarinpalProvider(PaymentProvider):
    ...
    @property
    def _base(self) -> str:
        return "https://sandbox.zarinpal.com/pg" if self._sandbox else "https://api.zarinpal.com/pg"
    ...
    def parse_webhook(self, payload: bytes) -> WebhookEvent:
        ...
        return WebhookEvent(
            provider=self.name,
            event_type=(
                WebhookEventType.PAYMENT_SUCCEEDED if ok else WebhookEventType.PAYMENT_FAILED
            ),
            invoice_id=invoice_id,
            amount=data.get("Amount", 0),
