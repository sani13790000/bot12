...
class WalkForwardWindow:
    ...
    is_result: Optional[Any] = None
    oos_result: Optional[Any] = None
    passed: bool = False
    ...
    def to_dict(self) -> dict:
        def _s(r):
            ...
            return {
                ...
                "wr": round(getattr(r, "win_rate", 0.0) * 100, 1),
                "net_pct": getattr(r, "net_profit_pct", 0.0),
                "trades": getattr(r, "total_trades", 0),
                "mdd": getattr(r, "max_drawdown_pct", 0.0),
            }
        ...
        return {
            ...
            "is_result": _s(self.is_result),
            "oos_result": _s(self.oos_result),
        }
...
class WalkForwardAdvanced:
    def __init__(
        ...
        self.min_fitness_is = min_fitness_is
    ...
    def _build_windows(self, start: datetime, end: datetime) -> List[WalkForwardWindow]:
        ...
        if self.mode == "anchored":
            anchor = start
            oos_s = self._add_months(anchor, self.is_months)
            while oos_s < end:
                oos_e = min(self._add_months(oos_s, self.oos_months), end)
                windows.append(
                    WalkForwardWindow(
                        window_id=idx, is_start=anchor, is_end=oos_s, oos_start=oos_s, oos_end=oos_e
                    )
                )
                oos_s = self._add_months(oos_s, self.step_months)
                idx += 1
        else:
            is_s = start
            while True:
                is_e = self._add_months(is_s, self.is_months)
                oos_s = is_e
                oos_e = self._add_months(oos_s, self.oos_months)
                if oos_e > end:
                    break
                windows.append(
                    WalkForwardWindow(
                        window_id=idx, is_start=is_s, is_end=is_e, oos_start=oos_s, oos_end=oos_e
                    )
                )
                is_s = self._add_months(is_s, self.step_months)
                idx += 1
        return windows
    ...
    async def run(self, start: datetime, end: datetime, run_backtest_fn) -> WalkForwardResult:
        ...
        for w in windows:
            try:
                is_res, oos_res = await run_backtest_fn(
                    w.is_start, w.is_end, w.oos_start, w.oos_end
                )
                w.is_result = is_res
                w.oos_result = oos_res
            except Exception:
                w.is_result = None
                w.oos_result = None
            w.fitness_is = self._fitness(w.is_result)
            w.fitness_oos = self._fitness(w.oos_result)
        ...
        notes: list = []
        if total == 0:
            notes.append("No windows evaluated")
        elif pass_count == 0:
            notes.append("No windows passed")
        elif _safe_div(pass_count, total) < 0.5:
            notes.append(f"Only {pass_count}/{total} windows passed")
        if avg_eff > 2.0:
            notes.append("Avg efficiency > 2.0 — possible look-ahead bias")
        if avg_eff < 0.3 and total > 0:
            notes.append("Avg efficiency < 0.3 — OOS worse than IS")
