...
BUG-AJ2 FIX: silent pass in date parse except → logger.debug
  - was: except Exception: pass  (silent error in expires_at parse)
  - now: except Exception as exc: logger.debug("[License] date parse: %s", exc)
...
class LicenseEngine:
    ...
    def validate(self, license_data: Dict[str, Any]) -> bool:
        ...
        Returns:
            ...
            None — خطا رخ داد (فایل-کلوز شدن برای محیط درست)
        """
            ...
            try:
                ...
                if expires_at:
                    ...
                    try:
                        ...
                        # BUG-AJ2 FIX: was bare pass — now logged
                        logger.debug("[License] date parse: %s", exc)
                return True
            except Exception as exc:
