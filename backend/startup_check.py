"""
backend/startup_check.py — RESTORED + BUG-R5-3 FIX
CB-8 FIX: Now actually called at startup via lifespan() in main.py
BUG-R5-3 FIX: None guard before r.ping() to avoid AttributeError
...
async def _check_redis() -> CheckResult:
    ...
    try:
        ...
        # BUG-R5-3 FIX: guard against None before calling ping()
        if r is None:
            return ("redis", False, "could not connect — get_redis() returned None")
        await r.ping()
