...
P15-OBS-ALERT-1: license_failure alert rule
P15-OBS-ALERT-2: heartbeat_loss alert rule
P15-OBS-ALERT-3: kill_switch alert rule (CRITICAL, immediate)
P15-OBS-ALERT-4: drawdown_critical alert rule
P15-OBS-ALERT-5: reconciliation_mismatch alert rule
P15-OBS-ALERT-6: deduplication
P15-OBS-ALERT-7: async Telegram with timeout=6s
P15-OBS-ALERT-8: rate limit max 15/min
P15-OBS-ALERT-9: callback system
P15-OBS-ALERT-10: per-level escalation
"""
...
class AlertRule:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"name": self.name, "description": self.description,
                "level": self.level, "enabled": self.enabled, "dedup_s": self.dedup_window_s}


_DEFAULT_RULES: List[AlertRule] = [
    AlertRule("license_failure", "License validation failed", AlertLevel.CRITICAL, dedup_window_s=60),
    ...
    AlertRule("kill_switch_activated", "Kill switch activated", AlertLevel.CRITICAL, dedup_window_s=30),
    AlertRule("kill_switch_reset", "Kill switch reset", AlertLevel.INFO, dedup_window_s=30),
    AlertRule("drawdown_critical", "Equity drawdown > 10%", AlertLevel.CRITICAL, dedup_window_s=180),
    AlertRule("drawdown_warning", "Equity drawdown > 5%", AlertLevel.WARNING),
    AlertRule("daily_loss_limit", "Daily loss limit reached", AlertLevel.CRITICAL, dedup_window_s=3600),
    AlertRule("reconciliation_mismatch", "Position mismatch broker vs local", AlertLevel.CRITICAL, dedup_window_s=300),
    ...
    AlertRule("ml_drift", "ML model drift", AlertLevel.WARNING),
...
@dataclass
class AlertRecord:
    rule_name: str; level: AlertLevel; message: str; context: Dict[str, Any]
    ts: float = field(default_factory=time.time)
    sent: bool = False; deduped: bool = False
...
class AlertManager:
    ...
    def add_rule(self, rule: AlertRule) -> None: self._rules[rule.name] = rule
    def get_rule(self, name: str) -> Optional[AlertRule]: return self._rules.get(name)
    def list_rules(self) -> List[Dict[str, Any]]: return [r.to_dict() for r in self._rules.values()]
    def enable_rule(self, name: str) -> bool:
        if name in self._rules: self._rules[name].enabled = True; return True
        return False
    def disable_rule(self, name: str) -> bool:
        if name in self._rules: self._rules[name].enabled = False; return True
        return False

    def add_callback(self, cb: AlertCallback) -> None: self._callbacks.append(cb)
    def remove_callback(self, cb: AlertCallback) -> None:
        self._callbacks = [c for c in self._callbacks if c is not cb]

    async def fire(self, rule_name: str, context: Optional[Dict[str, Any]] = None,
        ...
        if rule.dedup_window_s > 0:
            ...
            if time.time() - last < rule.dedup_window_s:
                ...
                return False
        now = time.time()
        while self._rate_win and self._rate_win[0] < now - _RATE_WIN_S: self._rate_win.popleft()
        if len(self._rate_win) >= _RATE_LIMIT_N:
            logger.warning("alert rate limit %s", rule_name)
            record.deduped = True; self._history.append(record); return False
        self._rate_win.append(now)
        self._dedup[dedup_key] = now
        record.sent = True; self._history.append(record); self._sent_count += 1
        tasks = []
        ...
        if tasks:
            ...
            for r in results:
                if isinstance(r, Exception): logger.error("alert dispatch error: %s", r)
        return True

    async def alert_license_failure(self, reason: str, user_id: Optional[str] = None,
    ...
    async def alert_reconciliation_mismatch(self, symbol: str, broker_qty: float,
    ...
    async def _send_telegram(self, rule: str, level: AlertLevel, msg: str, ctx: Dict[str, Any]) -> None:
        if not self._token or not self._chat_id: return
        emoji = {"CRITICAL": "CRIT", "WARNING": "WARN", "INFO": "INFO"}.get(level, "")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with asyncio.timeout(6.0):
                    await session.post(url, json={"chat_id": self._chat_id, "text": text})
        except Exception as exc: logger.warning("telegram failed: %s", exc)

    async def _send_webhook(self, rule: str, level: AlertLevel, msg: str, ctx: Dict[str, Any]) -> None:
        if not self._webhook: return
        try:
            import aiohttp
            async with aiohttp.ClientSession() as session:
                async with asyncio.timeout(6.0):
                    await session.post(self._webhook, json={"rule": rule, "level": level,
                                                            "message": msg, "context": ctx,
                                                            "ts": time.time()})
        except Exception as exc: logger.warning("webhook failed: %s", exc)

    def history(self, level: Optional[AlertLevel] = None, rule_name: Optional[str] = None,
                since_ts: Optional[float] = None, limit: int = 50) -> List[Dict[str, Any]]:
        records = list(self._history)
        if level: records = [r for r in records if r.level == level]
        if rule_name: records = [r for r in records if r.rule_name == rule_name]
        if since_ts: records = [r for r in records if r.ts >= since_ts]
        records = records[-limit:]
        return [{"rule": r.rule_name, "level": r.level, "message": r.message,
                 "context": r.context, "ts": r.ts, "sent": r.sent, "deduped": r.deduped}
                for r in records]
    ...
    def reset(self) -> None:
        self._history.clear(); self._dedup.clear(); self._rate_win.clear()
        self._sent_count = 0; self._dedup_count = 0
