...
class HealthStatus:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "components": {
                k: {"status": v.status, "message": v.message, "latency_ms": round(v.latency_ms, 2)}
...
class HealthChecker:
    ...
    def record_degraded(self, component: str, message: str = "") -> None:
        with self._lock:
            self._component_states[component] = ComponentHealth(
                name=component, status="degraded", message=message
            )

    def record_unhealthy(self, component: str, message: str = "") -> None:
        with self._lock:
            self._component_states[component] = ComponentHealth(
                name=component, status="unhealthy", message=message
            )
    ...
    def aggregate(self) -> HealthStatus:
        ...
        for name, (fn, timeout) in self._checks.items():
            ...
            if t.is_alive():
                ...
                components[name] = ComponentHealth(
                    name=name, status="unhealthy", message=str(exc_box[0]), latency_ms=elapsed_ms
                )
                ...
                components[name] = ComponentHealth(
                    name=name, status="healthy", latency_ms=elapsed_ms
                )
