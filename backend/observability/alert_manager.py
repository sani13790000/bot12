...
class AlertManager:
    ...
    async def send(self, message: str, level: AlertLevel = AlertLevel.INFO,
        ...
        self._history.append(AlertRecord(rule_name="", level=level, message=message,
                                         context=context or {}, sent=True))
    ...
    async def fire(self, rule_name: str, context: Optional[Dict[str, Any]] = None,
        ...
        if rule.dedup_window_s > 0:
            ...
            if time.time() - last < rule.dedup_window_s:
                self._history.append(AlertRecord(rule_name=rule_name, level=level,
                                                  message=msg, context=ctx, deduped=True))
                self._dedup_count += 1
                return False
        now = time.time()
        ...
        self._history.append(AlertRecord(rule_name=rule_name, level=level,
                                          message=msg, context=ctx, sent=True))
        self._sent_count += 1
        tasks: List[Any] = []
    ...
    async def alert_drawdown(self, pct: float, equity_usd: Optional[float] = None) -> bool:
        ...
        return await self.fire(rule, context={"drawdown_pct": pct, "equity_usd": equity_usd},
    ...
    async def _send_telegram(self, rule: str, level: AlertLevel, msg: str,
                              ctx: Dict[str, Any]) -> None:
        if not self._token or not self._chat_id:
            return
        emoji = {"CRITICAL": "\u26a0\ufe0f", "WARNING": "\U0001f514"}.get(str(level), "\u2139\ufe0f")
        text = f"{emoji} *[{level}]* {rule}: {msg}"
        try:
            import httpx
            async with httpx.AsyncClient(timeout=6.0) as client:
                await client.post(f"https://api.telegram.org/bot{self._token}/sendMessage",
                                  json={"chat_id": self._chat_id, "text": text,
                                        "parse_mode": "Markdown"})
        except Exception as exc:
            logger.warning("Telegram alert failed: %s", exc)

    async def _send_webhook(self, rule: str, level: AlertLevel, msg: str,
                             ctx: Dict[str, Any]) -> None:
        ...
        try:
            import httpx
            async with httpx.AsyncClient(timeout=6.0) as client:
                await client.post(self._webhook,
                                  json={"rule": rule, "level": str(level), "message": msg,
