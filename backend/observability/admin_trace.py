...
P15-OBS-TRACE-2: issue_trace() — admin می‌تواند یک issue را سریع trace کند
P15-OBS-TRACE-3: per-user timeline
P15-OBS-TRACE-4: correlated_events() — events مرتبط با یک trace_id
...
class AdminTracer:
    ...
    def record(
        ...
        event = TraceEvent(
            event_id=eid, trace_id=tid, category=category,
            action=action, level=level, user_id=user_id,
            device_id=device_id, detail=detail or {},
        )
        with self._lock:
    ...
    def record_license_failure(self, reason: str, user_id: str,
        ...
        return self.record("license", "license_failure", "CRITICAL",
                           ...
                           trace_id=trace_id, detail={"reason": reason})
    ...
    def record_heartbeat_loss(self, device_id: str, gap_s: float,
        ...
        return self.record("heartbeat", "heartbeat_loss", "CRITICAL",
                           user_id=user_id, device_id=device_id,
                           trace_id=trace_id, detail={"gap_s": gap_s})

    def record_kill_switch(self, actor: str, reason: str,
                            trace_id: Optional[str] = None) -> str:
        return self.record("kill_switch", "kill_switch_activated", "CRITICAL",
                           user_id=actor, trace_id=trace_id,
    ...
    def record_reconciliation_mismatch(self, symbol: str, broker_qty: float,
        ...
        return self.record("reconciliation", "reconciliation_mismatch", "CRITICAL",
                           trace_id=trace_id,
                           detail={"symbol": symbol, "broker": broker_qty,
                                   "local": local_qty,
    ...
    def record_drawdown(self, pct: float, equity_usd: Optional[float] = None,
        ...
        return self.record("drawdown", "drawdown_alert", level,
                           trace_id=trace_id,
    ...
    def export_csv(
        ...
        events = self.issue_trace(user_id=user_id, category=category,
                                   since_ts=since_ts, limit=limit)
        buf = io.StringIO()
        writer = csv.DictWriter(
            buf,
            fieldnames=["ts", "event_id", "trace_id", "category",
                        "action", "level", "user_id", "device_id", "detail"],
        )
        writer.writeheader()
        for ev in events:
            row = {**ev, "detail": str(ev.get("detail", {}))}
            writer.writerow(row)
        return buf.getvalue()
