...
Execution Service - FIX-1 + FIX-2 + BUG-ES-1 + BUG-ES-2 + BUG-ES-3 + INFO-ES-4

BUG-ES-1: await self._pr.run_once() -> AttributeError (method is _run_once, private)
BUG-ES-2: float(...) or None -> None passed to ManagedOrder float fields
BUG-ES-3: asyncio.Lock() at module level -> RuntimeError in Python 3.12+
INFO-ES-4: SemiAutoManager.mark_executed() never called after fill -> memory leak
"""
...
# BUG-ES-3 FIX: asyncio.Lock() must NOT be created at module level (import time).
# Python 3.10 raises DeprecationWarning; Python 3.12+ raises RuntimeError when no
# event loop is running. Use module-level Optional[asyncio.Lock] = None and
# initialise lazily on first use inside the running event loop.
...
class ExecutionService:
    ...
    BUG-ES-1:  run_once() -> _run_once() fixed.
    BUG-ES-2:  float(...) or None -> float(...) or 0.0 fixed.
    BUG-ES-3:  asyncio.Lock lazy init fixed.
    INFO-ES-4: mark_executed() called after semi_auto fill.
    """
    ...
    async def start(self) -> None:
        ...
        # BUG-PR-2 FIX: inject MT5Connector into PositionReconciliation singleton
        if hasattr(self._pr, "set_mt5"):
    ...
    async def _execute_signal_inner(self, signal: Dict[str, Any], signal_id: str, user_id: Optional[int]) -> Dict[str, Any]:
        ...
        order = ManagedOrder(
            ...
            requested_volume=volume, requested_price=entry,
        ...
        if self._semi_auto_enabled:
            ...
            # INFO-ES-4 FIX: wire on_approved so mark_executed() fires after fill
            async def _on_approved(sig) -> None:
                result = await self._submit_order(order)
                if result.get("status") == "filled":
                    await self._semi_auto.mark_executed(sig.signal_id)

            await self._semi_auto.submit_for_approval(pending, on_approved=_on_approved)
    ...
    async def _retry_execute(self, failed_order_meta: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # BUG-ES-1 FIX: public run_once() added to PositionReconciliation
            ...
            risk_result = await self._run_risk_check(
                symbol=symbol, direction=od.get("action", ""),
                sl_pips=sl_pips, signal=od,
            )
            if not risk_result["approved"]:
            ...
            # BUG-ES-2 FIX: removed 'or None' pattern; 0.0 = market order sentinel
            retry_order = ManagedOrder(
                order_id=new_order_id,
                signal_id=signal_id,
                symbol=symbol,
                action=od.get("action", ""),
                requested_volume=float(od.get("requested_volume", 0.01)),
                requested_price=float(od.get("requested_price", 0.0)),
                stop_loss=float(od.get("stop_loss", 0.0)),
                take_profit=float(od.get("take_profit", 0.0)),
            )
