...
class OrderJournal:
    ...
    async def record(self, event_type: JournalEventType, **kwargs: Any) -> JournalEntry:
        ...
        logger.debug("JOURNAL [%s] %s signal=%s", event_type.value,
                     entry.symbol or "-", (entry.signal_id or "-")[:8])
        if self._persist:
            asyncio.create_task(self._persist_entry(entry))
        return entry

    async def record_signal_received(self, signal_id: str, symbol: str, direction: str, **kw) -> JournalEntry:
        return await self.record(JournalEventType.SIGNAL_RECEIVED, signal_id=signal_id, symbol=symbol, direction=direction, **kw)

    async def record_risk_assessed(self, signal_id: str, approved: bool, lot_size: float,
                                    risk_reason: str = "", pip_value_used: float = 0.0, **kw) -> JournalEntry:
        evt = JournalEventType.RISK_ASSESSED if approved else JournalEventType.RISK_BLOCKED
        return await self.record(evt, signal_id=signal_id, risk_allowed=approved, lot_size=lot_size,
            risk_reason=risk_reason or None, pip_value_used=pip_value_used or None, **kw)

    async def record_order_submitted(self, signal_id: str, order_id: str, symbol: str,
                                      direction: str, lot_size: float, price: float,
                                      stop_loss: float = 0.0, take_profit: float = 0.0, **kw) -> JournalEntry:
        return await self.record(JournalEventType.ORDER_SUBMITTED, signal_id=signal_id, order_id=order_id,
            symbol=symbol, direction=direction, lot_size=lot_size, price=price,
            stop_loss=stop_loss or None, take_profit=take_profit or None, **kw)

    async def record_fill(self, signal_id: str, order_id: str, mt5_ticket: int,
                           fill_price: float, fill_volume: float,
                           requested_price: float = 0.0, pip_value: float = 10.0,
                           fill_latency_ms: float = 0.0, **kw) -> JournalEntry:
        ...
        return await self.record(JournalEventType.ORDER_FILLED,
            signal_id=signal_id, order_id=order_id, mt5_ticket=mt5_ticket,
            ...
            fill_latency_ms=fill_latency_ms or None, **kw)

    async def record_close(self, signal_id: str, order_id: str, mt5_ticket: int,
                            close_price: float, pnl_usd: float,
                            close_reason: str = "", duration_s: float = 0.0, **kw) -> JournalEntry:
        return await self.record(JournalEventType.POSITION_CLOSED,
            signal_id=signal_id, order_id=order_id, mt5_ticket=mt5_ticket,
            close_price=close_price, pnl_usd=round(pnl_usd, 2),
            close_reason=close_reason or None, duration_s=duration_s or None, **kw)

    async def record_circuit_open(self, breaker_name: str, reason: str,
                                   signal_id: Optional[str] = None) -> JournalEntry:
        return await self.record(JournalEventType.CIRCUIT_OPENED,
            signal_id=signal_id, breaker_name=breaker_name, error_message=reason)

    async def record_error(self, error: Exception, signal_id: Optional[str] = None,
                            order_id: Optional[str] = None, **kw) -> JournalEntry:
        return await self.record(JournalEventType.ERROR, signal_id=signal_id, order_id=order_id,
            error_message=str(error), error_type=type(error).__name__, **kw)
    ...
    async def get_by_ticket(self, ticket: int) -> List[JournalEntry]:
        async with self._lock: return [e for e in self._entries if e.mt5_ticket == ticket]
    ...
    async def get_by_event_type(self, event_type: JournalEventType, limit: int = 100) -> List[JournalEntry]:
        async with self._lock:
    ...
    async def get_recent(self, limit: int = 50) -> List[JournalEntry]:
        async with self._lock: return list(reversed(list(self._entries)))[:limit]
    ...
    async def stats(self) -> Dict[str, Any]:
        async with self._lock:
            ...
            n = len(pnl_values)
            return {
                "total_entries": self._total_recorded, "buffer_size": len(self._entries),
                "fills": len(fills), "closes": len(closes),
                "blocked_by_risk": len(blocked), "errors": len(errors),
                "persisted": self._total_persisted, "persist_errors": self._persist_errors,
                "pnl": {
                    "total_trades": n, "wins": len(wins),
                    "win_rate": round(len(wins) / n * 100, 1) if n else 0.0,
                    "net_usd": round(sum(pnl_values), 2) if pnl_values else 0.0,
                    "avg_usd": round(sum(pnl_values) / n, 2) if n else 0.0,
                },
                "slippage": {
                    "avg_pips": round(sum(slip_values) / len(slip_values), 2) if slip_values else 0.0,
                    "max_pips": round(max(slip_values), 2) if slip_values else 0.0,
                },
                "latency": {
                    "avg_ms": round(sum(latency_values) / len(latency_values), 1) if latency_values else 0.0,
