...
class SMCEngine:
    ...
    def __init__(
        self,
        ob_lookback: int = 5,
        fvg_min_size: float = 0.0002,
        swing_lookback: int = 10,
        min_impulse: float = 0.0005,
    ) -> None:
    ...
    def _determine_bias(
        self, candles: List[Candle], swing_high: Optional[float], swing_low: Optional[float]
    ) -> SMCBias:
    ...
    def _find_liquidity(self, candles: List[Candle]) -> List[LiquidityZone]:
        ...
        for c in candles:
            found = any(
                abs(c.high - p) <= tol and (highs.__setitem__(p, highs[p] + 1) or True)
                for p in highs
            )
            if not found:
                highs[c.high] = 1
            found = any(
                abs(c.low - p) <= tol and (lows.__setitem__(p, lows[p] + 1) or True) for p in lows
            )
            if not found:
                lows[c.low] = 1
    ...
    def _calculate_confidence(
        ...
        if structure_event == StructureEvent.BOS:
            score += 0.15
        elif structure_event == StructureEvent.CHoCH:
            score += 0.25
