...
_RE_SQL = re.compile(
    r"(?i)(\bUNION\b.{0,30}\bSELECT\b|\bDROP\b.{0,20}\bTABLE\b|'\s*OR\s*'or'1'\s*=\s*'1|--\s*(?:(|\n)|;\s*DROP|\bEXEC\s*\(|\bSLEEP\s*\(\s*\d)"
)
_RE_XSS = re.compile(r"(?i)(.script[^>]{0,200}>|javascript\s*:|on\w{1,30}\s*=|.iframe[^>]{0,200}>)")
_RE_CMD = re.compile(
    r"(?i)(`[^`]{0,200}`|\$\([^)]{0,200}\)|\|\s*(?:sh|bash|cmd)\b|&&\s*(?:rm|curl|wget)\b)"
)
...
class HardenedSecurityMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, internal_ip_allowlist: Optional[Set[str]] = None):
        ...
        env_set: Set[str] = (
            {ip.strip() for ip in env_ips.split(",") if ip.strip()} if env_ips else set()
        )
        default_ips: Set[str] = {"127.0.0.1", "::1"}
        self._internal_ips: Set[str] = internal_ip_allowlist or env_set or default_ips
        if env_set:
            log.info(
                "HardenedSecurityMiddleware: admin IPs from env ADMIN_IP_ALLOWLIST: %s", env_set
            )
        else:
            log.debug(
                "HardenedSecurityMiddleware: using default internal IPs: %s", self._internal_ips
            )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if any(path.startswith(p) for p in _INTERNAL_PATHS):
            if ip not in self._internal_ips:
                ...
                return JSONResponse(
                    err.to_response(), status_code=403, headers={"X-Request-ID": request_id}
                )
        ...
        if _RE_PATH_TRAVERSAL.search(path) or _RE_PATH_TRAVERSAL.search(raw_qs):
            ...
            return JSONResponse(
                err.to_response(), status_code=400, headers={"X-Request-ID": request_id}
            )
        ...
        try:
            ...
            _apply_headers(
                resp, request_id=request_id, elapsed_ms=(time.monotonic() - start) * 1000
            )
            return resp
...
def apply_security_middleware(
    app: FastAPI, allowed_origins: List[str], trusted_hosts: List[str], trusted_proxies: str = ""
) -> None:
...
_HTTP_TO_EC: Dict[int, str] = {
    400: EC.VALIDATION_ERROR,
    401: EC.AUTH_INVALID,
    403: EC.PERM_DENIED,
    404: EC.NOT_FOUND,
    409: EC.CONFLICT,
    422: EC.VALIDATION_ERROR,
    429: EC.RATE_LIMITED,
    500: EC.INTERNAL_ERROR,
    502: EC.SERVICE_UNAVAILABLE,
    503: EC.SERVICE_UNAVAILABLE,
    504: EC.TIMEOUT,
}
...
def install_exception_handlers(app: FastAPI) -> None:
    ...
    async def http_exc(request: Request, exc: StarletteHTTPException) -> JSONResponse:
        ...
        if exc.status_code < 500 and isinstance(exc.detail, str):
            ...
            return JSONResponse(
                exc.detail, status_code=exc.status_code, headers={"X-Request-ID": request_id}
            )
        return JSONResponse(
            err.to_response(), status_code=exc.status_code, headers={"X-Request-ID": request_id}
        )
    ...
    async def generic_exc(request: Request, exc: Exception) -> JSONResponse:
        request_id = getattr(request.state, "request_id", str(uuid.uuid4()))
        log.exception(
            "unhandled rid=%s path=%s type=%s", request_id, request.url.path, type(exc).__name__
        )
        err = api_error(EC.INTERNAL_ERROR, request_id=request_id)
        return JSONResponse(
            err.to_response(), status_code=500, headers={"X-Request-ID": request_id}
        )
